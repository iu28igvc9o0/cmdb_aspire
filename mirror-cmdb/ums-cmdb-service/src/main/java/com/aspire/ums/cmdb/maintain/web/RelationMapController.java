...
public class RelationMapController {
    ...
    public Object getRelationByCondition(ModuleRelation moduleRelation) {
        logger.info("query getRelationByCondition method params : " + moduleRelation);
...
//      PageHelper.startPage(pageNumber, pageSize ,"cc.isTop DESC, cc.updateTime DESC ");
    ...
    public Object addModuleRelation(@RequestBody ModuleRelationPojo moduleRelationPojo) {
        Map<String, Object> map=new HashMap<String, Object>();
        if (null == moduleRelationPojo) {
            map.put("success", false);
            return map;
        }
        logger.info("add module relation, params is : " + moduleRelationPojo);
        String[] moduleIds = moduleRelationPojo.getModuleIds();
        ...
        for(String moduleId:moduleIds){
            ...
            moduleRelation.setRestriction(moduleRelationPojo.getRestriction());
            moduleRelation.setId(UUIDUtil.getUUID());
            moduleRelation.setBuiltin("false");
            moduleRelation.setRelationId(moduleRelationPojo.getRelationId());
            if(StringUtils.isNotBlank(moduleRelationPojo.getTargetModuleId())){
                moduleRelation.setTargetModuleId(moduleRelationPojo.getTargetModuleId());
                moduleRelation.setSourceModuleId(moduleId);
            }
            if(StringUtils.isNotBlank(moduleRelationPojo.getSourceModuleId())){
                moduleRelation.setSourceModuleId(moduleRelationPojo.getSourceModuleId());
    ...
    public Object deleteModuleRelation(@RequestBody ModuleRelation moduleRelation) {
        logger.info("delete module relation, relation id is : " + moduleRelation.getId());
        Map<String, Object> map=new HashMap<String, Object>();
        if (StringUtils.isEmpty(moduleRelation.getId())) {
            map.put("success", false);
            return map;
        }
