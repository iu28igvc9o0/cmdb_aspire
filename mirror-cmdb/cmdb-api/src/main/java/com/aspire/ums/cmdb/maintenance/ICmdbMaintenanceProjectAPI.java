 ...
 */
@Api(value = "维保项目管理接口")
@RequestMapping("/cmdb/maintenance/project")
public interface ICmdbMaintenanceProjectAPI {
    ...
    /**
     *  保存维保项目
     * @param maintenanceProject 维保项目实例
     * @return
     */
     ...
     * 维保项目绑定设备
     * @param projectId 维保项目标识
     * @param projectInstanceList 维保绑定设备列表
     */
     ...
     * @return
     */
    @PostMapping(value = "/sync/instanceMaintenanceInfo" )
    @ApiOperation(value = "维保项目绑定设备维保信息同步", notes = "维保项目绑定设备维保信息同步", tags = {"Cmdb Maintenance Project API"})
    @ApiResponses(value = {@ApiResponse(code = 200, message = "同步成功", response = JSONObject.class),
            @ApiResponse(code = 500, message = "内部错误")})
    String syncInstanceMaintenanceInfo(@RequestParam(value = "project_id", required = false) String projectId);
