package com.aspire.ums.cmdb.restful.common;
 ...
 * 作者姓名           修改时间           版本号              描述
 */
@RequestMapping("/cmdb/restful/common")
public interface ICommonRestfulAPI {
     ...
     *   {"device_type": "设备类型", "idcType": "资源池名称"}
     */
    @RequestMapping(value = "/instance/listByName", method = RequestMethod.POST)
    @ApiOperation(value = "获取设备列表信息", notes = "获取设备列表信息", tags = {"Cmdb Common Restful API"})
    @ApiResponses(value = {@ApiResponse(code = 200, message = "查询成功", response = List.class),
            @ApiResponse(code = 500, message = "内部错误")})
    Result<Map<String, Object>> getInstanceListByName(@RequestBody Map<String, Object> params);
