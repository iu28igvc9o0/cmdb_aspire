...
public class ExcelReaderUtils {
     ...
     * @throws Exception
     */
    public List<InvalidResourceResp> doReadInvalidResourceData(MultipartFile file) throws Exception {

        List<InvalidResourceResp> list = new ArrayList<InvalidResourceResp>();
        boolean convertSuccess = true;// 时间格式是否正确
        // 指定日期格式为四位年/两位月份/两位日期，注意yyyy/MM/dd区分大小写；
        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
        ...
        for (int j = 1; j <= sheet.getLastRowNum(); j++) {
            ...
            row = sheet.getRow(j);
            int t = 0;
            InvalidResourceResp tpm = new InvalidResourceResp();
            while (t < colNum) {
                // 每个单元格的数据内容用"-"分割开，以后需要时用String类的replace()方法还原数据
                // 也可以将每个单元格的数据设置到一个javabean的属性中，此时需要新建一个javabean
                // str = getStringCellValue(row.getCell(t)).trim();
        ...
        /*
         * } catch (Exception e) { logger.error("软件维保服务excel解析失败:", e); }
         */
        return list;
    }
