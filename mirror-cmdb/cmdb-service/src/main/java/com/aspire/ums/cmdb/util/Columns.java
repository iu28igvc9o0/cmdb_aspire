...
public final class Columns {
    ...
    private static String[] sources = new String[] { "A", "B", "C", "D", "E", "F", "G", "H", "I", "J", "K", "L", "M", "N", "O",
        ...
        for (int i = 26; i < columnNum; i++) { // 第27列(对应数组的第26个位置)开始组合
            ...
            if (remainder == 0) {
                ...
                if (j == 0 && letterIdx != 0) {
                    ...
                    /**
                     * 下个组合的currentLen是上个组合的所有组合情形的个数 （等于上个组合的currentLen*26)，26也就是拼接在前面的A-Z的个数
                     */
            ...
            /**
             * sources[letterIdx%26]是该轮要拼接在前面的字母 columns[remainder+lastLen]是上个组合被拼接的情形
             */
    ...
    public static void main(String[] args) {
        ...
        System.out.println("创建" + columns.length + "列用时（纳秒）：" + (System.nanoTime() - start));
        System.out.println("xls第" + columns.length + "列：" + columns[columns.length - 1]);
        System.out.println();
        start = System.nanoTime();
        columns = getColumnLabels(16384);
        System.out.println("创建" + columns.length + "列用时（纳秒）：" + (System.nanoTime() - start));
        System.out.println("xlsx第" + columns.length + "列：" + columns[columns.length - 1]);

        columns = getColumnLabels(17);
        System.out.println("xlsx第" + columns.length + "列：" + columns[columns.length - 1]);
    }
}
