...
@Slf4j
@NoArgsConstructor
public class IPRepositoryFactory extends ImportFactory {
    ...
    public void execute(Map<String, String> dataMap) {
        ...
        // 补充模型ID字段
        if (!instanceData.containsKey("module_id") || instanceData.get("module_id") == null || ("").equals(instanceData.get("module_id").toString())) {
        ...
        String operatorUser = StringUtils.isEmpty(getProcessParams().get("operatorUser")) ? "系统管理员" : getProcessParams().get("operatorUser");
        if (existsInstanceData != null) {
            ...
            CmdbConfig canNotImportInsertModuleConfig = getCmdbConfigService().getConfigByCode(CmdbConfigConst.CAN_NOT_IMPORT_INSERT_MODULE);
    ...
    private void validCodeValue(CmdbCode cmdbCode, Map<String, String> dataMap, String filedName) {
        ...
        /* todo 级联涉及到ID与VALUE值的切换, 需要后面优化设计方案处理. 以下内容注释, 但不可删除
            // 判断字段是否被其他码表字段级联
            List<CmdbV3CodeCascade> parentCascadeList = codeCascadeService.getByChildCodeId(cmdbCode.getCodeId());
            if (parentCascadeList != null && parentCascadeList.size() > 0) {
                // 处理验证级联数据
                validCascader(parentCascadeList, cmdbCode, dataMap, value);
            }
         */
        // 验证可选值范围
    ...
    private void validDateTime(CmdbCode cmdbCode, String columnValue) {
        try {
            ...
            throw new RuntimeException("列[" + cmdbCode.getFiledName() + "]日期格式不正确, 支持['yyyy-mm-dd','yyyy-mm-dd HH:mm:ss','yyyy/mm/dd']格式");
        }
    }
        ...
        if (options.size() == 0) {
            ...
            if (Constants.YES.equals(cmdbCode.getIsBindSource())) {
                ...
                if (Constants.CODE_SOURCE_TYPE_DICT.equals(bindSource)) {
                    options.addAll(super.getDictList(cmdbCode.getCodeBindSource().getDictSource(),
                            null, null, null));
                }
                //码表绑定数据表
                else if (Constants.CODE_SOURCE_TYPE_TABLE.equals(bindSource)) {
                    try {
                        List<Map<String, Object>> list = super.getSchemaService().executeSql(cmdbCode.getCodeBindSource().getTableSql());
                        list.forEach(map -> {
