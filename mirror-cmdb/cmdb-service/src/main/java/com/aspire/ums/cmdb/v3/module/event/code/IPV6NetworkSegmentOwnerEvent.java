 ...
 * 作者姓名           修改时间           版本号              描述
 */
public class IPV6NetworkSegmentOwnerEvent extends AbstractModuleEvent {
    @Autowired
    ...
    public Map<String, Object> event(String moduleId, String instanceId, Map<String, Object> handleData) {
        ...
        CmdbConfig cmdbConfig = cmdbConfigService.getConfigByCode("ipv6_segment_module_id");
        String segmentModuleId = cmdbConfig.getConfigValue();
        //获取需要从网段中获取的字段
        cmdbConfig = cmdbConfigService.getConfigByCode("from_ipv6_segment_fileds");
        List<String> resultFileds = Arrays.asList(cmdbConfig.getConfigValue().split(","));
        //获取需要不需要设置禁用的字段
        cmdbConfig = cmdbConfigService.getConfigByCode("from_ipv6_segment_fileds_undisabled");
