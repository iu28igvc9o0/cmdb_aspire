<template>
    <el-form :label-position="top" label-width="80px" :model="formAttribute" ref="formAttribute" :rules="fromrules">
        ...
        值：<el-button size="small" style="margin-left:200px;" @click="addOptsion">添加</el-button>
        </div>
        <el-form-item v-for="(item,index) in formAttribute.params.options" :prop="'params.options['+index+'].name'" :rules="[{required: true, message: '请填写选项名称！', trigger: 'blur'},{validator:checkOptionName,trigger: 'blur'}]">
...
export default {
    name: 'multi',
    props: ['multiChildForm','Data','edit','builtin'],
    data() {
        return {
            formAttribute: {
                name: '多选',
    ...
    mounted: function() {
        if(this.multiChildForm !=null){
            this.formAttribute = this.multiChildForm;
        }
        //childFormData父类传递的参数值发生变化时调用
        this.$watch('multiChildForm', function(newVal, oldVal) {
    ...
    methods: {
        ...
        check(index){
            if(this.formAttribute.params.options[index]['isdefault'] == true){
                this.formAttribute.params.options[index]['isdefault'] = false;
            }else{
                this.formAttribute.params.options[index]['isdefault'] = true;
            }
        ...
        checkOptionName(rule, value, callback){
            ...
            for(let i=0;i<tmp.length;i++){
                ...
                if(tmp[i].name == $.trim(value)){
                    return callback(new Error('选项名称已存在'));
                }
