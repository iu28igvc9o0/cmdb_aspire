...
export default {
    name: 'image',
    props: ['imageChildForm','Data','edit','builtin'],
    data() {
        return {
            formAttribute:{
                name:'图片',
    ...
    mounted: function() {
        if(this.imageChildForm !=null){
            this.formAttribute = this.imageChildForm;
        }
        //childFormData父类传递的参数值发生变化时调用
        this.$watch('imageChildForm', function(newVal, oldVal) {
