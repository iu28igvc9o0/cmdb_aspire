<template>
    <el-form :label-position="top" label-width="80px" :model="formAttribute" ref="formAttribute" :rules="fromrules">
        ...
        <el-form-item label="保留几位小数:">
            <el-input-number v-model="formAttribute.params.precision"  :min="1" :max="10"></el-input-number>
        </el-form-item>
        ...
        <el-form-item label="最大值:" required prop="params.max">
            <el-input-number v-model="formAttribute.params.max" :min="1"></el-input-number>
        </el-form-item>
    </el-form>
</template>
<script>
export default {
    name: 'double',
    props: ['doubleChildForm','Data','edit','builtin'],
    data() {
        return {
            formAttribute: {
                name: '小数',
    ...
    mounted: function() {
        if(this.doubleChildForm !=null){
            this.formAttribute = this.parse(this.doubleChildForm);
        }
        //childFormData父类传递的参数值发生变化时调用
        this.$watch('doubleChildForm', function(newVal, oldVal) {
            console.log("---double");
            this.formAttribute = this.parse(this.doubleChildForm);
        }, {
            deep: true
    ...
    methods: {
        ...
        checkDouble(rule, value, callback){
            ...
            if($.trim(value)!='' && !c.test(value)){
                return callback(new Error('抱歉，小数的默认值必须是数值!'));
            }
            callback();
        },
        objDeepCopy(source) {
