<template>
    <el-form :label-position="top" label-width="80px" :model="formAttribute" ref="formAttribute" :rules="fromrules">
        ...
        <el-form-item label="引用对象" prop="params.refClassCode" required>
            <el-select v-model="formAttribute.params.refClassCode" placeholder="请选择">
                <el-option-group v-for="group in options" :key="group.name" :label="group.name">
                    <el-option v-for="item in group.childModules" :key="item.name" :label="item.name" :value="item.id">
                    </el-option>
                </el-option-group>
            </el-select>
        </el-form-item>
...
export default {
    name: 'refer',
    props: ['referChildForm','Data','edit','builtin'],
    data() {
        return {
            formAttribute: {
                name: '',
    ...
    mounted: function() {
        if(this.referChildForm !=null){
            this.formAttribute = this.referChildForm;
        }
        //childFormData父类传递的参数值发生变化时调用
        this.$watch('referChildForm', function(newVal, oldVal) {
            //sconsole.log("---referChildForm");
            this.formAttribute = this.referChildForm;
        }, {
            deep: true
        });
