...
public class CmdbDeviceAssetConsumer extends AbstractCmdbModuleSyncConsumer {
    ...
    @KafkaListener(id = "cmdb-device-asset-sync-kafka-0",
            containerFactory = "ackContainerFactory",
            topicPartitions = { @TopicPartition(topic = "${kafka.osa_topic.osa_cmdb_device_asset:osa_cmdb_device_asset}",
                    partitions = { "0" }) })
    public void listenPartition0(List<ConsumerRecord<?, ?>> records, Acknowledgment ack) {
        log.info("cmdb-device-asset-sync-kafka-0 Listener, Thread ID:{}", Thread.currentThread().getId());
        super.onMessage(records, ack);
    }

    @KafkaListener(id = "cmdb-device-asset-sync-kafka-1",
            containerFactory = "ackContainerFactory",
            topicPartitions = { @TopicPartition(topic = "${kafka.osa_topic.osa_cmdb_device_asset:osa_cmdb_device_asset}",
                    partitions = { "1" }) })
    public void listenPartition1(List<ConsumerRecord<?, ?>> records, Acknowledgment ack) {
        log.info("cmdb-device-asset-sync-kafka-1 Listener, Thread ID:{}", Thread.currentThread().getId());
        super.onMessage(records, ack);
    }

    @KafkaListener(id = "cmdb-device-asset-sync-kafka-2",
            containerFactory = "ackContainerFactory",
            topicPartitions = { @TopicPartition(topic = "${kafka.osa_topic.osa_cmdb_device_asset:osa_cmdb_device_asset}",
                    partitions = { "2" }) })
    public void listenPartition2(List<ConsumerRecord<?, ?>> records, Acknowledgment ack) {
        log.info("cmdb-device-asset-sync-kafka-2 Listener, Thread ID:{}", Thread.currentThread().getId());
        super.onMessage(records, ack);
    }

    @KafkaListener(id = "cmdb-device-asset-sync-kafka-3",
            containerFactory = "ackContainerFactory",
            topicPartitions = { @TopicPartition(topic = "${kafka.osa_topic.osa_cmdb_device_asset:osa_cmdb_device_asset}",
                    partitions = { "3" }) })
    public void listenPartition3(List<ConsumerRecord<?, ?>> records, Acknowledgment ack) {
        log.info("cmdb-device-asset-sync-kafka-3 Listener, Thread ID:{}", Thread.currentThread().getId());
        super.onMessage(records, ack);
    }

    @KafkaListener(id = "cmdb-device-asset-sync-kafka-4",
            containerFactory = "ackContainerFactory",
            topicPartitions = { @TopicPartition(topic = "${kafka.osa_topic.osa_cmdb_device_asset:osa_cmdb_device_asset}",
                    partitions = { "4" }) })
    public void listenPartition4(List<ConsumerRecord<?, ?>> records, Acknowledgment ack) {
        log.info("cmdb-device-asset-sync-kafka-4 Listener, Thread ID:{}", Thread.currentThread().getId());
        super.onMessage(records, ack);
    }

    @KafkaListener(id = "cmdb-device-asset-sync-kafka-5",
            containerFactory = "ackContainerFactory",
            topicPartitions = { @TopicPartition(topic = "${kafka.osa_topic.osa_cmdb_device_asset:osa_cmdb_device_asset}",
                    partitions = { "5" }) })
    public void listenPartition5(List<ConsumerRecord<?, ?>> records, Acknowledgment ack) {
        log.info("cmdb-device-asset-sync-kafka-5 Listener, Thread ID:{}", Thread.currentThread().getId());
        super.onMessage(records, ack);
    }

    @KafkaListener(id = "cmdb-device-asset-sync-kafka-6",
            containerFactory = "ackContainerFactory",
    ...
    public void listenPartition6(List<ConsumerRecord<?, ?>> records, Acknowledgment ack) {
        log.info("cmdb-device-asset-sync-kafka-6 Listener, Thread ID:{}", Thread.currentThread().getId());
        super.onMessage(records, ack);
    }

    @KafkaListener(id = "cmdb-device-asset-sync-kafka-7",
            containerFactory = "ackContainerFactory",
            topicPartitions = { @TopicPartition(topic = "${kafka.osa_topic.osa_cmdb_device_asset:osa_cmdb_device_asset}",
                    partitions = { "7" }) })
    public void listenPartition7(List<ConsumerRecord<?, ?>> records, Acknowledgment ack) {
        log.info("cmdb-device-asset-sync-kafka-7 Listener, Thread ID:{}", Thread.currentThread().getId());
        super.onMessage(records, ack);
    }

    @KafkaListener(id = "cmdb-device-asset-sync-kafka-8",
            containerFactory = "ackContainerFactory",
            topicPartitions = { @TopicPartition(topic = "${kafka.osa_topic.osa_cmdb_device_asset:osa_cmdb_device_asset}",
                    partitions = { "8" }) })
    public void listenPartition8(List<ConsumerRecord<?, ?>> records, Acknowledgment ack) {
        log.info("cmdb-device-asset-sync-kafka-8 Listener, Thread ID:{}", Thread.currentThread().getId());
        super.onMessage(records, ack);
    }

    @KafkaListener(id = "cmdb-device-asset-sync-kafka-9",
            containerFactory = "ackContainerFactory",
            topicPartitions = { @TopicPartition(topic = "${kafka.osa_topic.osa_cmdb_device_asset:osa_cmdb_device_asset}",
                    partitions = { "9" }) })
    public void listenPartition9(List<ConsumerRecord<?, ?>> records, Acknowledgment ack) {
        log.info("cmdb-device-asset-sync-kafka-9 Listener, Thread ID:{}", Thread.currentThread().getId());
        super.onMessage(records, ack);
    }
    ...
    protected CmdbSyncModuleCondition buildFieldCondition() {
        return new CmdbSyncModuleCondition(KafkaConfigConstant.MAPPING_MODULE, "",
                KafkaConfigConstant.MAPPING_TYPE_CMDB_DEVICE_ASSET, KafkaConfigConstant.MAPPING_KEY_FIELD);
    }

    @Override
    protected CmdbSyncType getSyncType() {
        return CmdbSyncType.CMDB_DEVICE;
    }
}
