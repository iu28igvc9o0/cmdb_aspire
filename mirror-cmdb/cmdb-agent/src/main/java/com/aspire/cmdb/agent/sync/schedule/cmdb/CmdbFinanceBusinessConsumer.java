...
public class CmdbFinanceBusinessConsumer extends AbstractCmdbModuleSyncConsumer {

    protected Logger log = LoggerFactory.getLogger(this.getClass());

    @KafkaListener(id = "cmdb-business-sync-kafka-0",
            containerFactory = "ackContainerFactory",
            topicPartitions = { @TopicPartition(topic = "${kafka.osa_topic.osa_cmdb_business:osa_cmdb_business}",
                    partitions = { "0" }) })
    public void listenPartition0(List<ConsumerRecord<?, ?>> records, Acknowledgment ack) {
        log.info("cmdb-business-sync-kafka-0 Listener, Thread ID:{}", Thread.currentThread().getId());
        super.onMessage(records, ack);
    }

    @KafkaListener(id = "cmdb-business-sync-kafka-1",
            containerFactory = "ackContainerFactory",
            topicPartitions = { @TopicPartition(topic = "${kafka.osa_topic.osa_cmdb_business:osa_cmdb_business}",
                    partitions = { "1" }) })
    public void listenPartition1(List<ConsumerRecord<?, ?>> records, Acknowledgment ack) {
        log.info("cmdb-business-sync-kafka-1 Listener, Thread ID:{}", Thread.currentThread().getId());
        super.onMessage(records, ack);
    }

    @KafkaListener(id = "cmdb-business-sync-kafka-2",
            containerFactory = "ackContainerFactory",
            topicPartitions = { @TopicPartition(topic = "${kafka.osa_topic.osa_cmdb_business:osa_cmdb_business}",
                    partitions = { "2" }) })
    public void listenPartition2(List<ConsumerRecord<?, ?>> records, Acknowledgment ack) {
        log.info("cmdb-business-sync-kafka-2 Listener, Thread ID:{}", Thread.currentThread().getId());
        super.onMessage(records, ack);
    }

    @KafkaListener(id = "cmdb-business-sync-kafka-3",
            containerFactory = "ackContainerFactory",
            topicPartitions = { @TopicPartition(topic = "${kafka.osa_topic.osa_cmdb_business:osa_cmdb_business}",
                    partitions = { "3" }) })
    public void listenPartition3(List<ConsumerRecord<?, ?>> records, Acknowledgment ack) {
        log.info("cmdb-business-sync-kafka-3 Listener, Thread ID:{}", Thread.currentThread().getId());
        super.onMessage(records, ack);
    }

    @KafkaListener(id = "cmdb-business-sync-kafka-4",
            containerFactory = "ackContainerFactory",
            topicPartitions = { @TopicPartition(topic = "${kafka.osa_topic.osa_cmdb_business:osa_cmdb_business}",
                    partitions = { "4" }) })
    public void listenPartition4(List<ConsumerRecord<?, ?>> records, Acknowledgment ack) {
        log.info("cmdb-business-sync-kafka-4 Listener, Thread ID:{}", Thread.currentThread().getId());
        super.onMessage(records, ack);
    }

    @KafkaListener(id = "cmdb-business-sync-kafka-5",
            containerFactory = "ackContainerFactory",
            topicPartitions = { @TopicPartition(topic = "${kafka.osa_topic.osa_cmdb_business:osa_cmdb_business}",
                    partitions = { "5" }) })
    public void listenPartition5(List<ConsumerRecord<?, ?>> records, Acknowledgment ack) {
        log.info("cmdb-business-sync-kafka-5 Listener, Thread ID:{}", Thread.currentThread().getId());
        super.onMessage(records, ack);
    }

    @KafkaListener(id = "cmdb-business-sync-kafka-6",
            containerFactory = "ackContainerFactory",
    ...
    public void listenPartition6(List<ConsumerRecord<?, ?>> records, Acknowledgment ack) {
        log.info("cmdb-business-sync-kafka-6 Listener, Thread ID:{}", Thread.currentThread().getId());
        super.onMessage(records, ack);
    }

    @KafkaListener(id = "cmdb-business-sync-kafka-7",
            containerFactory = "ackContainerFactory",
            topicPartitions = { @TopicPartition(topic = "${kafka.osa_topic.osa_cmdb_business:osa_cmdb_business}",
                    partitions = { "7" }) })
    public void listenPartition7(List<ConsumerRecord<?, ?>> records, Acknowledgment ack) {
        log.info("cmdb-business-sync-kafka-7 Listener, Thread ID:{}", Thread.currentThread().getId());
        super.onMessage(records, ack);
    }

    @KafkaListener(id = "cmdb-business-sync-kafka-8",
            containerFactory = "ackContainerFactory",
            topicPartitions = { @TopicPartition(topic = "${kafka.osa_topic.osa_cmdb_business:osa_cmdb_business}",
                    partitions = { "8" }) })
    public void listenPartition8(List<ConsumerRecord<?, ?>> records, Acknowledgment ack) {
        log.info("cmdb-business-sync-kafka-8 Listener, Thread ID:{}", Thread.currentThread().getId());
        super.onMessage(records, ack);
    }

    @KafkaListener(id = "cmdb-business-sync-kafka-9",
            containerFactory = "ackContainerFactory",
            topicPartitions = { @TopicPartition(topic = "${kafka.osa_topic.osa_cmdb_business:osa_cmdb_business}",
                    partitions = { "9" }) })
    public void listenPartition9(List<ConsumerRecord<?, ?>> records, Acknowledgment ack) {
        log.info("cmdb-business-sync-kafka-9 Listener, Thread ID:{}", Thread.currentThread().getId());
        super.onMessage(records, ack);
    }
    ...
    protected CmdbSyncType getSyncType() {
        return CmdbSyncType.FINANCE_CMDB_BUSINESS;
    }
}
