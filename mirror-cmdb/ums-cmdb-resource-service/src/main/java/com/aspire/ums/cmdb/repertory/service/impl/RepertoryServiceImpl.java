...
public class RepertoryServiceImpl implements RepertoryService {
	 ...
	 * @param workbook
	 */
	public void generateExcelForAs(List<PoiTitle> titles, List<Map<String, String>> contents, HSSFWorkbook workbook) {
		// 样式
	...
	public byte[] getExcelData(String moduleId) {
		...
		try {
			...
			generateExcelForAs(getTitle(moduleId), getContent(moduleId), workbook);
	...
	public byte[] getExcel(String moduleId) {
		...
		try {
			HSSFWorkbook workbook = new HSSFWorkbook();
			generateExcelForAs(getTitle(moduleId), null, workbook);
			out = new ByteArrayOutputStream();
			HSSFWorkbook hssWb = (HSSFWorkbook) workbook;
			hssWb.write(out);
		} catch (IOException e) {
	...
	@Override
	public Map<String, Object> uploadData(MultipartFile file, Module module, String circleId) {
		...
		try {
			...
			map = readExcel(wb, module,circleId);
		} catch (Exception e) {
	...
	public Map<String, Object> readExcel(Workbook wb, Module module, String circleId) {
		Map<String, Object> map = new HashMap<String, Object>();
		// 得到第一个shell
...
//			                    List<Map> clist = moduleRelationMapper.checkRelationInstanceList(inMap);
		...
		for (int c = 1; c < pts.size(); c++) {
			...
			if (Constants.MODULE_RELATION_SOURCE.equals(pt.getFieldType())) {
				...
				formValue.setFormCode(pt.getFieldType());
