...
@Configuration
@EnableConfigurationProperties(ZabbixDruidProperties.class)
@ConditionalOnClass(DruidDataSource.class)
@ConditionalOnExpression("${zabbix.dbConfig.switch:true}")
@MapperScan(basePackages = "com.aspire.ums.cdn.dao.zabbix", sqlSessionTemplateRef = "zabbixDbSqlSessionTemplate")
public class ZabbixDruidAutoConfiguration {
	@Autowired
	private ZabbixDruidProperties dbConfig;
    ...
    public DataSourceTransactionManager zabbixDbTransactionManager(
    						@Qualifier("zabbix") DataSource dataSource) {
	...
	public SqlSessionFactory zabbixDbSessionFactory(
			@Qualifier("zabbix") DataSource dataSource) throws Exception {
    ...
    public SqlSessionTemplate zabbixDbSqlSessionTemplate(
    		@Qualifier("zabbix") SqlSessionFactory sqlSessionFactory) throws Exception {
