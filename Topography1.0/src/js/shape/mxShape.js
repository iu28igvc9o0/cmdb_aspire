...
mxShape.prototype.configureCanvas = function(c, x, y, w, h)
{
	...
	// 添加class属性
	if (this.style && this.style.class) {
		c.setClassName(this.style.class)
	}
...
mxShape.prototype.addPointsNew = function(intersectionArr = [], c, pts, rounded, arcSize, close, exclude, initialMove)
{
	...
	if (pts != null && pts.length > 0)
	{
		...
		for (var j = 0; j < pts.length; j ++) {
			if (j < pts.length - 1) {
				newArr.push([pts[j], pts[j + 1]])
			}
		}
		console.log(newArr)
		var dotOnLineValue
		var newPts = JSON.parse(JSON.stringify(pts))
		for (var t = 0; t < intersectionArr.length; t ++) {
			for (var d = 0; d < newArr.length; d ++) {
				dotOnLineValue = dotOnLine([intersectionArr[0].x, intersectionArr[0].y], [newArr[d][0].x, newArr[d][0].y], [newArr[d][1].x, newArr[d][1].y], d)
				if (Object.keys(dotOnLineValue).length > 0) {
					newPts.splice(dotOnLineValue.index + 1, 0, {
						x: dotOnLineValue.dot[0] - 10,
						y: dotOnLineValue.dot[1]
					}, {
						x: dotOnLineValue.dot[0] + 10,
						y: dotOnLineValue.dot[1]
					})
		...
		console.log('是否在线上？', dotOnLineValue)
		console.log(newPts)
		while (i < ((close) ? newPts.length : newPts.length - 1))
		{

			var tmp = newPts[mxUtils.mod(i, newPts.length)];
			var dx = pt.x - tmp.x;
			var dy = pt.y - tmp.y;
...
function dotOnLine(dot = [], terminal1 = [], terminal2 = [], t){
