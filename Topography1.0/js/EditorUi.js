...
EditorUi = function (editor, container, lightbox) {
  ...
  graph.addListener(mxEvent.CLICK, function (sender, evt) {
    ...
    //   try {
    //     graph.setCellStyles('aspect', 'fixed', graph.getSelectionCells());
    //   } finally {
  ...
  // 添加卓望版权
  var $geDiagramContainer = document.querySelector('.geDiagramContainer');
  var $copyright = document.createElement('div');
  $copyright.classList.add('copyright')
  $copyright.innerHTML = 'Copyright©2019-2025 卓望公司版权所有'
  $geDiagramContainer.appendChild($copyright)

  // if (window.frames.length === parent.frames.length) {
  //   document.body.classList.add('has-logo')
  //   var $topbar = document.createElement('div')
  //   $topbar.classList.add('geTopbarContainer')
  //
  //   var $logo = document.createElement('div');
  //   $logo.classList.add('geTopbarLogo')
  ...
  var styles = ['rounded', 'shadow', 'glass', 'dashed', 'dashPattern', 'comic', 'flow','labelBackgroundColor'];
  var connectStyles = ['shape', 'edgeStyle', 'curved', 'rounded', 'elbow', 'comic', 'flow', 'jumpStyle', 'jumpSize'];
  ...
  this.addListener('styleChanged', mxUtils.bind(this, function (sender, evt) {
    // Checks if edges and/or vertices were modified   检查是否修改了边和/或顶点
  ...
  if (this.editor.isChromelessView()) {
    ...
    graph.sizeDidChange = function () {
      if (this.container != null && mxUtils.hasScrollbars(this.container)) {
        ...
        // var minw = Math.ceil(pad.x);
        // var minh = Math.ceil(pad.y);
  ...
  if (!this.editor.extendCanvas) {
    ...
    if (mxUtils.hasScrollbars(graph.container)) {
      if (graph.pageVisible) {
        ...
        graph.container.scrollLeft = Math.floor(Math.min(pad.x * 2,
...
EditorUi.prototype.updateActionStates = function () {
  ...
  let ClipboardA = document.getElementsByClassName('Cross_pageCopyData')[0]
  if (ClipboardA !== undefined) {
    if (localStorage.getItem('Cross_pageCopyDataEdges') || localStorage.getItem('Cross_pageCopyDataEdge')) {
      ClipboardA.classList.remove("mxDisabled")
    } else {
...
if(!edgeSelected && vertexSelected && graph.getSelectionCount() > 1 ){
  var arr=document.getElementsByClassName('alignShow')
    for(i = 0; i < arr.length; i++ ){
      arr[i].classList.remove("mxDisabled")
    }
} else{
  var arr=document.getElementsByClassName('alignShow')
  for(i = 0; i < arr.length; i++ ){
    arr[i].classList.add("mxDisabled")
  }
}
...
EditorUi.prototype.refresh = function (sizeDidChange) {
  ...
  this.sidebarContainer.style.top = tmp -34 + 'px';
  this.sidebarContainer.style.zIndex = 2 ;
  this.sidebarContainer.style.borderRight = '1px  solid #e6e6e6' ;
  this.sidebarContainer.style.width = effHsplitPosition + 'px';
  this.formatContainer.style.top = tmp -34 + 'px';
  this.formatContainer.style.borderLeft = '1px  solid #e6e6e6' ;
  ...
  this.diagramContainer.style.top = '34px';
...
EditorUi.prototype.createDivs = function () {
  ...
  this.toolbarContainer.classList.add('has-logo-toolbar')
  this.sidebarContainer.style.left = '0px';
  this.formatContainer.style.right = '0px';
  this.formatContainer.style.zIndex = '3';
...
EditorUi.prototype.createUi = function () {
  ...
  if (this.sidebar != null) {
    ...
    this.addSplitHandler(this.hsplit, true, 0, mxUtils.bind(this, function (value) {
      ...
      this.toolbar.container.style.paddingLeft = value + 20 + 'px'
      this.refresh();
    }));
  }
};
...
EditorUi.prototype.saveFile = function (forceDialog) {
  if (!forceDialog) {
    if (window.systemType === 0) {
       // save
     sendMessage('warning', '保存前建议检查配置设备编码!', 300)
      // 调用弹窗
      toast(this,1)
    } else {
...
EditorUi.prototype.save = function (name) {
  if (name != null) {
    ...
    var nodes = this.editor.graph.getModel().cells;
    ...
    Object.keys(nodes).forEach(function (key) {
      var state = self.editor.graph.view.getState(nodes[key]);
      var style = state.style;
      if (nodes[key].geometry && state.style.isBind) {
        arr.push({
          name: style.name ? JSON.parse(style.name) : null,
          unit: style.unit ? JSON.parse(style.unit) : null,
          conversionType: style.conversionType ? JSON.parse(style.conversionType) : null,
          conversionVal: style.conversionVal ? JSON.parse(style.conversionVal) : null,
          valueView: JSON.parse(style.valueView) || false,
          colorView: JSON.parse(style.colorView) || false,
          isBind: JSON.parse(style.isBind) || false,
          bandType: JSON.parse(style.bandType) || 0,
          sillList: JSON.parse(style.sillList) || null,
          countType: JSON.parse(style.countType) || null,
          id: state.cell.id || null,
          alertLevelList: JSON.parse(style.alertLevelList) || null,
          deviceList: JSON.parse(style.deviceList) || null,
          itemList: JSON.parse(style.itemList) || null
        })
      }
    })
    ...
    window.parent.postMessage({
      ...
      params: {
        ...
        bindObj: JSON.stringify(arr),
        name: sessionStorage.getItem('viewTitle'),
        idc: sessionStorage.getItem('idcType'),
        pod: sessionStorage.getItem('pod'),
        bizSystem: sessionStorage.getItem('bizSystem'),
