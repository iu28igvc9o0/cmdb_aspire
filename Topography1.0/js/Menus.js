...
Menus.prototype.init = function () {
	...
	// Two special dropdowns that are only used in the toolbar 只有在工具栏中使用的两个特殊下拉框
		...
		if (state != null) {
			...
			if (graph.getSelectionCount() == 1) {
				...
				if (graph.getModel().isVertex(cell) && mxUtils.getValue(state.style, mxConstants.STYLE_IMAGE, null) != null) {
					menu.addSeparator();
					// this.addMenuItem(menu, 'image', null, evt).firstChild.nextSibling.innerHTML = mxResources.get('editImage') + '...'; // 注释掉编辑图像
				}
			}
		}
	} else {
...
/**
 * Sets the enabled state of the action and fires a stateChanged event. 设置操作的启用状态并触发stateChanged事件
 */
