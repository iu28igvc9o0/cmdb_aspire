...
Actions.prototype.init = function () {
	...
	this.addAction('copy', function () {
		...
		let Cross_pageCopyDataEdge = []
		let Cross_pageCopyDataEdges = []
		mxClipboard.cells.forEach(function (item) {
			if (item.edge && !item.source && !item.target) {
				let itemEdge = {
					...
					sourcePoint: {
						x: item.geometry.sourcePoint.x,
						y: item.geometry.sourcePoint.y
					},
					targetPoint: {
						x: item.geometry.targetPoint.x,
						y: item.geometry.targetPoint.y
					}
				}
				let points = []
				if (item.geometry.points) {
					item.geometry.points.forEach(function (item) {
						points.push({
							x: item.x,
							y: item.y
						})
					})
					itemEdge.points = points
				}
				Cross_pageCopyDataEdge.push(itemEdge)
			} else if (!item.edge && item.vertex) {
				Cross_pageCopyDataEdges.push({
					vertex: true,
					_copyIndex: item.geometry._copyIndex,
					value: item.value,
					style: item.style,
					geometry: {
						x: item.geometry.x,
			...
			} else if (item.edge && (item.source || item.target)) {
				if (item.target && item.source) {
					Cross_pageCopyDataEdges.push({
						edges: 1,
						source: item.source.geometry._copyIndex,
						target: item.target.geometry._copyIndex,
						style: item.style,
						value: item.value
					})
				} else if (!item.target && item.source) {
					Cross_pageCopyDataEdges.push({
						edges: 2,
						source: item.source.geometry._copyIndex,
						targetPoint: {
		...
		localStorage.setItem('Cross_pageCopyDataEdge', JSON.stringify(Cross_pageCopyDataEdge))
		localStorage.setItem('Cross_pageCopyDataEdges', JSON.stringify(Cross_pageCopyDataEdges))
	...
	this.addAction('backgroundColor...', function () {
		ui.menus.pickColor(mxConstants.STYLE_LABEL_COLOR, 'backcolor');
	});
	...
	// 绑定按钮
	this.addAction('setBindNode...', mxUtils.bind(this, function () {
		var cells = graph.getSelectionCells();
		// 向父vue页面发送信息
	...
	action = this.addAction('outline', mxUtils.bind(this, function () {
		...
		// 	this.outlineWindow = new OutlineWindow(ui, 200, 200, 240, 240);
