...
function Sidebar(editorUi, container) {
	...
	// Handles mouse leaving the window 处理鼠标离开窗口
...
Sidebar.prototype.init = function () {
	...
	this.addtitlePalette(false)
	this.addImagePaletteBase('base', mxResources.get('base'), dir + '/editor/base/', '_base.svg',
		['dongtaiwenben'], null, {
			'Wireless_Router_N': 'wireless router switch wap wifi access point wlan',
			'Router_Icon': 'router switch'
		}, false);
	// this.addImagePalette('animation', mxResources.get('animation'), dir + '/editor/base/', '_base.svg',
	// 	['animation'], null, {
	// 		'Wireless_Router_N': 'wireless router switch wap wifi access point wlan',
	// 		'Router_Icon': 'router switch'
	// 	}, false);
	// this.addImagePalette('svg', mxResources.get('svg'), dir + '/editor/base/', '_base.svg',
	// 	['pue'], null, {
	// 		'Wireless_Router_N': 'wireless router switch wap wifi access point wlan',
	// 		'Router_Icon': 'router switch'
	// 	}, true);
	...
	if (window.systemType === 0) {
		// 根据类型显示隐藏元件
		if (window.elementType) {
			this.addImagePaletteWarning('Equipment', mxResources.get('Equipment'), dir + '/editor/equipment/', '_E.svg',
				['byq', 'yj', 'UPS', 'PDU', 'dcz', 'djdbyq'], null, {
					'Wireless_Router_N': 'wireless router switch wap wifi access point wlan',
					'Router_Icon': 'router switch'
				}, false); // 设备

			this.addImagePalette('Distribution', mxResources.get('Distribution'), dir + '/editor/DistributionComponents/', '_D.svg',
				['shidian', 'dly', 'bhd', 'jiedi', 'djkg', 'ejkg', 'sjkg', 'dl', 'dlhg', 'drbc1', 'drbc2', 'dyhgqg', 'gldyhgqg', 'glmx', 'kg', 'mulian', 'scskg'], null, {
					'Wireless_Router_N': 'wireless router switch wap wifi access point wlan',
					'Router_Icon': 'router switch'
				}, false); // 配电
		}
		...
		this.addImagePalettesimulationDiYa('SimulationDiYa', mxResources.get('SimulationDiYa'), dir + '/editor/simulationDiya/', '_S.svg',
			['ATS', 'byq', 'yj', 'dycxg', 'dyjxg', 'pdg', 'ltg','wmk'], null, {
				'Wireless_Router_N': 'wireless router switch wap wifi access point wlan',
				'Router_Icon': 'router switch'
			}, false);
		this.addImagePalettesimulationPeiDian('SimulationPeiDian', mxResources.get('SimulationPeiDian'), dir + '/editor/simulationPeiDian/', '_S.svg',
			['UPS', 'dykg', 'gyzl'], null, {
				'Wireless_Router_N': 'wireless router switch wap wifi access point wlan',
				'Router_Icon': 'router switch'
			}, false);
			this.addImagePaletteOthers('Others', mxResources.get('Others'), dir + '/editor/newpicture/', '_C.svg',
				['zm'], null, {
					'Wireless_Router_N': 'wireless router switch wap wifi access point wlan',
					'Router_Icon': 'router switch'
				}, false); // 其他
		...
		// 根据类型显示隐藏元件
		if (window.elementType) {
			this.addImagePaletteMakeCool('MakeCool', mxResources.get('MakeCool'), dir + '/editor/newpicture/', '_C.svg',
			['beng', 'lqt', 'xlg', 'fsysj','dbr','fmkzg','zm','zlj10'], null, {
				'Wireless_Router_N': 'wireless router switch wap wifi access point wlan',
				'Router_Icon': 'router switch'
			}, false); // 制冷
		}
	} else if (window.systemType === 1) {
		//中移信息图标数据
		this.addImagePaletteSwitchDevice('Switch', mxResources.get('Switch'), dir + '/editor/switch/', '_switch.svg',
		['access', 'agg', 'core', 'CE12800', 'normal', 'stack', 'stack_CE', 'stack_TOR', 'TOR', 'vm'], null, {
			'Wireless_Router_N': 'wireless router switch wap wifi access point wlan',
			'Router_Icon': 'router switch'
		}, false);

		this.addImagePaletteFirewallDevice('Firewall', mxResources.get('Firewall'), dir + '/editor/firewall/', '_firewall.svg',
			['a','b'], null, {
				'Wireless_Router_N': 'wireless router switch wap wifi access point wlan',
				'Router_Icon': 'router switch'
			}, false);
		this.addImagePaletteRouterDevice('Router', mxResources.get('Router'), dir + '/editor/router/', '_router.svg',
			['core'], null, {
				'Wireless_Router_N': 'wireless router switch wap wifi access point wlan',
				'Router_Icon': 'router switch'
			}, false);
		this.addImagePaletteLoadBalanceDevice('LoadBalance', mxResources.get('LoadBalance'), dir + '/editor/loadBalance/', '_f5.svg',
			['normal'], null, {
				'Wireless_Router_N': 'wireless router switch wap wifi access point wlan',
				'Router_Icon': 'router switch'
			}, false);
		this.addImagePaletteServiceDevice('Service', mxResources.get('Service'), dir + '/editor/service/', '_service.svg',
			['normal'], null, {
				'Wireless_Router_N': 'wireless router switch wap wifi access point wlan',
				'Router_Icon': 'router switch'
			}, false);
		this.addImagePaletteInternetDevice('Internet', mxResources.get('Internet'), dir + '/editor/internet/', '_internet.svg',
			['normal', 'ip', 'LAN','SAN','WAN'], null, {
				'Wireless_Router_N': 'wireless router switch wap wifi access point wlan',
				'Router_Icon': 'router switch'
			}, false);
	}
	// this.addSearchPalette(false);
	// this.addGeneralPalette(false); // 通用
	// this.addMiscPalette(false); // 杂项
	// this.addAdvancedPalette(false); // 高级m
...
/**
 * Specifies the width of the thumbnails. // sidebar 里面的图标大小
 */
 ...
 */
Sidebar.prototype.thumbHeight = 32;
 ...
 */
Sidebar.prototype.defaultImageWidth = 48;

/**
 * Specifies the height for clipart images. Default is 80.
 */
Sidebar.prototype.defaultImageHeight = 48;
	...
	if (this.enableTooltips && this.showTooltips) {
		if (this.currentElt != elt) {
			...
			var show = mxUtils.bind(this, function () {
				// Lazy creation of the DOM nodes and graph instance 懒加载创建DOM节点和图实例
...
/**
 * Hides the current tooltip. 隐藏当前工具提示。
 */
...
Sidebar.prototype.addEntry = function (tags, fn) {
	if (this.taglist != null && tags != null && tags.length > 0) {
		...
		for (var i = 0; i < tmp.length; i++) {
			...
			// Adds additional entry with removed trailing numbers 添加附加项，并删除尾随号
...
Sidebar.prototype.filterTags = function (tags) {
	if (tags != null) {
		...
		// Ignores tags with leading numbers, strips trailing numbers 忽略带前导数字的标记，删除尾随数字
		for (var i = 0; i < arr.length; i++) {
			// Removes duplicates 删除重复的
			if (hash[arr[i]] == null) {
				hash[arr[i]] = '1';
				result.push(arr[i]);
			}
...
/**
 * Adds the general palette to the sidebar. 将常规调色板添加到侧栏。
 */
...
// addtitlePalette
Sidebar.prototype.addtitlePalette = function (expand) {
	let a = document.createElement("a")
	let text = document.createTextNode("元件库")
	// a.className = 'geTitle'
	a.className += ' leftTopBar'
	a.appendChild(text)
...
Sidebar.prototype.addSearchPalette = function (expand) {
	...
	// Needed to block event transparency in IE 需要在IE中阻止事件透明性
	...
	// Workaround for inherited line-height in quirks mode 在quirks模式下处理继承的行高
	...
	// Count is dynamically updated below     总数在下面动态更新
	var count = 12;
					...
					this.searchEntries(searchTerm, count, page, mxUtils.bind(this, function (results, len, more, terms) {
						if (this.currentSearch == current) {
							...
							for (var i = 0; i < results.length; i++) {
								...
								// Avoids duplicates in results 避免结果重复
	...
	// Workaround for blocked text selection in Editor 解决编辑器中被阻止的文本选择问题
	...
	// Keeps references to the DOM nodes 保存对DOM节点的引用
...
Sidebar.prototype.addGeneralPalette = function (expand) {
	...
	var fns = [
		...
		this.addEntry('curve', mxUtils.bind(this, function () { // s弯箭头曲线
		...
		this.createEdgeTemplateEntry('endArrow=none;dashed=1;html=1;', 50, 50, '', 'Dashed Line', null, lineTags + 'dashed undirected no'), // 虚线
...
Sidebar.prototype.addMiscPalette = function (expand) {
	...
	var fns = [
		this.createVertexTemplateEntry('shape=text;strokeColor=none;fillColor=none;html=1;fontSize=24;fontStyle=1;verticalAlign=middle;align=center;', 100, 40, 'Title', 'Title', null, null, 'text heading title'),
		this.createVertexTemplateEntry('shape=text;strokeColor=none;fillColor=none;html=1;whiteSpace=wrap;verticalAlign=middle;overflow=hidden;', 100, 80,
		...
		this.createVertexTemplateEntry('shape=text;html=1;strokeColor=#c0c0c0;fillColor=#ffffff;overflow=fill;rounded=0;', 280, 160,
		...
		this.createVertexTemplateEntry('shape=text;html=1;strokeColor=#c0c0c0;fillColor=none;overflow=fill;', 180, 140,
			...
			'<tr><td align="center">Value 7</td><td align="center">Value 8</td><td align="center">Value 9</td></tr></table>', 'Table 2'), // 表格
		this.createVertexTemplateEntry('shape=text;html=1;strokeColor=none;fillColor=none;overflow=fill;', 180, 140,
			'<table border="1" width="100%" height="100%" style="width:100%;height:100%;border-collapse:collapse;">' +
			'<tr><td align="center">Value 1</td><td align="center">Value 2</td><td align="center">Value 3</td></tr>' +
			'<tr><td align="center">Value 4</td><td align="center">Value 5</td><td align="center">Value 6</td></tr>' +
			'<tr><td align="center">Value 7</td><td align="center">Value 8</td><td align="center">Value 9</td></tr></table>', 'Table 3'), // 表格
		this.createVertexTemplateEntry('shape=text;html=1;strokeColor=none;fillColor=none;overflow=fill;', 160, 140,
			'<table border="1" width="100%" height="100%" cellpadding="4" style="width:100%;height:100%;border-collapse:collapse;">' +
		...
		this.createEdgeTemplateEntry('rounded=0;comic=1;strokeWidth=2;endArrow=blockThin;html=1;fontFamily=Comic Sans MS;fontStyle=1;', 500, 500, '', 'Comic Arrow'),
		...
		// this.createVertexTemplateEntry('line;strokeWidth=2;direction=south;html=1;', 10, 160, '', 'Vertical Line'), // 竖线
...
/**
 * Adds the container palette to the sidebar.将容器调色板添加到侧栏。
 */
...
Sidebar.prototype.createAdvancedShapes = function () {
	// Avoids having to bind all functions to "this" 避免将所有函数绑定到“this”
	var sb = this;

	// Reusable cells 可重用的单元格
	...
	return [
		...
		this.createVertexTemplateEntry('shape=swimlane;', 200, 200, 'Container', 'Container', null, null, 'container swimlane lane pool group'),
...
/**
 * Adds the BPMN library to the sidebar. 将BPMN库添加到侧栏。
 */
...
/**
 * Creates and returns the given title element. 创建并返回给定的title元素。
 */
...
/**
 * Creates a thumbnail for the given cells. 为给定的单元格创建缩略图。
 */
Sidebar.prototype.createThumb = function (cells, width, height, parent, title, showLabel, showTitle, realWidth, realHeight) {
	...
	// For supporting HTML labels in IE9 standards mode the container is cloned instead 为了支持IE9标准模式下的HTML标签，容器被克隆
	if (this.graph.dialect == mxConstants.DIALECT_SVG && !mxClient.NO_FO) {
		node = this.graph.view.getCanvas().ownerSVGElement.cloneNode(true);
	}
	// LATER: Check if deep clone can be used for quirks if container in DOM 稍后:检查是否可以使用深度克隆来处理DOM中的容器中的怪癖
	else {
		node = this.graph.container.cloneNode(false);
		node.innerHTML = this.graph.container.innerHTML;

		// Workaround for clipping in older IE versions 在旧的IE版本中剪辑的工作方法
	...
	// Catch-all event handling //全面性事件处理
	if (mxClient.IS_IE6) {
	...
	node.style.left = (52 - width - this.thumbBorder) / 2 + 'px'; // 一行放几个
	...
	// Adds title for sidebar entries 为侧栏条目添加标题
...
/**
 * Creates and returns a new palette item for the given image. 为给定的图像创建并返回一个新的调色板项。
 */
Sidebar.prototype.createItem = function (cells, title, showLabel, showTitle, width, height, allowCellsInserted) {
	...
	// Blocks default click action   块默认单击操作
	...
	if (cells.length > 1 || cells[0].vertex) {
		...
		// Uses guides for vertices only if enabled in graph 仅当在图形中启用时，才对顶点使用向导
	...
	// Shows a tooltip with the rendered cell  显示带有呈现单元格的工具提示
	if (!mxClient.IS_IOS) {
	...
	labelTitle.className = 'sidebarLabelTitle'
	elt.appendChild(labelTitle)
...
/**
 * Creates a drop handler for inserting the given cells. 创建用于插入给定单元格的drop处理程序。
 */
	...
	try {
		...
		for (var i = 0; i < targets.length; i++) {
			...
			if ((graph.getModel().isVertex(targetCell) == graph.getModel().isVertex(source)) ||
				...
				// Removes all children of composite cells 移除复合单元格的所有子单元格
				...
				if (style != null) {
					// Replaces the participant style in the lifeline shape with the target shape 用目标形状替换生命线形状中的参与者样式
	...
	return mxUtils.bind(this, function (graph, evt, target, x, y, force) {
		...
		if (elt == null && graph.isEnabled()) {
			...
			if (cells.length > 0) {
				...
				// Holding alt while mouse is released ignores drop target 释放鼠标时按住alt键会忽略掉目标
				...
				if (!graph.isCellLocked(target || graph.getDefaultParent())) {
					...
					try {
						...
						// Splits the target edge or inserts into target group 将目标边缘分割或插入到目标组中
					...
					let state = graph.view.getState(select[0]);

					if (state.style.type === "dongtaiwenben") {
						graph.removeCells(select, true); // 移除
						var select = new mxCell('text', new mxGeometry(0, 0, 100, 30)); // 创建新的
						graph.getModel().setStyle(select,'text;verticalAlign=middle;align=center;lineHeight=30;strokeColor=none;fillColor=none;');
						select.vertex = true;
						var select = graph.importCells([select], x, y, target);
					} 
...
/**
 * Adds a handler for inserting the cell with a single click.--添加用于通过单击插入单元格的处理程序。
 */
...
Sidebar.prototype.addFoldingHandler = function (title, content, funct) {
	...
	title.style.backgroundPosition = '95% 50%';
...
Sidebar.prototype.addImagePalettesimulationDiYa = function (id, title, prefix, postfix, items, titles, tags, expand) {
	...
	for (var i = 0; i < items.length; i++) {
		(mxUtils.bind(this, function (item, title, tmpTags) {
			...
			if (Object.keys(titles).indexOf(items[i]) >= 0) {
				privatetitle = titles[items[i]]
			}
			if (items[i] == 'UPS' || items[i] == 'pdg') {
				fns.push(this.createVertexTemplateEntry('shape=image;needInputCode=true;warning=true;type=' + items[i] + ';labelPosition=left;verticalLabelPosition=middle;whiteSpace=wrap;aspect=fixed;align=right;labelBackgroundColor=#ffffff;image=' + prefix + item + postfix,
					28, 48, privatetitle, privatetitle, title != null, null, this.filterTags(tmpTags)));
			} else if (items[i] == 'gyzl' || items[i] == 'ltg' || items[i] == 'dykg') {
				fns.push(this.createVertexTemplateEntry('shape=image;needInputCode=true;warning=true;type=' + items[i] + ';labelPosition=left;verticalLabelPosition=middle;whiteSpace=wrap;aspect=fixed;align=right;labelBackgroundColor=#ffffff;image=' + prefix + item + postfix,
					21, 48, privatetitle, privatetitle, title != null, null, this.filterTags(tmpTags)));
			} else {
				fns.push(this.createVertexTemplateEntry('shape=image;needInputCode=true;warning=true;type='+ items[i] + ';labelPosition=left;verticalLabelPosition=middle;whiteSpace=wrap;aspect=fixed;align=right;labelBackgroundColor=#ffffff;image=' + prefix + item + postfix,
					this.defaultImageWidth, this.defaultImageHeight, privatetitle, privatetitle, title != null, null, this.filterTags(tmpTags)));
			}
		}))(items[i], (titles != null) ? titles[i] : null, (tags != null) ? tags[items[i]] : null);
	}
	this.addPaletteFunctions(id, title, expand, fns);
};
Sidebar.prototype.addImagePalettesimulationPeiDian = function (id, title, prefix, postfix, items, titles, tags, expand) {
	// console.log(id) // SimulationPeiDian
	// console.log(title) // 电源设备
	// console.log(items) //  ["UPS", "dykg", "gyzl", "xdcz"]
	// console.log(titles) // null
	var showTitles = titles != null;
	var fns = [];
	for (var i = 0; i < items.length; i++) {
		(mxUtils.bind(this, function (item, title, tmpTags) {
...
Sidebar.prototype.addImagePaletteSwitchDevice = function (id, title, prefix, postfix, items, titles, tags, expand) {
	var showTitles = titles != null;
	var fns = [];
	for (var i = 0; i < items.length; i++) {
		(mxUtils.bind(this, function (item, title, tmpTags) {
...
Sidebar.prototype.addImagePaletteFirewallDevice = function (id, title, prefix, postfix, items, titles, tags, expand) {
	var showTitles = titles != null;
	var fns = [];
	for (var i = 0; i < items.length; i++) {
		(mxUtils.bind(this, function (item, title, tmpTags) {
...
Sidebar.prototype.addImagePaletteRouterDevice = function (id, title, prefix, postfix, items, titles, tags, expand) {
	var showTitles = titles != null;
	var fns = [];
	for (var i = 0; i < items.length; i++) {
		(mxUtils.bind(this, function (item, title, tmpTags) {
...
Sidebar.prototype.addImagePaletteLoadBalanceDevice = function (id, title, prefix, postfix, items, titles, tags, expand) {
	var showTitles = titles != null;
	var fns = [];
	for (var i = 0; i < items.length; i++) {
		(mxUtils.bind(this, function (item, title, tmpTags) {
...
Sidebar.prototype.addImagePaletteServiceDevice = function (id, title, prefix, postfix, items, titles, tags, expand) {
	var showTitles = titles != null;
	var fns = [];
	for (var i = 0; i < items.length; i++) {
		(mxUtils.bind(this, function (item, title, tmpTags) {
...
Sidebar.prototype.addImagePaletteInternetDevice = function (id, title, prefix, postfix, items, titles, tags, expand) {
	var showTitles = titles != null;
	var fns = [];
	for (var i = 0; i < items.length; i++) {
		(mxUtils.bind(this, function (item, title, tmpTags) {
...
Sidebar.prototype.addImagePaletteMakeCool = function (id, title, prefix, postfix, items, titles, tags, expand) {
	var showTitles = titles != null;
	var fns = [];
	for (var i = 0; i < items.length; i++) {
		(mxUtils.bind(this, function (item, title, tmpTags) {
			...
			fns.push(this.createVertexTemplateEntry('shape=image;type=' + items[i] + ';labelBackgroundColor=#ffffff;labelPosition=left;verticalLabelPosition=middle;whiteSpace=wrap;aspect=fixed;align=right;image=' + prefix + item + postfix,
				this.defaultImageWidth, this.defaultImageHeight, privatetitle, privatetitle, title != null, null, this.filterTags(tmpTags)));
		}))(items[i], (titles != null) ? titles[i] : null, (tags != null) ? tags[items[i]] : null);
	}
	this.addPaletteFunctions(id, title, expand, fns);
...
Sidebar.prototype.addImagePaletteOthers = function (id, title, prefix, postfix, items, titles, tags, expand) {
	var showTitles = titles != null;
	var fns = [];
	for (var i = 0; i < items.length; i++) {
		(mxUtils.bind(this, function (item, title, tmpTags) {
			...
			fns.push(this.createVertexTemplateEntry('shape=image;type=' + items[i] + ';imageBackground=none;labelPosition=left;verticalLabelPosition=middle;whiteSpace=wrap;aspect=fixed;align=right;image=' + prefix + item + postfix,
				this.defaultImageWidth, this.defaultImageHeight, privatetitle, privatetitle, title != null, null, this.filterTags(tmpTags)));
		}))(items[i], (titles != null) ? titles[i] : null, (tags != null) ? tags[items[i]] : null);
	}
	this.addPaletteFunctions(id, title, expand, fns);
 ...
 */
Sidebar.prototype.addImagePaletteWarning = function (id, title, prefix, postfix, items, titles, tags, expand) {
	var showTitles = titles != null;
	var fns = [];
	...
	for (var i = 0; i < items.length; i++) {
		(mxUtils.bind(this, function (item, title, tmpTags) {
			...
			fns.push(this.createVertexTemplateEntry('shape=image;needInputCode=true;warning=true;type=' + items[i] + ';labelBackgroundColor=#ffffff;labelPosition=left;verticalLabelPosition=middle;whiteSpace=wrap;aspect=fixed;align=right;image=' + prefix + item + postfix,
				this.defaultImageWidth, this.defaultImageHeight, privatetitle, privatetitle, title != null, null, this.filterTags(tmpTags)));
		}))(items[i], (titles != null) ? titles[i] : null, (tags != null) ? tags[items[i]] : null);
	}
	this.addPaletteFunctions(id, title, expand, fns);
 ...
 * Adds the given image palette.
 */
Sidebar.prototype.addImagePalette = function (id, title, prefix, postfix, items, titles, tags, expand) {
	var showTitles = titles != null;
	var fns = [];
	...
	for (var i = 0; i < items.length; i++) {
		(mxUtils.bind(this, function (item, title, tmpTags) {
			...
			fns.push(this.createVertexTemplateEntry('shape=image;type=' + items[i] + ';labelBackgroundColor=#ffffff;labelPosition=left;verticalLabelPosition=middle;align=right;aspect=fixed;whiteSpace=wrap;image=' + prefix + item + postfix,
				this.defaultImageWidth, this.defaultImageHeight, privatetitle, privatetitle, title != null, null, this.filterTags(tmpTags)));
		}))(items[i], (titles != null) ? titles[i] : null, (tags != null) ? tags[items[i]] : null);
	}
...
Sidebar.prototype.addImagePaletteBase = function (id, title, prefix, postfix, items, titles, tags, expand) {
	var showTitles = titles != null;
	var lineTags = 'line lines connector connectors connection connections arrow arrows ';
	var fns = [];
	for (var i = 0; i < items.length; i++) {
		(mxUtils.bind(this, function (item, title, tmpTags) {
			...
			if (i == 0) {
				fns.push(this.createVertexTemplateEntry('line;type=shuxian;strokeWidth=1;strokeColor=#00ccff;direction=south;html=1;', 10, 160, '', '竖线'))
				fns.push(this.createVertexTemplateEntry('line;type=hengxian;strokeWidth=1;strokeColor=#00ccff;html=1;', 160, 10, '', '横线'), )
				fns.push(this.createEdgeTemplateEntry('endArrow=none;type=edge;html=1;class=aaa;', 50, 50, '', '直线', null, ''))
				fns.push(this.createEdgeTemplateEntry('endArrow=classic;type=edge;startArrow=classic;html=1;', 50, 50, '', '箭头线', ''))
				fns.push(this.createVertexTemplateEntry('whiteSpace=wrap;type=rectangle;html=1;strokeColor=#00ccff;', 120, 24, '', '长方形', null, null, '长方形')) // 长方形
				fns.push(this.createVertexTemplateEntry('ellipse;type=ellipse;whiteSpace=wrap;html=1;strokeColor=#00ccff;', 80, 30, '', '椭圆形', null, null, '椭圆形')) // 椭圆形
				fns.push(this.createVertexTemplateEntry('shape=text;type=table;html=1;strokeColor=none;fillColor=none;fontColor=#00ccff;overflow=fill;', 180, 140,
					'<table border="1" width="100%" height="100%" style="width:100%;height:100%;border-collapse:collapse;border:1px solid #00ccff;">' +
					'<tr style="border:1px solid #00ccff;"><td align="center">Value 1</td><td align="center">Value 2</td><td align="center">Value 3</td></tr>' +
					'<tr style="border:1px solid #00ccff;"><td align="center">Value 4</td><td align="center">Value 5</td><td align="center">Value 6</td></tr>' +
					'<tr style="border:1px solid #00ccff;"><td align="center">Value 7</td><td align="center">Value 8</td><td align="center">Value 9</td></tr></table>', '表格')) // 表格
				fns.push(this.createVertexTemplateEntry('text;type=textbox;html=1;strokeColor=#00ccff;fontColor=#00ccff;fillColor=none;spacing=5;spacingTop=-20;whiteSpace=wrap;overflow=hidden;rounded=0;', 190, 120,
					'<h1>Heading</h1><p>Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p>',
					'文本框', null, null, '文本框')) // 文本框
			}
	...
	this.addPaletteFunctions(id, title, expand, fns);
	// this.addPaletteFunctions('general', mxResources.get('general'), (expand != null) ? expand : true, lineFns);
};
/**
