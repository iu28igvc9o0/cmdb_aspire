...
export default class AccountServiceFactory {
    ...
    // 批量新增授权
    static batchAssign(data){
      return rbHttp.sendRequest({
        method: 'POST',
        data:data,
        url: '/v1/orgs/user/roles/assign'
    })
    }
    // 批量移除授权
    static batchRevoke(data){
      return rbHttp.sendRequest({
        method: 'POST',
        data:data,
        url: '/v1//orgs/user/roles/revoke'
    })
    }
    // 组织树图查询接口
    static deptUserTree(deptId){
      return rbHttp.sendRequest({
        method: 'GET',
        url: '/v1/department/deptUserTree?deptId='+deptId
    })
    }
}
