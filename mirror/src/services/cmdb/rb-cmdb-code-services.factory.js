...
export default class cmdbCodeService {
    // 查询码表对象
    static async getCodeObj(params) {
        return rbHttp.sendRequest({
            method: 'GET',
            params: params,
            url: '/v1/cmdb/code/get'
        }).then(function (data) {
            return data
        })
    }
    // 查询码表
    static async getModuleCodes(moduleId) {
        return rbHttp.sendRequest({
            method: 'GET',
            url: `/v1/cmdb/code/list/${moduleId}`
        }).then(function (data) {
            return data
        })
    }
    ...
    // 查询所有码表信息, 并按分组显示
    static async queryCodeListFormatByGroup(params) {
        return rbHttp.sendRequest({
            method: 'Get',
            params: params,
    ...
    // 查询模型下的码表
    static async getCodeByModuleId(moduleId) {
        return rbHttp.sendRequest({
            method: 'GET',
            url: cmdbCodePoint + '/list/' + moduleId
        }).then(function (data) {
            return data
        })
    }
