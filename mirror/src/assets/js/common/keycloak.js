...
var Keycloak = function (config) {
    ...
    kc.getToken = function (username, password) {
        var url;
        if (config.url.charAt(config.url.length - 1) == '/') {
            url = config.url + 'realms/' + encodeURIComponent(config.realm);
        } else {
            url = config.url + '/realms/' + encodeURIComponent(config.realm);
        }

        url = url + '/protocol/openid-connect/token';
        var params = `client_id=${encodeURIComponent(config.clientId)}&username=${username}&password=${password}&grant_type=password`;
        var req = new XMLHttpRequest();
        req.open('POST', url, true);
        req.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
        req.withCredentials = true;
    ...
    kc.initConfig = function () {
        adapter = loadAdapter();
        loadConfig(config);
    }

    kc.setTokens = function (token, refreshToken, idToken) {
        kc.onReady && kc.onReady(kc.authenticated);
        var timeLocal = new Date().getTime() + 200;
    ...
    function setupCheckLoginIframe() {
        ...
        var messageCallback = function (event) {
            ...
            // 此处为keycloak和浏览器建立的websocket通讯，用于监听token过期后通知浏览器重新登录
            // chrome版本对event解析的差异，导致event.data结果为changed，触发了该操作，本地服务暂时屏蔽，避免死循环
            if (event.data != 'unchanged' && !location.origin.includes('localhost')) {
                kc.clearToken();
            }
