    ...
    <div class="component-container">
        <div class="rb-event-list">
            <div class="rb-event-header">
                ...
                <div class="rb-form">
                    ...
                    <div style="display: flex">
                        <el-input size="small"
                                  v-model="queryString"
                                  @keyup.enter.native="queryEvents">
                            <el-button slot="append"
                                       icon="el-icon-search"
                                       @click="queryEvents"></el-button>
                        </el-input>
                    </div>
                </div>
            </div>
            ...
            <div class="rb-event-content">
                <ul v-if="events.length > 0"
                    class="rb-event-list">
                    <li :class="{'rb-popover-success': item.log_level === 0, 'rb-popover-warnning': item.log_level === 1, 'rb-popover-error': item.log_level === 2}"
                        v-for="(item,index) in events"
                        :key="index">
                        <div @click="itemClickEvent(index)"
                            ...
                            <template v-if="['generic','node'].includes(item.template_id)">
                                ...
                                <a v-if="item.title.routers[0]"
                                   @click.self.prevent="redirect(item.title.routers[0])">{{item.title.routers[0].name}}</a>
                            </template>
                            <template v-if="['sub_resource','build_result','build_v2'].includes(item.template_id)">
                                ...
                                <a v-if="item.title.routers[1]"
                                   @click.self.prevent="redirect(item.title.routers[1])">{{item.title.routers[1].name}}</a>
                            </template>
                            ...
                            <el-popover v-if="item.detail.message"
                                        placement="top-start"
                                        trigger="hover">
                                <span class="fa fa-commenting item-message-icon"
                                      slot="reference">
                                </span>
                ...
                <div v-if="events.length === 0 && !noMassage"
                     class="rb-event-not-found">
                    <div>
                        <h2>找不到事件</h2>
                        <hr>
                        <ul>
      ...
      "resource_type": "service"
                ...
                <div style="display: flex;flex-direction: row-reverse">
                    <el-pagination v-if="pagination.total_items"
