        ...
        <div class="body-container">
            ...
            <div v-show="active == 0 ? true:false"
                ...
                <el-form :model="newTemplate"
                    ...
                    <el-form-item prop="mon_type"
                                  label="监控类型："
                                  class="add-tit"
                                  label-width="93px">
                        <el-select v-model="newTemplate.mon_type"
                                   placeholder="请选择"
                                   class="list-sel add-con"
                                   disabled>
                            <el-option key="1"
            ...
            <div class="add-table"
                ...
                <div style=""
                    ...
                    <el-button class="item-related"
                               type="primary"
                               @click="relateItems"
                               style="width:100px">监控项关联</el-button>
                    <!-- <el-button class="item-add" type="primary" @click="addItems">新增</el-button> -->
                    <el-button class="item-del"
                               type="primary"
                               @click="delteItems">删除</el-button>
                </div>
                <div v-if="newTemplate.mon_type ==='1' || newTemplate.mon_type ==='3'">
                    <el-table :data="tableData"
                        ...
                        <el-table-column prop="expression"
                            ...
                            <template slot-scope="scope">
                                <el-select v-model="tableData[scope.$index].expression"
                                           placeholder="请选择"
                                           class="add-exp">
                                    <el-option v-for="item in selExpression(tableData[scope.$index].value_type)"
                                               :key="item.value"
                                               :label="item.label"
                                               :value="item.value">
                                    </el-option>
                        ...
                        <el-table-column prop="match"
                                         ...
                                         align="center">
                            <template slot-scope="scope">
                                <el-input class="add-exp dom"
                                          v-model="tableData[scope.$index].match"
                                          @blur="check(scope.$index, tableData[scope.$index])"></el-input>
                            </template>
                        </el-table-column>
                    </el-table>
                </div>
            ...
            <div class="add-device"
                ...
                <div v-if="newTemplate.mon_type === '1'">
                    <div style=""
                        ...
                        <el-button class="device-add"
                                   type="primary"
                                   @click="addevices">新增</el-button>
                        <el-button class="device-del"
                                   type="primary"
                                   @click="delteDevices">删除</el-button>
                    </div>
                    <div class="add-table-tit"
                         style="margin-top:15px">已选设备：</div>
                    <el-table :data="hosts"
                              border
                ...
                <div v-if="newTemplate.mon_type === '3'"
                     style="text-align:  center;">
                    <div :is="agentDeviceChoose"
                         :deviceList="selectedAgentList"
                         @changeDeviceList="changeDeviceList"></div>
                    <!-- 确认结果 -->
                    <footer>
                        <el-button class="prevStep"
                                   type="primary"
                ...
                <div v-if="newTemplate.mon_type === '2'"
                     class="biz-tem">
                    <div style=""
                         class="device">
                ...
                <el-form :model="form"
                    ...
                    <el-form-item label="数据类型："
                        ...
                        <el-select v-model="form.value_type"
                                   ...
                                   class="input1"
                                   key="value_type">
                            <el-option v-for="item in data_type_option"
                                       :key="item.value"
                                       :label="item.label"
                                       :value="item.value">
                            </el-option>
                    ...
                    <el-form-item label="判断符号："
                                  :label-width="formLabelWidth"
                                  prop="expression">
                        <el-select v-model="form.expression"
                                   placeholder=""
                                   class="input2"
                                   key="expression">
...
<script>
    ...
    export default {
        name: 'MirrorTemplateUpdate',
        components: {
            agentDeviceChoose: () => import('../add/components/agent-device-choose.vue')
        },
        data() {
            ...
            return {
                ...
                // 已有的触发器列表
                triggerData: [],
                agentDeviceChoose: 'agentDeviceChoose',
                selectedAgentList: [],
                // 选择的触发器列表
                triggerItems: [],
                trigger_options: {},
        ...
        methods: {
            ...
            next() {
                ...
                if (this.active === 0) {
                    ...
                    } else if (this.newTemplate.mon_type === '3') {
                        this.newTemplate.trigger_list = this.packSend(this.tableData)
                        this.newTemplate.object_ids = this.selectedAgentList.join(',')
                        this.newTemplate.fun_type = '2'
                    } else {
                        if (!this.ruleTrigger()) {
                            return
            ...
            relateItems() {
                ...
                } else if (this.newTemplate.mon_type === '2') {
                    this.$router.push('add/relateditemsBiz')
                } else if (this.newTemplate.mon_type === '3') {
                    this.$router.push('add/relateditemsScript')
                }
            },
            // 模板新增设备
            addevices() {
            ...
            detail(str) {
                rbProjectDataServiceFactory.detail(str).then((res) => {
                    ...
                    this.name1 = res.name
                    if (this.newTemplate.mon_type === '1' || this.newTemplate.mon_type === '3') {
                        ...
                        if (this.newTemplate.mon_type === '1') {
                            ...
                            this.selectedAgentList = this.packAgentHosts(res.object_list)
                        }
