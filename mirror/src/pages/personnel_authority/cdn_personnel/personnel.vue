...
<script>
    ...
    // import personDialog from 'src/pages/personnel_authority/cdn_personnel-add/personnel-add.vue'
    import batchmodDeptDialog from 'src/pages/personnel_authority/cdn_personnel-edit/personnel-edit.vue'
    ...
    export default {
        ...
        methods: {
            handleMoveDepartment() {
                ...
                personDataService.updateDepartment({ id: data.uuid, name: data.name, 'parent_id': parentId }).then(() => {
                    this.$message({
                        message: '修改成功',
                        type: 'success'
                    })
            ...
            handleDrop(draggingNode, dropNode) {
                personDataService.updateDepartment({ id: draggingNode.data.uuid, name: draggingNode.data.name, 'parent_id': dropNode.data.uuid }).then(() => {
                    this.$message({
                        message: '修改成功',
                        type: 'success'
                    })
            ...
            customDelete(node, data) {
                this.$confirm('确定要删除吗?', '提示', {
                    ...
                    personDataService.deleteDepartment(data.uuid).then(() => {
                        this.$message({
                            message: '操作成功',
                            type: 'success'
                        })
            ...
            handleUpdateDepartment() {
                this.$refs['departmentupdateform'].validate((valid) => {
                    if (valid) {
                        personDataService.updateDepartment(this.departmentUpdateForm).then(() => {
                            this.$message({
                                message: '修改成功',
                                type: 'success'
                            })
                ...
                this.$refs['departmentform'].validate((valid) => {
                    if (valid) {
                        if (this.departmentForm.type === 1 && !this.clickParentNode) {
                            ...
                            personDataService.addDepartment(this.departmentForm).then(() => {
                                this.$message({
                                    message: '创建成功',
                                    type: 'success'
                                })
