...
<script>
    ...
    export default {
        components: {
            YwAlertStatus: () => import('./sub_components/yw-alert-status.vue'),
        },
        data() {
            return {
                chartData: {
                    ...
                    chartList: [
                        {
                            name: '待确认', status: 0, icon: require('src/assets/theme/dark/img/alert-wait-confirm.png'), num: 0,
                            numList: [
                                { status: 'red', num: 0 },
                                { status: 'orange', num: 0 },
                                { status: 'yellow', num: 0 },
        ...
        methods: {
            ...
            getDatas() {
                ...
                this.$api.queryTodayAlertOverview(params).then((res) => {
                    ...
                    this.chartData.chartList[2].num = this.validData(res.cleared.summary) || 0
                    this.chartData.chartList[2].numList[0].num = this.validData(res.cleared.serious) || 0
                    this.chartData.chartList[2].numList[1].num = this.validData(res.cleared.high) || 0
                    this.chartData.chartList[2].numList[2].num = this.validData(res.cleared.medium) || 0
                    this.chartData.chartList[2].numList[3].num = this.validData(res.cleared.low) || 0
