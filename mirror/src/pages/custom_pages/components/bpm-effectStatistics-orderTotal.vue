    ...
    <div class="content-chart">
        ...
        <section class="chart-section">
            ...
            <div class="chart-section-main"
                 v-loading="loading">
                <YwBar :barDatas="barDatas[0]"
                       v-if="barDatas[0].show"></YwBar>
            </div>
        </section>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        components: {
            ...
            YwBar: () => import('src/components/common/charts-lightColor/yw-bar.vue'),
        },
        data() {
            return {
                activeSelect: 1,
                ...
                barDatas: [
                    {
                        show: false,
                        id: 'bar-1',
                        chartOption: 'bar-option',
                        chartDatas: [],
                        option: {
                            legend: { show: false },
                            series: [
                                { seriesName: '工单数', yAxisName: '工单数(个)', yData: 'cnt', xData: 'taskName' }
                            ]
                        }
                    },
                ],
        ...
        methods: {
            ...
            query() {
                ...
                let params = {
                    ...
                    workType: this.moduleData.datas.workType,// 1. 工单总数 2处理中工单 3超时工单 4关闭工单
                }
                rbBpmHomeServices.getInstEfficiencyReport(params).then(res => {
                    this.tabList.forEach((item) => {
                        item.time = `${res.startTime} 至${res.endTime}`
                    })
                    this.barDatas[0].chartDatas = res.data
