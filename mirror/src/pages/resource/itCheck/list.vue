<template>
    <el-container class="components-container"
                  ...
                  :element-loading-text="loadingInfo.loading_text">
                ...
                <ul class="select-section">
                    <template v-for="(item,index) in provinceData">
                        <li class="select-item"
                            :class="{active: item.orgName === provinceObj.name}"
                            v-if="!filterText || item.orgName.indexOf(filterText.trim()) > -1 "
                            @click="queryByProvince(item)"
                            :key="index">{{item.orgName}}
                            <span class="wait"
                                  v-if="provinceStatus[item.orgName] === -1">待审批</span>
                            <span class="approved"
                                  v-if="provinceStatus[item.orgName] === 1">已审批</span>
                        </li>
                    </template>
                </ul>
            </el-scrollbar>
        ...
        <el-main class="yw-dashboard">
            <el-form class="components-condition yw-form"
                ...
                <el-form-item>
                    <el-select v-model="quarter.quarter"
                               @change="query()">
                ...
                <el-form-item label="生成评分类型"
                              label-width="85px">
                    <el-select v-model="evaluateDeviceType"
                               filterable>
                        <el-option v-for="(item, index) in deviceTypeData"
                                   :key="index"
                                   :label="item.name"
                ...
                <el-form-item>
                    <el-button type="primary"
                               @click="makeEvaluate()">生成评分</el-button>
                </el-form-item>
            </el-form>
            ...
            <!-- tab -->
            <template v-if="provinceObj.name && resetComponent">
                <component :ref="currentTabComponent"
...
<script>
    ...
    export default {
        name: 'ResourceCheck',
        mixins: [QueryObject, department, CommonOption],
        components: {
        ...
        methods: {
            ...
            // 查询省市
            async queryProvince() {
                this.provinceData = await this.getDepartmentList()
            ...
            resolveProvince() {
                ...
                copyData.forEach((item) => {
                    if (!that.provinceStatus[item.orgName]) {
                        otherData.push(item)
                        return true
                    }
                    for (let i in this.provinceStatus) {
                        // 拿出待审批数据
                        if (i === item.orgName && that.provinceStatus[i] === -1) {
                            waitData.push(item)
                        } else if (i === item.orgName && that.provinceStatus[i] === 1) {
                            appData.push(item)
                        }
                    }
                })
                that.provinceData = (waitData.concat(appData)).concat(otherData)
                that.provinceObj = JSON.parse(sessionStorage.getItem('loginUserInfo'))['dept_list'][0]
            },
            // 通过省市查询数据
            queryByProvince(val) {
                this.provinceObj = Object.assign({ name: val.orgName }, val)
