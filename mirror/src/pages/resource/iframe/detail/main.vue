...
<script>
    ...
    export default {
        ...
        components: {
            ...
            automaticConfiguration: () => import('src/pages/resource/iframe/detail/automaticConfiguration.vue'),
            deviceData: () => import('src/pages/resource/iframe/detail/deviceDataMain.vue'),
            // deviceData: () => import('src/pages/mirror_alert/device_monitor/device_network/device_network.vue')
            hostProfile: () => import('src/pages/resource/iframe/detail/hostProfile.vue')
        },
        data() {
            return {
                // 触发刷新
                ...
                deviceInfo: {
                    ...
                    fieldValue: JSON.parse(this.$route.query.queryParams).fieldValue,
                    id: JSON.parse(this.$route.query.queryParams).id,
                }
            }
        },
        mounted() {
            this.getTabsInformation()
            this.getDeviceTypeWithDisplay()
            console.log('this.$route.query===', this.$route.query.queryParams)
        },
        methods: {
            // 请求tabs栏信息
            getTabsInformation() {
                ...
                let obj = {
                    modelId: JSON.parse(this.$route.query.queryParams).moduleId
                }
                rbTabsServiceFactory.getModelTabsList(obj).then((res) => {
                    if (res.data.length > 0) {
                        this.activeName = res.data[0].subassemblyName
                        this.currentTabComponent = res.data[0].subassemblyName
