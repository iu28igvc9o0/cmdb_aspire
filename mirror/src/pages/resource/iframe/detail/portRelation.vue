            ...
            <el-form class="yw-form"
                ...
                <div class="yw-el-table-wrap"
                    ...
                    <el-table :data="tableData"
                        ...
                        <el-table-column label="对端资源池"
                                         prop="zIdcType">
                        </el-table-column>
                        <el-table-column label="对端设备IP">
                            <template slot-scope="scope">
                                <el-button type="text"
                                           @click="toDetail(scope.row)">{{scope.row.zIp}}</el-button>
                            </template>
                        </el-table-column>
                        <el-table-column label="对端设备名称">
                            <template slot-scope="scope">
                                <el-button type="text"
                                           @click="toDetail(scope.row)">{{scope.row.zDeviceName}}</el-button>
                            </template>
                        </el-table-column>
                        <el-table-column label="对端设备端口"
                                         prop="zPortId">
                        </el-table-column>
                        <el-table-column label="连接属性"
                                         prop="connectType">
                        </el-table-column>
                        ...
                        <el-table-column label="操作"
                                         width="60px">
                            <template slot-scope="scope">
                                <i style="font-size: 16px;color:#409EFF;cursor: pointer"
    ...
    export default {
        ...
        data() {
            return {
                ...
                total: 0, // 总共多少行数据
                instanceId: this.deviceId ? this.deviceId : JSON.parse(this.$route.query.queryParams).instanceId,
                loading: false,
                importData: {
                    isImport: false,
        ...
        methods: {
            toDetail(row) {
                let queryParams = {
                    instanceId: row.zInstanceId,
                    idcType: row.zIdcType,
                    ip: row.zIp,
                    state: 'detail'
                }
                queryParams = JSON.stringify(queryParams)
                this.$router.push({ path: 'detail', query: { queryParams: queryParams } })
            },
            deleteById(id) {
                let data = {
                    id: id
