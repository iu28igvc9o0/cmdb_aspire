...
<script>
    ...
    export default {
        name: 'ResourceIframeNetworkSegment',
        components: {
            instanceList,
            otherinstanceList,
            Tab: () => import('./tab/view'),
        ...
        data() {
            return {
                ...
                listShow: true,
                moduleType: 'host',
                condicationCode: 'ip_repository_inner',// 'instance_list','ip_repository_inner'
                condicationCode1: 'search_ip_repository_inner_ip',
                loading: false,
                treeLoading: false,
                loading_text: '正在加载数据，请稍等...',
                businessUuid: '',
                moduleId: '7e6a5f63a0204b92bd783508eed0818d',
                moduleId1: '2893541da55d415c8f99220137cdc599',
                filterText: '',
                department1: '',
                department2: '',
                bizSystem: '',
                ...
                viewCodeName: 'inner_segment_view',
                isRefresh: true,
                routerName: 'ResourceIframeNetworkSegment'
            }
        },
        mounted: function () {
            this.initTree()
        ...
        methods: {
            ...
            queryViewsByTree(val) {
                ...
                console.log(this.bizSystemCondition, 'bizSystemCondition')
            },
            tabCheck(data) {
                console.log(data)
                this.segmentAddr = ''
                if (data) {
                    this.condicationCode = 'ip_repository_inner'
                    this.moduleId = '7e6a5f63a0204b92bd783508eed0818d'
                    this.ipSingal = ''
                } else {
                    this.condicationCode1 = 'search_ip_repository_inner_ip'
                    this.moduleId1 = '2893541da55d415c8f99220137cdc599'
                    this.ipSingal = 'all'
                }
                this.listShow = data
                if (!data) {
            ...
            tabCheck1(data) {
                ...
                if (data.check) {
                    ...
                    if (data.query.network_segment_address) {
                        this.bizSystemCondition = {
                            all: {
                                value: data.query.network_segment_address,
                                key: data.query.network_segment_address,
                                id: data.query.id,
                                type: {
                                    idcType: data.query.idcType_dict_note_name,
                                    inner_segment_sub_type: data.query.inner_segment_sub_type_dict_note_name,
                                    inner_segment_type: data.query.inner_segment_type_dict_note_name,
                                    network_segment_address: data.query.network_segment_address,

                                },
                                tree: {
                                    queryToData: {
                                        idcType: data.query.idcType,
                                        inner_segment_sub_type: data.query.inner_segment_sub_type,
                                        inner_segment_type: data.query.inner_segment_type,
                                        network_segment_address: data.query.network_segment_address
                                    }
                                }
                            }
