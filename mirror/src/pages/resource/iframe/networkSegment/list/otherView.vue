    ...
    <div class="resourceManagement"
        ...
        <div class="managementRight">
                ...
                <div class="overview">
                                ...
                                <div class="content"
                                    ...
                                    <div style="flex:1;">
                                        <p :class='item.value == queryForm.idcType?"poolColor":""'
                                           @click="jilianclick(item.value,item.id,'idcType',idcTypeArr,onehead,item)"
                                ...
                                <div class="content"
                                    ...
                                    <div style="flex:1;">
                                        <p :class='item.value==queryForm.deviceProject?"poolColor":""'
                                           @click="jilianclick(item.value,item.id,'deviceProject',projectnameArr,twohead,item)"
                            ...
                            <div class="content"
                                ...
                                <div style="flex:1;">
                                    <p :class='item.value==queryForm.pod?"poolColor":""'
                                       @click="jilianclick(item.value,item.id,'pod',podnameArr,threehead,item)"
                            ...
                            <div class="content"
                                ...
                                <div style="flex:1;">
                                    <p :class='item.value==queryForm.deviceClass?"poolColor":""'
                                       @click="jilianclick(item.value,item.id,'deviceClass',deviceTypeArr,fourhead,item)"
                    ...
                    <div class="content" style="display:flex;min-height:60px;">
                        ...
                        <template v-for="(item1,index1) in otherList">
                            <div class="aLine"
                                 v-if="item1.one.cmdbCode.controlType.controlCode == 'cascader' || item1.one.cmdbCode.controlType.controlCode == 'listSel'"
                ...
                <div class="selected"
                    ...
                    <div id="querySelector"
                        ...
                        <div class="public"
                            ...
                            <span class="publicContent">{{item.son.value}}</span>
            ...
            <el-form class="yw-form form"
                ...
                <div class="table-operate-wrap clearfix">
                    ...
                    <div style="display:inline-block;vertical-align:middle;"
                         v-if="resetComponent && !this.queryForm.moduleId">
                    ...
                    <el-button @click="exportData()">导出1111</el-button>
                </div>
                <div class="yw-el-table-wrap">
                    <el-table class="yw-el-table"
                        ...
                        <el-table-column prop="device_name"
                                         ...
                                         show-overflow-tooltip>
                            <template slot-scope="scope">
                                <a class="table-link">{{scope.row.device_name}}</a>
                            </template>
                        </el-table-column>
            ...
            <importInstances ref="importInstances"
                             ...
                             :moduleType="moduleType"></importInstances>
        </div>
...
<script>
    ...
    export default {
        ...
        props: ['queryType', 'parentParams', 'moduleType', 'condicationCode'],
        components: {
        ...
        data() {
            return {
                ...
                condicationCode: this.condicationCode || 'instance_list',
                fixedColumns: {},
                boraderActive: false,
                aLinePublic1: true,
                aLinePublic2: true,
        ...
        watch: {
            ...
            parentParams(val) {
                ...
                function treelist(value) {
                    itt = itt - 500
                    if (value.data.type) {
                        for (let j = 0; j < _this.reducelist.length; j++) {
                            if (_this.reducelist[j].one.cmdbCode.filedCode == value.data.type) {
                                let json = {
                                ...
                                setTimeout(() => {
                                    _this.commonsql(value.data.name, value.data.name, _this.reducelist[j], j, '2', json)
                                }, itt)
                            }
                        }
                        treelist(value.parent)
                    }
                    if (value.data.type) {
                        for (let j = 0; j < _this.otherList.length; j++) {
                            if (_this.otherList[j].one.cmdbCode.filedCode == value.data.type) {
                                let json = {
                                    value: value.data.name,
                                    key: value.data.name,
                                    id: value.data.uuid,
                                ...
                                setTimeout(() => {
                                    _this.commonsql(value.data.name, value.data.name, _this.otherList[j], j, '1', json)
                                }, itt)
                            }
                ...
                if (val.all) {
                    treelist(val.all.tree.parent)
                    // if (val.all.tree.parent.data.type) {
                    //   for (let j = 0; j < reducelist.length; j++) {
                    //     if (reducelist[j].one.cmdbCode.filedCode == val.all.tree.parent.data.type) {
                    //       let json = {
                    //         value: val.all.tree.parent.data.name,
                    //         key: val.all.tree.parent.data.name,
                    //         id: val.all.tree.parent.data.uuid,
                    //         type: val.all.tree.parent.data.type
                    //       }
                    //       this.commonsql(val.all.tree.parent.data.name, val.all.tree.parent.data.name, reducelist[j], j, '2', json)
                    //     }
                    //   }
                    // }
                    // if (val.all.tree.parent.data.type) {
                    //   for (let j = 0; j < otherList.length; j++) {
                    //     if (otherList[j].one.cmdbCode.filedCode == val.all.tree.parent.data.type) {
                    //       let json = {
                    //         value: val.all.tree.parent.data.name,
                    //         key: val.all.tree.parent.data.name,
                    //         id: val.all.tree.parent.data.uuid,
                    ...
                    //       this.commonsql(val.all.tree.parent.data.name, val.all.tree.parent.data.name, otherList[j], j, '1', json)
                    //     }
                    //   }
                    // }
                    console.log('进入111')
                    setTimeout(() => {
                        for (let i = 0; i < reducelist.length; i++) {
                            if (reducelist[i].one.cmdbCode.filedCode == val.all.type) {
                                console.log(12)
                                this.commonsql(val.all.value, val.all.value, reducelist[i], i, '2', val.all)
                            }
                        }
                        for (let i = 0; i < otherList.length; i++) {
                            if (otherList[i].one.cmdbCode.filedCode == val.all.type) {
                                // alert(2)
                                this.commonsql(val.all.value, val.all.value, otherList[i], i, '1', val.all)
                            }
                        }
                    }, 2500)
                    this.initParams()
        ...
        methods: {
            ...
            query(activePagination = true) {
                ...
                for (let i = 0; i < allcheckList.length; i++) {
                    resdata[allcheckList[i].father.cmdbCode.filedCode] = allcheckList[i].son.value
                }
                if (this.publicBrand) {
            ...
            deletechange(index) {
                ...
                } else if (this.allcheckList[index].father.cmdbCode.filedCode == 'device_type') {
            ...
            commonsql(value, id, item, index, type, info) {
                console.log(type)
                console.log(value)
                console.log(info)
                console.log(item)
                ...
                for (let i = 0; i < arr.length; i++) {
                    ...
                    // device_type
                    if (sigalArr.subFiledCode == 'device_type') {
                        if (value == '全部') {
                            this.deviceClassList = this.fillSelectOption(this.deviceClassListStorage, 'device_type')
                        } else {
                            this.rbHttp.sendRequest({
                                method: 'POST',
                                data: { 'sql': sqlstr },
                                ...
                                this.deviceClassList = this.fillSelectOption(res, 'device_type')
                                console.log(res)
            ...
            jilianclick(value, id, type, newarr, head, info) {
                ...
                for (let i = 0; i < arr.length; i++) {
                    ...
                    } else if (arr[i].subFiledCode == 'device_type') {
                        if (value == '全部') {
                            this.deviceClassList = this.fillSelectOption(this.deviceClassListStorage, 'device_type')
                        } else {
                            this.rbHttp.sendRequest({
            ...
            getQueryParams(value) {
                if (!value) {
                    let resdata = {
                        ...
                        'module_id': this.queryForm.moduleId,
            ...
            inputFun(value, id, item) {
                ...
                // for(let i = 0;i<this.otherList.length;i++){
                //     if(this.otherList[i].one.cmdbCode.controlType.controlCode != 'cascader' && this.otherList[i].one.cmdbCode.controlType.controlCode != 'listSel'){
                //         if(this.otherList[i].value != '' && this.otherList[i].value != null){
                //             this.selectedShow = true
                //             this.allcheckList.push({
                //                     father:this.otherList[i].one,
                //                     son:{value:this.otherList[i].value}
                //                 })
                //         }
                //     }
                ...
                // }
                // for(let i = 0;i<this.reducelist.length;i++){
                //     if(this.reducelist[i].one.cmdbCode.controlType.controlCode != 'cascader' && this.reducelist[i].one.cmdbCode.controlType.controlCode != 'listSel'){
                //         if(this.reducelist[i].value != '' && this.reducelist[i].value != null){
                //             this.selectedShow = true
                //             this.allcheckList.push({
                //                     father:this.reducelist[i].one,
                //                     son:{value:this.reducelist[i].value}
                //                 })
                //         }
                //     }
                // }
                // for(let i = 0;i<this.allcheckList.length;i++){
                //     if(this.allcheckList[i].father.cmdbCode.controlType.controlCode != 'cascader' && this.allcheckList[i].father.cmdbCode.controlType.controlCode != 'listSel'){
                //         this.allcheckList.splice(i,1)
                //     }
                // }
                // for(let i = 0;i<this.otherList.length;i++){
                ...
                conditionManagement.getCondicationGet(queryheadNew.condication).then(res => {
                    ...
                    for (let i = 0; i < headlist.length; i++) {
                        if (headlist[i].cmdbCode.filedCode == 'idcType') {
                            ...
                            indexNew++
                        } else if (headlist[i].cmdbCode.filedCode == 'device_type') {
                            indexNew++
                        ...
                        if (headlist[i].cmdbCode.filedCode == 'idcType') {
                            ...
                            if (headlist[i].cmdbCode.isBindSource == '是') {
                                if (headlist[i].cmdbCode.codeBindSource.bindSourceType == '数据字典') {
                                    ...
                                    this.onestepFun(headlist[i].cmdbCode.filedCode, headlist[i])
                                } else {
                                    console.log('数据表')
                            ...
                            if (headlist[i].cmdbCode.isBindSource == '是') {
                                if (headlist[i].cmdbCode.codeBindSource.bindSourceType == '数据字典') {
                                    ...
                                    // this.queryForm.deviceProject = '全部'
                                    this.onestepFun1(headlist[i].cmdbCode.filedCode, headlist[i])
                                } else {
                                    if (headlist[i].defaultValue != '') {
                                        this.queryForm.deviceProject = headlist[i].defaultValue
                                    } else {
                            ...
                            if (headlist[i].cmdbCode.isBindSource == '是') {
                                if (headlist[i].cmdbCode.codeBindSource.bindSourceType == '数据字典') {
                                    ...
                                    // this.queryForm.pod = '全部'
                                    this.twoStepFun(headlist[i].cmdbCode.filedCode, headlist[i])
                                } else {
                                    if (headlist[i].defaultValue != '') {
                                        this.queryForm.pod = headlist[i].defaultValue
                                    } else {
                            ...
                            if (headlist[i].cmdbCode.isBindSource == '是') {
                                if (headlist[i].cmdbCode.codeBindSource.bindSourceType == '数据字典') {
                                    ...
                                    // this.queryForm.deviceClass = '全部'
                                    this.threeStepFun(headlist[i].cmdbCode.filedCode, headlist[i])
                                } else {
                                    if (headlist[i].defaultValue != '') {
                                        this.queryForm.deviceClass = headlist[i].defaultValue
                                    } else {
                                ...
                                // this.queryForm.deviceClass = '全部'
                                this.deviceClassList = this.fillSelectOption([], 'device_type')
                            }
                            ...
                            if (headlist[i].cmdbCode.isBindSource == '是') {
                                if (headlist[i].cmdbCode.codeBindSource.bindSourceType == '数据字典') {
                                    ...
                                    this.fourStepFun(headlist[i].cmdbCode.filedCode, headlist[i])
                                } else {
                                    if (headlist[i].defaultValue != '') {
                                        this.queryForm.mfrFactory = headlist[i].defaultValue
                                    } else {
                            ...
                            if ((indexNew + indexNumber) > 4) {
                                ...
                                // })
                                if (headlist[i].cmdbCode.isBindSource == '是') {
                                    if (headlist[i].cmdbCode.codeBindSource == '数据字典') {
                                        setf.rbHttp.sendRequest({
                                            method: 'GET',
                                            params: { 'type': resdata },
                                            url: '/v1/cmdb/configDict/getDictsByType'
            ...
            toDetail(row) {
                ...
                // this.$router.push({ path: '/resource/iframe/networkSegment/detail', query: { queryParams: queryParams } })
            },
            // 批量修正
            toBatchUpdate() {
            ...
            toUpdate() {
                ...
                queryParams = JSON.stringify(queryParams)
                this.$router.push({ path: '/resource/iframe/networkSegment/add', query: { queryParams: queryParams } })
            },
            onestepFun(type, value) {
                // 获取资源池
                this.rbHttp.sendRequest({
                    method: 'GET',
                    params: { 'type': type },
