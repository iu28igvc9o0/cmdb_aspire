                    ...
                    <div class="wp100 displaybox condition-row boxalignstart"
                        ...
                        <div class="boxflex01">
                            <div class="displaybox boxalignstretch">
                                ...
                                <div v-else>
                                    <el-input v-model="inputParams[item.cmdbCode.filedCode]"
                                              class="mleft20"
                                              clearable
                        ...
                        <div class="mleft10 mright10 row-btn"
                             v-if="isCascaderOrListSel(item.cmdbCode.controlType.controlCode)">
    ...
    export default {
        ...
        watch: {
            ...
            queryParams: {
                ...
                handler(newVal) {
                    ...
                    // 触发重新搜索
                    this.$emit('queryByCondition', this.queryParams)
                },
                deep: true,
                immediate: true
            },
            ...
            parentParams: {
                handler(newVal) {
                    if (newVal.all && newVal.all.tree) {
                        // 一级、二级部门、业务系统
                        this.handleNodeData(newVal.all.tree)

                        // 品牌 
                    } else if (newVal.mfrFactory) {
                        this.matchCurNodeCondition('device_mfrs', newVal.mfrFactory)

                        // 设备分类、设备类型
                    } else if (newVal.deviceClass && newVal.level === 2) {
                        this.matchCurNodeCondition('device_class', newVal.deviceClass)
                    } else if (newVal.deviceClass && newVal.level === 3) {
                        this.matchCurNodeCondition('device_class', newVal.deviceClass)
                        this.matchCurNodeCondition('device_type', newVal.deviceType)
                    }
                },
                deep: true,
                immediate: true
        ...
        methods: {
            ...
            highLightCurNode(cmdbCode, curNodeId) {
                cmdbCode.conditionList.forEach((condition, index) => {
            ...
            handleConditionData(cmdbCode, conditionList) {
                this.$emit('queryAllList', this.conditionInfo)
                let option = {
                    value: '全部',
