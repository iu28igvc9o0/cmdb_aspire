<template>
    <el-dialog class="yw-dialog"
               :visible.sync="processInfo.showProcess"
               width="500px"
               title="配置项审核录入进度"
               append-to-body>
        <el-form ref="processPanel"
            ...
            <div style="font: 14px Small"
                 v-if="showCancel">
                正在向配置项审核录入数据, 共<span style="color: #409EFF; font-weight: bold;"> {{totalCount}} </span>条记录,
            ...
            <div style="font: 14px Small"
                 v-if="showFinish">
                录入配置项审核数据结束, 共<span style="color: #409EFF; font-weight: bold;"> {{totalCount}} </span>条记录,
                已处理<span style="color: #409EFF; font-weight: bold;"> {{processCount}} </span>条,
                成功<span style="color: #67C23A; font-weight: bold;"> {{successCount}} </span>条,
                失败<span style="color: #F56C6C; font-weight: bold;"> {{failCount}} </span>条.
    ...
    export default {
        ...
        methods: {
            closeImportDialog() {
                ...
                if (this.processId !== '') {
                    let _t = this
                    rbCmdbServiceFactory.stopApprovalProcess(this.processId).then(() => {
                        clearInterval(this.timeSetter)
                        _t.timeSetter = null
                    }).catch(() => {
                        _t.$message.error('移除配置项审核录入进程' + _t.processId + '失败')
                    })
                }
            },
            stopProcess() {
                this.$confirm('确认要中止配置项审核录入操作?').then(() => {
                    if (this.processId !== '') {
                        let _t = this
                        rbCmdbServiceFactory.stopApprovalProcess(this.processId).then((item) => {
                            if (item.flag === 'false') {
                                ...
                                _t.timeSetter = null
                                _t.processId = ''
                                _t.processInfo.showProcess = false
            ...
            downImportErrorRecord() {
                rbCmdbServiceFactory.downloadApprovalErrorRecord(this.processId).then((data) => {
                    ...
                    downLoadElement.download = '配置项审核录入失败记录.xlsx'
