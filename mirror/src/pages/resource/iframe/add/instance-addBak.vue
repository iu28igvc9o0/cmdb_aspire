                    ...
                    <el-form-item :prop="'groupList.' + index + '.codeList.' + codeIndex + '.defaultValue'"
                        ...
                        <div class="inline-block-middle"
                            ...
                            <el-select filterable
                                       ...
                                       v-model="code.defaultValue"
                                       :disabled="code.code ==='device_type'"
                                       @clear="resetCode(code)"
                                       @change="handleCascaderChange($event, code)">
                                <el-option v-for="(option, index) in codeOptions[code.code]"
                                           :key="index"
                            ...
                            <el-select filterable
                                       ...
                                       v-model="code.defaultValue"
                                       :disabled="code.code ==='device_class'"
                                       @clear="resetCode(code)"
                                       @change="handleCascaderChange($event, code)">
                                <el-option v-for="(option, index) in codeOptions[code.code]"
                                           :key="index"
    ...
    export default {
        ...
        data() {
            return {
                ...
                deviceClass: this.$route.query.deviceClass,
                moduleInfo: {},
                form: {
                    groupList: []
                },
        ...
        methods: {
            ...
            initCas() {
                ...
                // let _this= this
                // _this.cascaderSource.forEach(item => {
                //     if (!_this.codeOptions.hasOwnProperty(item.filedCode)) {
                //         _this.form.groupList.forEach(item => {
                //             item.codeList.forEach(c => {
                //                 if (c.refCode !== null && c.refCode.filedCode === item.filedCode) {
                //                     let option = this.codeOptions[c.refCode.filedCode].filter(function (option) {
            ...
            async resolveData() {
                ...
                this.moduleInfo.groupList.forEach(group => {
                    ...
                    group.codeList.forEach(code => {
                        ...
                        if (currentCode.controlType !== null) {
                            if (currentCode.controlType.controlCode === 'listSel' && currentCode.bindSource !== null) {
                                ...
                                if (currentCode.bindSourceType === '数据表') {
                                    if (currentCode.filedCode === 'device_class') {
                                        code.defaultValue = this.deviceClass
                                    }
                                    if (defaultValue !== null) {
                                        this.bindValues.push({ code: currentCode, value: defaultValue })
                                    }
                            ...
                            if (currentCode.controlType.controlCode === 'cascader') {
                                if (currentCode.filedCode === 'device_type') {
                                    code.defaultValue = this.deviceType
                                }
                                if (defaultValue !== null) {
                                    _this.bindValues.push({ code: currentCode, value: defaultValue })
                                }
                                _this.cascaderData.push(currentCode.refCode.filedCode)
                                _this.cascaderSource.push(currentCode)
                                // let bindValue = ''
                                if (currentCode.refCode !== null) {
                                    this.moduleInfo.groupList.forEach(group => {
                                        group.codeList.forEach(code1 => {
                                            if (code1.cmdbCode.cmdbCode.filedCode === currentCode.refCode.filedCode) {
                                                let refValue = _this.detail[code1.cmdbCode.cmdbCode.filedCode]
                                                let refBind
                                                // console.log(this.detail)
                                                // console.log(JSON.stringify(this.codeOptions))
                                                // console.log(code1.cmdbCode.cmdbCode.filedCode)
                                                if (refValue !== null) {
                                                    if (!_this.codeOptions[code1.cmdbCode.cmdbCode.filedCode]) {
                                                        rbCmdbModuleService.queryTableData({ sql: code1.cmdbCode.cmdbCode.bindSource }).then((data) => {
                                                            _this.$set(_this.codeOptions, code1.cmdbCode.cmdbCode.filedCode, data)
                                                            refBind = _this.codeOptions[code1.cmdbCode.cmdbCode.filedCode].filter(function (option) {
                                                                return option.value === refValue
                                                            })
                                                        })
                                                    } else {
                                                        refBind = _this.codeOptions[code1.cmdbCode.cmdbCode.filedCode].filter(function (option) {
                                                ...
                                                if (refBind) {
                                                    console.log('refBind')
                                                    let sql = currentCode.bindSource.replace(/\?/g, refBind[0].id)
                                                    console.log(sql)
                                                    rbCmdbModuleService.queryTableData({ sql: sql }).then((data) => {
                                                        _this.$set(_this.codeOptions, currentCode.filedCode, data)
                                                    })
                        ...
                        codeList.push({
                            code: code.cmdbCode.cmdbCode.filedCode,
