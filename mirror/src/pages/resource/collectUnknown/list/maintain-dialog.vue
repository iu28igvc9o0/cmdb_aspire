    ...
    <el-dialog class="yw-dialog"
        ...
        <el-form ref="processPanel"
            ...
            <div style="font: 14px Small"
                 v-if="showCancel">
                正在维护CI数据, 共<span style="color: #409EFF; font-weight: bold;"> {{totalCount}} </span>条记录,
            ...
            <div style="font: 14px Small"
                 v-if="showFinish">
                维护CI数据结束, 共<span style="color: #409EFF; font-weight: bold;"> {{totalCount}} </span>条记录,
                已处理<span style="color: #409EFF; font-weight: bold;"> {{processCount}} </span>条,
                成功<span style="color: #67C23A; font-weight: bold;"> {{successCount}} </span>条,
                失败<span style="color: #F56C6C; font-weight: bold;"> {{failCount}} </span>条.
    ...
    export default {
        ...
        methods: {
            closeImportDialog() {
                ...
                if (this.processId !== '') {
                    ...
                    rbCmdbServiceFactory.removeImportProcess(this.processId).then(() => {
                        ...
                        _t.$message.error('移除维护进程' + _t.processId + '失败')
                    })
                }
            },
            stopProcess() {
                this.$confirm('确认要中止维护操作?').then(() => {
                    if (this.processId !== '') {
                        let _t = this
            ...
            downImportErrorRecord() {
                ...
                rbCmdbServiceFactory.downloadUnknownError(this.processId).then((data) => {
                    let blob = new Blob([data], { type: 'application/msword' })
                    // 创建下载链接
                    let objectUrl = URL.createObjectURL(blob)
                    let downLoadElement = document.createElement('a')
                    downLoadElement.href = objectUrl
                    downLoadElement.download = '维护失败记录.xlsx'
