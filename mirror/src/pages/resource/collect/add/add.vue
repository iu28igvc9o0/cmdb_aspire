<template>
    <div class="component-container">
        <el-form ref="sumbitForm"
                 ...
                 label-width="180px">
            <el-row>
                <template v-for="prop in this.props">
                    <el-form-item v-if="prop.type!=='textarea'"
                                  :prop="prop.prop"
                                  :label="prop.label"
                                  :key="prop"
                                  :rules="prop.rules">
                        <el-col :span=11>
                            <!-- select选择框 -->
                            <el-select v-if="prop.type==='select'"
                                       :label="prop.label"
                                       v-model="values[prop.prop]"
                                       style="width:150px">
                                <el-option v-for="item in prop.option"
                                           :key="item.key"
                                           :label="item.key"
                            ...
                            <el-date-picker style="width:150px"
                                            ...
                                            size="mini"
                                            v-if="prop.type==='datetime'"
                                            v-model="values[prop.prop]"
                                            :editable="false">
                            </el-date-picker>
                            <!-- 文本输入框 -->
                            <el-input style="width:150px"
                                      v-if="prop.type===undefined"
                                      v-model="values[prop.prop]"></el-input>
                        </el-col>
                    </el-form-item>
                </template>
            </el-row>
            <template v-for="prop in this.props">
                <el-row v-if="prop.type==='textarea'"
                        :key="prop">
                    <el-col :span=24>
                        <el-form-item :prop="prop.prop"
                                      :label="prop.label"
                                      :rules="prop.rules">
                            <!-- 富文本框 -->
                            <el-input type="textarea"
                                      :rows="4"
                                      v-model="values[prop.prop]"
                                      style="width: calc(100vmin)"></el-input>
                        </el-form-item>
                    </el-col>
                </el-row>
            </template>
        ...
        <span slot="footer"
            ...
            <el-button type="primary"
                       @click="save()"
                       class="tem-search1">保存</el-button>
            <el-button type="primary"
                       @click="cancle()"
        ...
        methods: {
            save() {
                this.$refs['sumbitForm'].validate((valid) => {
                    if (valid) {
                        rbProjectDataServiceFactory.saveCollect(this.values).then(() => {
