                        ...
                        <el-table-column label="设备型号"
                            ...
                            <template slot-scope="scope">
                                <el-input class="edit-input"
                                          ...
                                          @blur="queryDeviceType($event, scope.row)"
                                          placeholder=""></el-input>
                            </template>
                        </el-table-column>
...
<script>
    ...
    import rbAssSerivceFacory from 'src/services/cmdb/rb-assessment-services.factory.js'
    export default {
        ...
        methods: {
            queryDeviceType(event, row) {
                rbAssSerivceFacory.getDeviceTypeByModel({ deviceModel: event.target.value }).then((res) => {
                    if (res) {
                        this.$set(row, 'deviceType', res.deviceType)
                    }
                })
            },
            // 数字校验
            ...
            query(activePagination = false) {
                ...
                rbAssSerivceFacory.getRepairList(params, this.queryParams).then((res) => {
                    this.loading = false
