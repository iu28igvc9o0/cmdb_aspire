    ...
    <div class="components-container yw-dashboard">
            ...
            <div class="yw-el-table-wrap">
                <el-table border
                    ...
                    <el-table-column prop="includeKey"
                                     ...
                                     :show-overflow-tooltip="true"
                                     sortable></el-table-column>
                    <el-table-column prop="noIncludeKey"
                                     label="(不包含)关键字"
                                     :show-overflow-tooltip="true"
                                     sortable></el-table-column>
        ...
        <el-dialog class="yw-dialog"
            ...
            <sysLogAlertRule ref="sysLogAlertRule"
                             ...
                             :noParam="this.$route.query.noParam"></sysLogAlertRule>
            <section class="btn-wrap">
                <el-button type="primary"
                           size="small"
        ...
        methods: {
            init_ () {
                ...
                if (this.$route.query.dialog) {
                    this.createLogAlertRuleDialog = true
                }
                if (this.timer) {
                    clearInterval(this.timer)
                    this.timer = null
            ...
            searchData () {
                ...
                LogServiceFactory.getLogAlertRuleList(this.getRequest()).then((res) => {
                    ...
                    res.result.forEach((item) => {
                        ...
                        if (item.includeKey) {
                            let keys = item.includeKey.split(',')
                            let param = []
                            for (let i = 0; i < keys.length; i++) {
                                param.push('关键字' + (i+1) + ':' + keys[i])
                            }
                            item.includeKey = param.toString()
                        }
                        if (item.noIncludeKey) {
                            let keys = item.noIncludeKey.split(',')
                            let param = []
                            for (let i = 0; i < keys.length; i++) {
                                param.push('关键字' + (i+1) + ':' + keys[i])
                            }
                            item.noIncludeKey = param.toString()
                        }
                    })
            ...
            validateAddRequest () {
                ...
                if (!logAlertRuleData.name) {
                    obj.flag = false
                    obj.msg = '请先输入日志告警规则名称!'
                    return obj
                } else if (!logAlertRuleData.alertLevel) {
                    obj.flag = false
                    obj.msg = '请先选择告警等级!'
                    return obj
                } else if (!logAlertRuleData.include) {
                    obj.flag = false
                    obj.msg = '请先选择包含(必填)!'
                    return obj
                } else if (!logAlertRuleData.includeKey && logAlertRuleData.include) {
                    obj.flag = false
                    obj.msg = '请先设置包含关键字!'
                    return obj
                } else if (!logAlertRuleData.noIncludeKey && logAlertRuleData.noInclude) {
                    obj.flag = false
                    obj.msg = '请先设置不包含关键字!'
                    return obj
                } else {
            ...
            getLogAlertRuleRequest (addForm) {
                let obj = {
                    ...
                    'keyComment': addForm.keyComment,
                }
                if (addForm.include) {
                    obj.include = 'include'
                    obj.includeKey = addForm.includeKey.toString()
                }
                if (addForm.noInclude) {
                    obj.noInclude = 'noInclude'
                    obj.noIncludeKey = addForm.noIncludeKey.toString()
                }
                return obj
            },
            addSubmit () {
                ...
                LogServiceFactory.checkName(obj.logAlertRuleData.name).then((resp) => {
                    if (resp) {
            ...
            updateSubmit () {
                ...
                LogServiceFactory.checkName(obj.logAlertRuleData.name).then((resp) => {
                    if (resp && resp != obj.logAlertRuleData.uuid) {
                        this.$alert('配置名称重复,请重新输入!', '警告', {
                            confirmButtonText: '确定'
                        })
                    } else {
            ...
            toDetail (obj) {
                LogServiceFactory.getLogAlertRuleDetail(obj.uuid).then((res) => {
                    let params = {}
                    params.ip = res.ip
                    params.pool = res.idcType
                    params.includeKey = res.includeKey
                    params.noIncludeKey = res.noIncludeKey
                    params.create_time_start = res.createTime
                    params.create_time_end = formatDate(new Date())
                    this.$router.push({
