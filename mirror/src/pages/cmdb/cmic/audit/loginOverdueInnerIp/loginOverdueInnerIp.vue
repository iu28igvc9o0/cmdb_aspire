<template>
    ...
    <div class="components-container yw-dashboard"
        ...
        <ul class="conflict-title">
            <li><span>未登记IP总数：</span><span>{{ipCount.numOfUnregistration}}</span></li>
        ...
        <el-form class="yw-form"
            ...
            <div class="senior-search"
                ...
                <el-form-item label="安全域"
                              prop="safeRegion">
                    <el-input v-model="dialogForm.safeRegion"></el-input>
                </el-form-item>
...
<script>
    ...
    export default {
        name: 'CmdbCmicAuditLoginOverdueInnerIp',
        data() {
        ...
        methods: {
            ...
            async tableExport() {
                ...
                queryData.ipType = '内网IP'
                try {
                    let res = await rbIpCollectionServiceFactory.loginOverdueIpExport(queryData)
                    let blob = new Blob([res], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' })
                    let isSupportDownload = 'download' in document.createElement('a')
                    if (isSupportDownload) {
                        const link = document.createElement('a')
                        link.download = '登记已过期IP列表' // a标签添加属性
                        ...
                        window.navigator.msSaveBlob(blob, '登记已过期IP列表.xlsx')
                    }
                } catch (error) {
                    console.log(error)
                    this.$message({
            ...
            cellStyle({ row, columnIndex }) {
                if (columnIndex === 16 && row.processingStatus === '0') {
                    return 'color:red;'
                } else {
                    return ''
                }
