<template>
    ...
    <div class="components-container yw-dashboard"
        ...
        <ul class="conflict-title">
            <li><span>未录CMDB的IP总数：</span><span>{{ipCount.numOfUnrecordIp}}</span></li>
            <li><span>待处理IP数：</span><span>{{ipCount.numOfToBeProcessedIp}}</span></li>
            <li><span>暂不处理IP数：</span><span>{{ipCount.numOfNotProcessIp}}</span></li>
            <li><span>非法使用IP数：</span><span>{{ipCount.numOfUnsuitableUseIp}}</span></li>
        </ul>
        <el-form class="yw-form"
            ...
            <div class="senior-search"
                ...
                <el-form-item label="使用状态"
                              prop="useStatus">
                    <el-select v-model="dialogForm.useStatus"
                               clearable
                               style="width:120px;"
                               placeholder="请选择">
                        <el-option v-for="item in useOptions"
                                   :key="item.value"
                                   :label="item.label"
                                   :value="item.value">
                        </el-option>
                ...
                <el-form-item label="地址用途"
                    ...
                    <el-select v-model="dialogForm.ipUse"
                               ...
                               style="width:120px;"
                               placeholder="请选择">
                        <el-option v-for="(item,index) in ipUseOptions"
                                   :key="index"
                                   :label="item.label"
                                   :value="item.value">
                        </el-option>
                ...
                <el-form-item label="分配一级业务"
                    ...
                    <el-select v-model="dialogForm.firstBusinessSystem"
                               ...
                               filterable
                               placeholder="请选择">
                        <el-option v-for="(item,index) in firstBusinessOptions"
                                   :key="index"
                                   :label="item.label"
                                   :value="item.value">
                        </el-option>
...
<script>
    ...
    export default {
        name: 'CmdbCmicAuditSurvivalCmdb',
        data() {
        ...
        methods: {
            ...
            async getTableList(type) {
                ...
                for (let prop in this.dialogForm) {
                    if (prop && prop !== 'time' && prop !== 'operateTime' && prop !== 'assignTime' && prop !== 'dcList') {
            ...
            async tableExport() {
                ...
                queryData.assignTimeEnd = this.dialogForm.assignTime ? this.dialogForm.assignTime[1] : ''
                try {
                    let res = await rbIpCollectionServiceFactory.survialCmdbExport(queryData)
                    ...
                    if (isSupportDownload) {
                        ...
                        window.navigator.msSaveBlob(blob, '存活未录入CMDB列表.xlsx')
                    }
                } catch (error) {
                    console.log(error)
                    this.$message({
            ...
            async getHandleData(type) {
                ...
                try {
                    let res = await rbIpCollectionServiceFactory.survialCmdbUpdate(queryData)
                    this.$message({
                        message: res.msg,
                        type: 'success'
                    })
            ...
            cellStyle({ row, columnIndex }) {
                if ((columnIndex === 3 && row.useStatus === '非法使用') || (columnIndex === 14 && row.processingStatus === '0')) {
                    return 'color:red;'
                } else {
                    return ''
                }
