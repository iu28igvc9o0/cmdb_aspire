<template>
    ...
    <div class="components-container yw-dashboard"
        ...
        <ul class="conflict-title">
            <li><span>未存活IP总数：</span><span>{{ipCount.numOfUnsurvivingIp}}</span></li>
            <li><span>一个月内未存活IP总数：</span><span>{{ipCount.numOfUnsurvivingIpWithinOneMonth}}</span></li>
            <li><span>三个月内未存活IP总数：</span><span>{{ipCount.numOfUnsurvivingIpWithinThreeMonths}}</span></li>
        ...
        <el-form class="yw-form"
            ...
            <el-form-item label="分配一级业务"
                          prop="firstBusinessSystem">
                <el-select v-model="dialogForm.firstBusinessSystem"
                           multiple
                           collapse-tags
                           clearable
                           placeholder="请选择">
            ...
            <el-form-item label="分配独立业务"
                          prop="aloneBusinessSystem">
                <el-select v-model="dialogForm.aloneBusinessSystem"
                           multiple
                           collapse-tags
                           clearable
                           placeholder="请选择">
            ...
            <div class="senior-search"
                ...
                <el-form-item label="地址用途"
                              prop="ipUse">
                    <el-select v-model="dialogForm.ipUse"
                               clearable
                               filterable
                               style="width:120px;"
                               placeholder="请选择">
                ...
                <el-form-item label="分配状态"
                              prop="assignStatus">
                    <el-select v-model="dialogForm.assignStatus"
                               clearable
                               style="width:120px;"
                               placeholder="请选择">
                        <el-option v-for="item in assignOptions"
                                   :key="item.value"
                                   :label="item.label"
                                   :value="item.value">
                        </el-option>
                ...
                <el-form-item label="分配时间"
                              prop="assignTime">
                    <el-date-picker v-model="dialogForm.assignTime"
                                    format="yyyy-MM-dd HH:mm:ss"
                                    value-format="yyyy-MM-dd HH:mm:ss"
                                    type="datetimerange"
                                    range-separator="至"
                ...
                <el-form-item label="分配人"
                              prop="assignUser">
                    <el-input v-model="dialogForm.assignUser"></el-input>
                </el-form-item>
                <el-form-item label="最近发现时间"
                              prop="recentDiscoverTime">
                    <el-date-picker v-model="dialogForm.recentDiscoverTime"
                                    format="yyyy-MM-dd HH:mm:ss"
                                    value-format="yyyy-MM-dd HH:mm:ss"
                                    type="datetimerange"
                                    range-separator="至"
    ...
    export default {
        ...
        data() {
            ...
            return {
                ...
                ipCount: {
                    numOfUnsurvivingIp: '',
                    numOfUnsurvivingIpWithinOneMonth: '',
                    numOfUnsurvivingIpWithinThreeMonths: '',
                ...
                collectionData: {
                    tableHeader: [
                        ...
                        {
                            label: '分配一级业务',
                            value: 'firstBusinessSystem',
                            sortable: false,
                            width: '120px'
                        },
                        {
                            label: '分配独立业务',
                            value: 'aloneBusinessSystem',
                            sortable: false,
                            width: '120px'
                        },
                        {
        ...
        methods: {
            ...
            async getTableList(type) {
                ...
                for (let prop in this.dialogForm) {
                    if (prop && prop !== 'time' && prop !== 'operateTime' && prop !== 'assignTime' && prop !== 'recentDiscoverTime' && prop !== 'requestTime' && prop !== 'dcList') {
                ...
                queryData.assignTimeBegin = this.dialogForm.assignTime ? this.dialogForm.assignTime[0] : ''
                queryData.assignTimeEnd = this.dialogForm.assignTime ? this.dialogForm.assignTime[1] : ''
                ...
                queryData.lastSurviveTimeBegin = this.dialogForm.recentDiscoverTime ? this.dialogForm.recentDiscoverTime[0] : ''
                queryData.lastSurviveTimeEnd = this.dialogForm.recentDiscoverTime ? this.dialogForm.recentDiscoverTime[1] : ''
                queryData.pageSize = this.collectionData.pagination.pageSize
                queryData.pageNo = this.collectionData.pagination.currentPage
                try {
                    let res = await rbIpCollectionServiceFactory.unsavedIpSearch(queryData)
                    ...
                    this.ipCount = {
                        ...
                        numOfUnsurvivingIpWithinOneMonth: res.numOfUnsurvivingIpWithinOneMonth,
                        numOfUnsurvivingIpWithinThreeMonths: res.numOfUnsurvivingIpWithinThreeMonths,
            ...
            async tableExport() {
                ...
                queryData.assignTimeEnd = this.dialogForm.assignTime ? this.dialogForm.assignTime[1] : ''
                try {
                    let res = await rbIpCollectionServiceFactory.unsavedIpExport(queryData)
                    let blob = new Blob([res], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' })
                    let isSupportDownload = 'download' in document.createElement('a')
                    if (isSupportDownload) {
                        const link = document.createElement('a')
                        link.download = '已分配未存活IP列表' // a标签添加属性
                        ...
                        window.navigator.msSaveBlob(blob, '已分配未存活IP列表.xlsx')
                    }
                } catch (error) {
                    console.log(error)
                    this.$message({
            ...
            async getHandleData(type) {
                ...
                try {
                    let res = await rbIpCollectionServiceFactory.unsavedIpUpdate(queryData)
                    this.$message({
                        message: res.msg,
                        type: 'success'
                    })
            ...
            cellStyle({ row, columnIndex }) {
                if (columnIndex === 17 && row.processingStatus === '0') {
                    return 'color:red;'
                } else {
                    return ''
                }
