        ...
        <el-form class="yw-form"
            ...
            <div class="senior-search"
                ...
                <el-form-item label="线路类型"
                              prop="circuitType">
                              <el-select v-model="dialogForm.circuitType"
                               clearable
                               filterable
                               style="width:120px;"
                               placeholder="请选择">
                        <el-option v-for="item in circuitTypeOptions"
                                   :key="item.value"
                                   :label="item.label"
                                   :value="item.value">
                        </el-option>
...
<script>
    ...
    export default {
        name: 'CmdbCmicAuditSurvialUnassignPublicIp',
        data() {
        ...
        created() {
            ...
            this.getSelectValue('public_segment_line_type', 'circuitTypeOptions')// 线路类型
        ...
        methods: {
            ...
            async getTableList(type) {
                ...
                for (let prop in this.dialogForm) {
                    if (prop && prop !== 'time' && prop !== 'operateTime') {
                ...
                try {
                    let res = await rbIpCollectionServiceFactory.survialUnassignPublicIpSearch(queryData)
            ...
            async tableExport() {
                ...
                queryData.operatingTimeEnd = this.dialogForm.operateTime ? this.dialogForm.operateTime[1] : ''
                try {
                    let res = await rbIpCollectionServiceFactory.survialUnassignPublicIpExport(queryData)
            ...
            async getHandleData(type) {
                ...
                try {
                    let res = await rbIpCollectionServiceFactory.survialPublicIpUpdate(queryData)
                    this.$message({
                        message: res.msg,
                        type: 'success'
                    })
            ...
            cellStyle({ row, columnIndex }) {
                if (columnIndex === 10 && row.processingStatus === '0') {
                    return 'color:red;'
                } else {
                    return ''
                }
