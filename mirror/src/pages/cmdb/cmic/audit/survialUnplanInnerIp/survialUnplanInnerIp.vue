<template>
    ...
    <div class="components-container yw-dashboard"
        ...
        <ul class="conflict-title">
            <li><span>存活未规划IP总数：</span><span>{{unPlanCount.numOfSurvivingUnplannedIp}}</span></li>
        ...
        <el-form class="yw-form"
            ...
            <div class="senior-search"
                ...
                <el-form-item label="数据来源"
                              prop="auditSource">
                    <el-select v-model="dialogForm.auditSource"
                               clearable
                               style="width:120px;"
                               placeholder="请选择">
                        <el-option v-for="item in auditSourceOptions"
                                   :key="item.value"
                                   :label="item.label"
                                   :value="item.value">
                        </el-option>
...
<script>
    ...
    export default {
        name: 'CmdbCmicAuditSurvialUnplanInnerIp',
        data() {
            ...
            var validateIp1 = (rule, value, callback) => {
                ...
                if (newVal) {
                    ...
                    // let unqualifiedIp = temp.filter(item => !this.isValidIps(item))
                    // if (unqualifiedIp.length > 0) {
                    //     callback(new Error('请输入正确的网管设备IP'))
                    // } else {
        ...
        methods: {
            ...
            async getTableList(type) {
                ...
                try {
                    let res = await rbIpCollectionServiceFactory.survialUnplanInnerIpSearch(queryData)
                    this.collectionData.tableBody = res.result || []
                    this.collectionData.pagination.total = res.count
                    this.unPlanCount = {
                        numOfSurvivingUnplannedIp: res.numOfSurvivingUnplannedIp,
            ...
            async tableExport() {
                ...
                queryData.operatingTimeEnd = this.dialogForm.operateTime ? this.dialogForm.operateTime[1] : ''
                try {
                    let res = await rbIpCollectionServiceFactory.survialUnplanInnerIpExport(queryData)
            ...
            async getHandleData(type) {
                ...
                try {
                    let res = await rbIpCollectionServiceFactory.survialUnplanInnerIpUpdate(queryData)
                    this.$message({
                        message: res.msg,
                        type: 'success'
                    })
            ...
            register() {
                let queryParams = {
                    ...
                    device_type: {
                        id: this.currentModuleInfo && this.currentModuleInfo.module_id || '',
