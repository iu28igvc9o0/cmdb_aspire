<template>
    <div>
        <el-menu v-loading="loading"
                 ...
                 style="border: none;">
            <el-submenu v-for="(item) in (show === 'configured' || show === 'unconfigured' ? show === 'configured' ? configuredMenu : unconfiguredMenu : allMenu)"
                        :index="item.id"
                        :key="item.id"
                        style="border-bottom: 1px solid #EBEEF5">
                <template slot="title">{{item.name}}</template>
                <template v-for="child in item.childModules">
                    <el-menu-item v-if="child !== undefined"
                                  :index="child.id"
                                  :key="child.id"
                                  @click="changeRouter(child.id,child.name)">
                        {{child.name}}
                    </el-menu-item>
                </template>
            </el-submenu>
