    ...
    <el-dialog v-loading="loading"
                ...
                <el-row>
                    <el-col :span="21">
                        <el-col style="line-height: 30px"
                                :span="6">模型名称：</el-col>
                        <el-col :span="18">
                            <el-select v-model="currentModuleId"
                                       value-key="name"
                                       style="width: 100%">
                                <el-option :key="currentModuleInfo.moduleId"
                                           :value="currentModuleInfo.moduleId"
                                           :label="currentModuleInfo.moduleName">
                                </el-option>
                            </el-select>
                        </el-col>
                    </el-col>
                        ...
                        <el-col :span="18">
                            <el-select v-loading="loading"
                                ...
                                <div v-for="item in data"
                                     :key="item.code">
                                    <el-option v-if="item.code !== 'Y_name'"
                                               :value="item"
                                               :label="item.name">
                                    </el-option>
                                </div>
                    ...
                    <el-scrollbar style="height: 100%">
                        <el-form :data="formData"
                            ...
                            <el-row v-for="item in formItems"
                                ...
                                <el-col :span="21">
                                    <el-row type="flex">
                                        <el-tooltip :content="item.name">
                                            <el-col style="line-height: 30px;text-overflow: ellipsis;overflow:hidden;white-space: nowrap; "
                                                    :span="6">{{item.name}}</el-col>
                                        </el-tooltip>
                                        <el-col :span="18">
                                            <el-form-item :prop="item.code">
                                                <el-input style="width: 100%"
                                                          v-model="formData[item.code]"
                                                          v-if="item.type === 'singleRowText'"></el-input>
                                                <el-select clearable
                                                           style="width: 100%"
                                                           value-key="name"
                                                           v-model="formData[item.code]"
                                                           v-if="item.type === 'listSel'">
                                ...
                                <el-col :span="2"
                                        @mouseover.native="showDelete(item)"
                                        @mouseleave.native="hidDelete(item)">
        ...
        <span slot="footer"
            ...
            <el-button type="primary"
                       @click="query()">查询并保存</el-button>
            <el-button @click="resetDialog()">取 消</el-button>
            <el-button v-show="selectQuery && selectQuery.id"
                       @click="deleteQueryCondition()">删 除</el-button>
        </span>
    </el-dialog>
</template>
    ...
    export default {
        ...
        methods: {
            ...
            toDoAddOrUpdate() {
                ...
                this.emitData()
                rbCmdbServiceFactory.updateQueryConditions(data).then(() => {
                }).catch(() => {
            ...
            deleteQueryCondition() {
                this.$confirm('确认是否删除当前查询条件?', '提示', {
                    ...
                    rbCmdbServiceFactory.deleteQueryConditions(this.selectQuery.id).then(() => {
