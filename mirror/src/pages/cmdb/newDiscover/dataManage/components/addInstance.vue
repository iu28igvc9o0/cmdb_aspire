        ...
        <el-form :model="validateForm"
            ...
            <el-form-item label=""
                ...
                <el-select style=""
                           ...
                           placeholder="配置类型"
                           v-model="moduleId"
                           :disabled="discoveryModuleId !== null && discoveryModuleId !== ''">
                    <el-option-group v-for="group in selectData"
                                     :key="group.id"
                                     :label="group.name">
                        <el-option v-for="item in group.item"
                                   :key="item.id"
                                   :label="item.name"
                                   :value="item.id">
                        </el-option>
            ...
            <div v-if="moduleId!=null && moduleId != ''">
                ...
                <div id="ss"
                    ...
                    <div v-for="(item, index) in dataList"
                         :key="index">
                        <el-form-item :label="item.form.name+':'"
                                      prop="name"
                                      v-if="item.form.type=='singleRowText' && item.form.code == 'Y_name'"
                                      required>
                            <div style="width:calc(100% - 87px);float:left;"
                                 align="left">
                        ...
                        <el-form-item :label="item.form.name+':'"
                                      v-if="item.form.hidden == 'false' && item.form.code != 'Y_name'"
                                      :prop="'key'+item.form.id"
                                      :required=" item.form.required == 'true' ? true : false ">
                            <div style="width:calc(100% - 87px);float:left;"
                                 align="left">
                                <!--单行-->
                                <el-input v-if="item.form.type=='singleRowText'"
                                          :disabled="item.form.code == 'ip' && discoveryLog.instanceName !== null && discoveryLog.instanceName !== ''"
                                          v-model="validateForm['key'+item.form.id]"></el-input>
                                <!-- 多行-->
                                <el-input v-if="item.form.type=='multiRowText'"
                                          type="textarea"
                                          :rows="2"
                                          v-model="validateForm['key'+item.form.id]"></el-input>

                                <!--下拉-->
                                <el-select v-if="item.form.type=='listSel'"
                                           v-model="validateForm['key'+item.form.id]">
                                    <el-option v-for="op in item.formOptions"
                                               :key="op.value"
                                               :label="op.name"
                                               :value="op.value">
                                    </el-option>
                                ...
                                <!--单选-->
                                <el-radio-group v-if="item.form.type=='singleSel'"
                                                v-model="validateForm['key'+item.form.id]">
                                    <el-radio v-for="op in item.formOptions"
                                              :label="op.value"
                                              :key="op.value">{{op.name}}</el-radio>
                                </el-radio-group>
                                <!--多选  -->
                                <el-checkbox-group v-if="item.form.type=='multiSel'"
                                                   v-model="validateForm['key'+item.form.id]">
                                    <el-checkbox v-for="op in item.formOptions"
                                                 :label="op.value"
                                ...
                                <div v-if="item.form.type=='double' ">
                                    <el-input-number v-model="validateForm['key'+item.form.id]"
                                                     :step="0.01"></el-input-number>&nbsp;
                                    {{item.form.unit}}
                                </div>
                                <!--整数-->
                                <div v-if="item.form.type=='int' ">
                                    <el-input-number v-if="item.form.type=='int' "
                                                     v-model="validateForm['key'+item.form.id]"
                                ...
                                <!--表格-->
                                <el-table v-if="item.form.type=='table'"
                                          :data="validateForm['key'+item.form.id]"
                                          highlight-current-row
                                          @current-change="handleCurrentChange">
                                    <el-table-column v-for="it in item.formFields"
                                                     :key="it.id"
                                                     inline-template
                                                     :label="it.name">
                                        <div>
                                            <el-input :value="row.name"
                                                      size="small"
                                                      v-model="row[it.key]">
                                            </el-input>
                                        </div>
                                ...
                                <!--引用-->
                                <el-input v-if="item.form.type=='reference' "
                                          v-model="validateForm['key'+item.form.id]"></el-input>
                                <!--富文本-->
                                <vue-editor v-if="item.form.type=='richText' "
                                            v-model="validateForm['key'+item.form.id]"></vue-editor>

                                <!-- 级联菜单 -->
                                <el-cascader v-if="item.form.type=='cascader'"
                                             :options="item.casoptions[item.form.id]"
                                             v-model="validateForm['key'+item.form.id]">
                                </el-cascader>
                                <!-- 属性分组 -->
                                <div v-if="item.form.type=='groupLine'">
                                    <div style="height:35px;background:#20a0ff;margin-left:-100px;margin-right:-70px;color:#fff;line-height:35px;border-color:#20a0ff;border-radius:4px">
                                        ...
                                        <i class="el-icon-caret-bottom"
                                           style="float:right;margin-right:15px;line-height:35px;"></i>
                                    </div>
                                </div>
                ...
                <el-form-item align="center"
                              style="margin-top:15px;padding-right:50px;padding-left:0px;">
                        ...
                        rbCmdbServiceFactory.updateDiscoveryLog(this.log.id, { status: '已维护' }).then((data) => {
                            if (data.code === 'success') {
                                rbCmdbServiceFactory.addInstance(this.ruleForm).then((data) => {
                                    ...
                                    rbCmdbServiceFactory.updateDiscoveryLog(this.log.id, '待处理').then(() => {
                                    })
                                })
                            }
                        }).catch(() => {
