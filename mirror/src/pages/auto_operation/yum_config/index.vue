...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            // 更新YUM配置文件
            updateYum(req) {
                rbAutoOperationYumServicesFactory.updateYumConfigModel(req).then(res => {
                    if (res.flag) {
                        this.$message('更新成功')
                        this.search()
                        this.closeYumBox()
            ...
            // 创建YUM配置文件
            createYum(req) {
                rbAutoOperationYumServicesFactory.createYumConfigModel(req).then(res => {
                    if (res.flag) {
                        this.$message('创建成功')
                        this.search()
                        this.closeYumBox()
            ...
            saveYum() {
                this.$refs.yumForm.validate(valid => {
                    ...
                    let req = {
                        ...
                        uploadFilePath: this.yumInfo.uploadFilePath, // YUM配置文件上传路径
            ...
            removeYum(id) {
                this.$confirm('您确定要删除该文件吗？').then(() => {
                    this.loading = true
                    rbAutoOperationYumServicesFactory.removeYumConfig(id).then(res => {
                        if (res.flag) {
                            this.$message('删除成功')
                            this.search()
                        } else {
            ...
            showYumBox(type, row) {
                ...
                if (type === 'addYum') {
                    this.yumDialogName = '新建YUM配置记录'
                    this.viewDetail(this.currentYumData)
                } else if (type === 'editYum') {
                    this.yumDialogName = '编辑YUM配置记录'
                    this.viewDetail(this.currentYumData)
                } else {
                    this.yumDialogName = 'YUM配置记录详情'
                    this.viewDetail(this.currentYumData)
                }
