<template>
    <div class="components-container alertListTable"
         style="margin:10px">
        ...
        methods: {
                    ...
                    if (valid) {
                        if (this.editName !== this.form.name) {
                            rbAlertFilterServicesFactory.getByName(this.form.name).then((res) => {
                                ...
                                rbAlertFilterServicesFactory.update(this.form.id, this.form).then(() => {
                                    this.resetFields()
                                    this.currentPage = 1
                ...
                this.$refs['form'].validate((valid) => {
                    if (valid) {
                        ...
                        rbAlertFilterServicesFactory.getByName(this.form.name).then((res) => {
                            ...
                            rbAlertFilterServicesFactory.create(this.form).then(() => {
                                this.resetFields()
                                this.currentPage = 1
            ...
            copyPost() {
                rbAlertFilterServicesFactory.getByName(this.form.name).then((res) => {
                    ...
                    rbAlertFilterServicesFactory.copy(this.form).then(() => {
                        this.resetFields()
                        this.currentPage = 1
                        this.$message.success('新增成功')
                        this.$nextTick(() => {
