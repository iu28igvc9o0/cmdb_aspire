...
<script>
    ...
    export default {
        name: 'MirrorAlertFilterSceneAdd',
        data() {
            return {
                editName: '',
                editFilterId: '',
                ...
                if (this.sceneId && !this.oper) {
                    if (this.editName !== this.form.name || this.editFilterId !== this.form.filter_id) {
                        rbAlertFilterSceneServicesFactory.getByName(this.form.name, this.form.filter_id).then((res) => {
                            ...
                            rbAlertFilterSceneServicesFactory.update(this.form.id, this.form).then(() => {
                                this.back()
                    ...
                    rbAlertFilterSceneServicesFactory.getByName(this.form.name, this.form.filter_id).then((res) => {
                        ...
                        rbAlertFilterSceneServicesFactory.create(this.form).then(() => {
                            this.back()
                    ...
                    if (option.source) {
                        rbAlertFilterSceneServicesFactory.getSourceVal(option.source, option.method).then((res) => {
                            ...
                            if (typeof data[0] !== 'object') {
                                ...
                                let transform = JSON.parse(option.transform)
                                data.forEach(item => {
                                    if (transform) {
                                        let obj = {}
                                        for (let tf in transform) {
                                            obj[tf] = item[transform[tf]]
                                        }
                                        this.dataFilterList[filterIndex].rowList[rowIndex].colList[2].sources.push(obj)
                                    } else {
                                        this.dataFilterList[filterIndex].rowList[rowIndex].colList[2].sources.push(item)
                                    }
                                })
                            }
                        })
