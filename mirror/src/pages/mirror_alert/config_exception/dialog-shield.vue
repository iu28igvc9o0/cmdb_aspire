...
<script>
    ...
    export default {
        ...
        methods: {
            queryDetail(id) {
                rbAlertConfigExceptionServiceFactory.getDetail(id).then((date) => {
                    this.formData = date
            ...
            submit() {
                this.$refs['formData'].validate((valid) => {
                    ...
                    if (valid) {
                        ...
                        if (this.updateFlag) {
                            rbAlertConfigExceptionServiceFactory.update(this.formData).then(() => {
                                this.$emit('closeDialog', 'update')
                            }).finally(() => {
                                this.submitDisabled = false
                            })
                        } else {
                            rbAlertConfigExceptionServiceFactory.insert(this.formData).then(() => {
                                this.$emit('closeDialog', 'update')
                            }).finally(() => {
                                this.submitDisabled = false
                            })
