...
<script>
    ...
    import rbAlertConfigExceptionServiceFactory from 'src/services/alert/rb-alert-config-exception-service.factory'
    ...
    export default {
        ...
        methods: {
            ...
            query(activePagination = false) {
                ...
                rbAlertConfigExceptionServiceFactory.getConfigExceptionList(this.queryParams).then((data) => {
            ...
            delBatch() {
                ...
                rbAlertConfigExceptionServiceFactory.delete(ids).then(() => {
                    this.query()
                })
            },
            del(id) {
                rbAlertConfigExceptionServiceFactory.delete([id]).then(() => {
                    this.query()
                })
            },
            ...
            openBatch() {
                ...
                if (!openFlag) {
                    this.$alert('已启用异常信息规则无法再次启用', '警告', {
                        confirmButtonText: '确定'
                    })
                    return
                }
                this.$confirm('确认启用?').then(() => {
                    rbAlertConfigExceptionServiceFactory.start(ids).then(() => {
                        this.query()
                    })
                })
            },
            open(id) {
                this.$confirm('确认启用?').then(() => {
                    rbAlertConfigExceptionServiceFactory.start([id]).then(() => {
                        this.query()
                    })
                })
            },
            ...
            closeBatch() {
                ...
                if (!closeFlag) {
                    this.$alert('已停用异常信息规则无法再次停用', '警告', {
                        confirmButtonText: '确定'
                    })
                    return
                }
                this.$confirm('确认停用?').then(() => {
                    rbAlertConfigExceptionServiceFactory.stop(ids).then(() => {
                        this.query()
                    })
                })
            },
            close(id) {
                this.$confirm('确认停用?').then(() => {
                    rbAlertConfigExceptionServiceFactory.stop([id]).then(() => {
                        this.query()
                    })
                })
            },
