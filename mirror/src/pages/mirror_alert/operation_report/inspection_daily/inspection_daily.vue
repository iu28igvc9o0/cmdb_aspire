...
<script>
    ...
    export default {
        name: 'MirrorAlertOperationReportInspectionDaily',
        data() {
            return {
                loading: true,
                dateRange: [],
        ...
        mounted() {
            // this.curdDate = moment(new Date(new Date().getTime() - 24 * 60 * 60 * 1000)).format('YYYY-MM-DD HH:')
            let now = new Date()
            let before = new Date(new Date().getTime() - 24 * 60 * 60 * 1000)
            this.dateRange = [formatDate(before), formatDate(now)]
            this.initDict()
        },
        methods: {
            checkTime() {
            ...
            initDict() {
                ...
                rbmaintenanceCommonUtil.getProducesByType(obj).then((res) => {
                    const list = []
                    for (let i in res) {
                        list.push(res[i].produce_name)
                    }
                    this.deviceMfrsList = list
                })
        ...
        if (res) {
          this.deviceMfrsList = res
            ...
            search() {
                ...
                let page = {
                    inspectionDateStart: this.dateRange[0],
                    inspectionDateEnd: this.dateRange[1],
