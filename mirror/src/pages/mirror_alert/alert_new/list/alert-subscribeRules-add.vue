        ...
        <el-dialog class="yw-dialog"
            ...
            <section class="yw-dialog-main">
                <el-form class="yw-form is-required"
                         ...
                         ref="addForm"
                         label-width="100px">
                    <p class="tit-box">为已选的<span>{{notifyParmas.alertIds.split(',').length}}</span>条告警新增告警通知订阅</p>
                    <el-form-item label="订阅规则">
                        <el-radio v-model="newOld"
                                  label="1">已配置订阅规则</el-radio>
                        <el-radio v-model="newOld"
                                  label="2">新增订阅规则</el-radio>
                    </el-form-item>
                    <el-form-item label="订阅规则"
                                  prop="subscribeRules">
                        <template v-if="newOld == 2">
                            <el-input placeholder="请输入订阅规则名称"
                                      v-model="addForm.subscribeRules"
                                      style="width:526px"></el-input>
                        </template>
                        <template v-else>
                            <el-select v-model="rulesId"
                                       placeholder="请选择"
                                       style="width:526px">
                                <el-option v-for="item in rulesName"
                                           :key="item.id"
                    ...
                    <el-form-item label="维护用户"
                        ...
                        <el-input placeholder=""
                                  v-model="addForm.defensetor.join(',')"
                        ...
                        <div class="transfer-wrap">
                            ...
                            <section class="transfer-right">
                                <div class="transfer-right-top">
                                    ...
                                    <!-- <div class="search-box-wrap">
                                        <el-input placeholder="请输入姓名、手机号、邮箱查询"></el-input>
    ...
    export default {
        ...
        data() {
            return {
                ...
                addForm: {
                    defensetor: [],//   维护人
                    alertIds: '04f6ebc6-934a-4a02-8034-1edad7e4e272',// 用户选择的告警id
                    creator: sessionStorage.username,// 创建人
                ...
                notifyParmas: {
                    emails: '',
                    alertIds: '04f6ebc6-934a-4a02-8034-1edad7e4e272',
                    subject: ''
                },
                readData: {
                    emails: '',
        ...
        methods: {
            ...
            querySubscribeRules() {
                subscribeFactory.querySubscribeRules().then((res) => {
                    const map = new Map()
                    this.rulesName = res.filter(e => !map.has(e.id) && map.set(e.id, 1))
                    // this.rulesName = res
                })
            },
            ...
            goAdd() {
                ...
                this.addForm.alertIds = this.notifyParmas.alertIds
                ...
                if (!this.showWrite) {
                    if (!this.addForm.subscribeRules) {
                        ...
                        return
                    } else if (this.addForm.notifyType == 1 && this.addForm.emailType === '') {
                    ...
                    this.addForm.isRecurrenceInterval = ''
                    this.addForm.period = ''
                    this.addForm.defensetor = ''
                }
                console.log('addForm===', this.addForm)
                subscribeFactory.createSubscribeService(this.addForm).then((res) => {
                    console.log(res)
                    if (res === 'success') {
                        this.$message({
                            message: '创建成功!',
                        ...
                        this.notifyParmas.alertIds = ''
                        this.addForm.alertIds = ''
            ...
            // 新增自定义次数
            handleChange(value) {
                // this.addForm.isRecurrenceInterval = value
            ...
            dialogCancel() {
                this.addDialog.dialogVisible = false
                this.addDialog.id = this.addDialog.id.split(',')
            ...
            log(evt) {
                if (!this.showWrite) {
                    ...
                    this.notifyParmas.subject += '【' + evt.added.element.name + '】'
            ...
            goNotify() {
                subscribeFactory.emailNotify(this.notifyParmas).then((res) => {
                    this.readData.smsContent = res[0].emails
            ...
            queryDetail(id) {
                subscribeFactory.getSubscribeRulesById(id).then(res => {
                    if (res) {
                        ...
                        // this.notifyParmas.alertIds = res.alertSubscribeRulesManagementResponeList[0].alertId
                        // this.addForm.uuid = res.alertSubscribeRulesManagementResponeList[0].alertId
                        // this.addForm.alertIds = res.alertSubscribeRulesManagementResponeList[0].alertId
                    }
                })
            },
        ...
        mounted() {
            this.getDepartment()
            this.querySubscribeRules()
            // this.queryDetail({ id: '1d95c385-9517-4749-9af5-9cf8c9a7a9ad' })
        },
        watch: {
            rulesId(val) {
                this.queryDetail({ id: val })
            },
            ...
            repass(val) {
                if (val === '-1') {
                    ...
                    this.addForm.recurrenceCount = this.customVal
                }
            },
            addDialog: {
                handler(val) {
                    this.notifyParmas.alertIds = val.id
                    console.log(this.notifyParmas.alertIds)

                    this.addForm.alertIds = val.id
                },
                immediate: true
            },
            filterText(val) {
