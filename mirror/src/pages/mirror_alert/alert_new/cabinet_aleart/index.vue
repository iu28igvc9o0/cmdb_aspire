<template>
    <div class="preview">
        <!-- 条件、列表 -->
        <div v-if="$route.query.keyComment.indexOf('列头柜')>-1"
             class="th4">
        ...
        <div v-if="$route.query.keyComment.indexOf('机柜')>-1"
             class="th4">
            <asp-smart-table ref="aspSmartTable"
                             :tableJson="powerJson"
                             v-model="powerModel"
...
<script>
    import tableJson from './cabinetAleart.json'
    import powerJson from './powerAleart.json'
        ...
        methods: {
                    ...
                    case 'bizSystemCount':
                        {
                            this.$router.push({
                                ...
                                query: {
                                    idcType: this.idcParams,
                                    roomId: this.routeInfo.roomId,
                                    cabinetColumn: this.totalParams.cabinetColumn,
                                    bizSystemCount: this.totalParams.cabinetCount,
                                    alertStatus: this.totalParams.alertStatus,
                                }
            ...
            */
            beforeHttp({ tableItem, params, httpMethod, row }) {
                console.log('idcParams', this.idcParams)
                this.modelList.cabinetCount = this.totalParams.cabinetCount
                this.modelList.deviceCount = this.totalParams.deviceCount
                this.modelList.idcType = this.idcParams
                this.modelList.cabinetColumn = this.totalParams.cabinetColumnName
                this.powerModel.cabinetColumn = this.totalParams.cabinetColumnName
                this.powerModel.cabinetName = this.totalParams.cabinetColumnName
                this.powerModel.deviceCount = this.totalParams.deviceCount
                this.powerModel.idcType = this.idcParams
                params.idcType = this.idcParams
                params.roomId = this.routeInfo.roomId
                Object.assign(this.modelList, this.routeInfo)
                Object.assign(this.powerModel, this.routeInfo)
                console.log('params===', params)
                console.log('modelList===', this.modelList)
                console.log('powerModel===', this.powerModel)

                // params.cabinetColumnName = this.routeInfo.cabinetColumn
                switch (tableItem.columnName) {
                    // 查询短信发送记录
                    case 'table_1605147046872':
                        Object.assign(params, {
                            page_no: params.page,
                            page_size: params.rows,
                            roomId: this.routeInfo.roomId,
                            cabinetCount: this.totalParams.cabinetCount,
                            deviceCount: this.totalParams.deviceCount,
                            cabinetColumn: this.totalParams.cabinetColumnName,
                            cabinetName: this.totalParams.cabinetColumnName,
