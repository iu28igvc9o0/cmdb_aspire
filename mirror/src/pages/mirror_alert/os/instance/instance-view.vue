            ...
            <div class="condition">
                <div class="content">
                    ...
                    <div class="center"
                        ...
                        <div>网络设备</div>
                        <div class="POD">
                            <p v-for="item in deviceTypeList"
                               :key="item.id"
                               @click="equipment(item.value, item.id)"
                               :class='item.value==queryForm.device_type?"equipmentColor":""'><span></span>{{item.name}}</p>
                        </div>
                        <!-- <div class="moreClass" @click="MoreS()" v-if="moreIconS">更多<i class="el-icon-arrow-down"></i></div>
                    ...
                    <div class="center"
                        ...
                        <div class="POD">
                            <p class="PODp"
                               ...
                               :class='item.value==queryForm.pod_name?"PODColor":""'><span></span>{{item.name}}</p>
                        </div>
                        <div v-show="pod"
                             class="moreClass"
                             @click="More()"
                             v-if="moreIcon">更多<i class="el-icon-arrow-down"></i></div>
                        <div class="moreClass moreClassRight"
                             @click="MoreUP()"
                             v-else>收起<i class="el-icon-arrow-up"></i></div>
                    </div>
                    <div class="down"
                        ...
                        <div class="selectQuery"
                            ...
                            <p>
                                <el-select v-model="queryForm.device_mfrs"
                                           placeholder="设备厂商"
                                           filterable
                                           :clearable="true"
                                           @change="changeQuery">
                                    <el-option v-for="item in mfrFactoryList"
                                               :key="item.id"
                                               :label="item.name"
                                               :value="item.value"
                                               prefix-icon="el-icon-search"></el-option>
                            ...
                            <p>
                                <el-input v-model="queryForm.device_sn"
                                          placeholder="设备序列号"
                                          :clearable="true"
                                          @blur="blurQuery"></el-input>
                            </p>
                            ...
                            <p>
                                <el-input v-model="queryForm.project_name"
                                          placeholder="项目名称"
                                          :clearable="true"
                                          @blur="blurQuery"></el-input>
                            </p>
                            <p>
                                <el-input v-model="queryForm.dept_operation"
                                          placeholder="维护部门"
                                          :clearable="true"
                                          @blur="blurQuery"></el-input>
                            </p>
                            ...
                            <div>
                                ...
                                <!--<p>-->
                                <!--<el-select v-model="queryForm.deviceType" placeholder="设备类型" filterable :clearable = "true" @change="changeQuery">-->
                                ...
                                <!-- <p style="display:-webkit-inline-box; margin: 1px -9px;">
                                    <el-date-picker v-model="insertTime" style="width: 280px" type="daterange"
                                                    range-separator="至"
                                                    start-placeholder="设备录入日期" end-placeholder="设备录入日期" value-format="yyyy-MM-dd"
                                                    @change="changeQuery">
                                    </el-date-picker>
                                </p> -->
                            </div>
                    ...
                    <div style="border:1px solid #DEE9FC">
                        <el-table stripe
                            ...
                            <el-table-column type="selection"
                                             :width="customWidth('check')"></el-table-column>
                            <!-- sortable="custom" -->
                            <el-table-column prop="ip"
                                ...
                                <template slot-scope="scope">
                                    <el-button style="text-decoration:none"
                                               type="text"
                                               align="left"
                                               @click="toDetail(scope.row)">{{scope.row.ip}}</el-button>
                                </template>
                            </el-table-column>
                            <el-table-column prop="device_name"
                                ...
                                <template slot-scope="scope">
                                    <el-button style="text-decoration:none"
                                               type="text"
                                               align="left"
                                               @click="toDetail(scope.row)">{{scope.row.device_name}}</el-button>
                                </template>
                            </el-table-column>
                            ...
                            <el-table-column prop="os_detail_version"
                                             label="os详细版本"
                                             :min-width="'120'"
                                             sortable
                            ...
                            <el-table-column label="操作"
                                             :width="customWidth('操作')"
                                             fixed="right">
                                <template slot-scope="scope">
                                    <div class="yw-table-operator">
                                        <el-button type="text"
                                                   @click="toInstanceDetail(scope.row)">{{scope.row.approvalCount}}</el-button>
                                    </div>
                                </template>
                            </el-table-column>
                        </el-table>
    ...
    export default {
        ...
        data() {
            return {
                ...
                queryKeys: ['moduleId', 'idcType', 'pod', 'deviceClass', 'deviceType', 'deviceModel', 'department1', 'department2', 'bizSystem', 'ip', 'deviceName', 'deviceSN', 'room', 'project_name', 'deviceStatus', 'maintenanceFactory', 'mfrFactory', 'startInsertTime', 'endInsertTime', 'dept_operation', 'ops'],
                moreIcon: true,
                moreIconZ: true,
                moreIconS: true,
                PODheightZ: false,
        ...
        methods: {
            ...
            MoreS() {
                this.rbHttp.sendRequest({
                    ...
                    this.deviceTypeList = this.fillSelectOption(res, 'device_type')
                    if (this.deviceTypeList.length > 5) {
            ...
            MoreUPS() {
                this.rbHttp.sendRequest({
                    ...
                    this.deviceTypeList = this.fillSelectOption(res, 'device_type')
                    this.deviceTypeList = this.deviceTypeList.slice(0, 5)
            ...
            queryInstanceHeader() {
                ...
                rbCmdbServiceFactory.getInstanceHeader(this.queryForm.moduleId || '').then((item) => {
                    item.forEach((item2) => {
                        if (item2.filedCode !== 'ip' && item2.filedCode !== 'device_name' && item2.filedCode !== 'insert_time' && item2.filedCode !== 'update_time') {
            ...
            queryData: function () {
                ...
                this.queryForm.condicationCode = 'instance_list'
                this.queryForm.moduleType = 'host'
                this.queryForm.module_id = this.queryForm.moduleId
                rbCmdbServiceFactory.getInstanceOsListV3(this.queryForm).then((data) => {
            ...
            initDict() {
                ...
                this.rbHttp.sendRequest({
                    method: 'POST',
                    data: { 'produce_type': '维保供应商' },
                    url: '/v1/cmdb/module/module/data?moduleType=default_produce_module_id'
                }).then((res) => {
                    res.forEach((item) => {
                        let obj = {}
                        obj.name = item.produce_name
                        obj.value = item.produce_name
                        this.maintenanceFactoryList.push(obj)
                    })
            ...
            // 选择设备分类
            equipment(item) {
                this.queryForm.device_type = item
                this.searchInstanceList()
            },
            ...
            queryBizSystem: function () {
                this.rbHttp.sendRequest({
                    ...
                    res.forEach((item) => {
                        let obj = {}
                        obj.name = item.bizSystem
                        obj.value = item.bizSystem
                        this.bizSystemList.push(obj)
                    })
            ...
            exportData() {
                this.loading = true
                this.loading_text = '正在导出数据,由于数据量较大请耐心等待...'
                ...
                this.queryForm.sortType = this.paginationData.order
                this.queryForm.queryType = this.queryType || 'normal'
                rbCmdbServiceFactory.exportOsListV3(this.queryForm).then((res) => {
                    let blob = new Blob([res], { type: 'application/vnd.ms-excel' })
                    let objectUrl = URL.createObjectURL(blob)
                    // window.location.href = objectUrl
                    let downLoadElement = document.createElement('a')
                    downLoadElement.href = objectUrl
                    downLoadElement.download = '版本变更数据列表.xlsx'
