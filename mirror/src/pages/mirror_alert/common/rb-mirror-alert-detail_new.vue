        ...
        <el-collapse class="yw-dashboard-section"
            ...
            <el-collapse-item name="1">
              ...
              <rb-mirror-alert-status :mode="'detail'"
                    ...
                    <el-form-item v-for="(item,index) in detailShowlist"
                        ...
                        <span v-else-if="item.fieldCode === 'alert_level'">
                            <rb-mirror-alert-status :mode="'detail'"
                                                    :status="obj.alert_level"></rb-mirror-alert-status>
                        </span>
                        <span v-else-if="item.detailShowName !== '告警级别'">
                            <span v-if="Object.keys(obj).indexOf(item.fieldCode) != 1">
                                <span>{{obj[item.fieldCode]}}</span>&nbsp;&nbsp;&nbsp;&nbsp;
                                <a v-if="item.fieldCode === 'remark'"
                                   style="padding-left: 10px"
                                   @click="change.remarkChangeDialog=true">[修改]</a>
                                <a v-else-if="item.fieldCode === 'host_name'"
                                   style="text-decoration: none;cursor:pointer"
            ...
            <el-collapse-item name="2">
                ...
                <el-tabs class="yw-tabs"
                    ...
                    <el-tab-pane label="关联告警分析"
                                 name="twe">
                        <cabinetAleart v-if="tableList.twe.flag"
                                       :alertId="obj.alert_id"
                                       :totalParams="totalParams"
                                       :idcParams="obj.idc_type"
                                       :source="obj.source"></cabinetAleart>
                    </el-tab-pane>
...
<script>
    ...
    import cabinetAleart from 'src/pages/mirror_alert/alert_new/cabinet_aleart/index.vue'
    ...
    export default {
        ...
        methods: {
            getAlertDeviceInformation() {
                rbAlertServiceFactory.alertDeviceInformation({ alertId: this.alertId }).then(res => {
                    if (res) {
            ...
            refreshGen() {
                ...
                this.tableList.twe.flag = false // 销毁组件
                this.$nextTick(() => {
                    this.tableList.twe.flag = true // 重建组件
                })
            },
            alertDetailInit() {
            ...
            //     this.obj = res
            //     console.log('ididididididid',res)
            ...
            activeNameInit() {
                if (this.obj.source === '衍生告警') {
                    this.tableList.six.flag = true
                    this.activeName = 'six'
                } else if (this.obj.event_id && this.obj.event_id.indexOf('primary_') === 0) {
                    this.tableList.ten.flag = true
                    this.activeName = 'ten'
                }
            },
        },
