            ...
            <div class="yw-el-table-wrap">
                <el-table :data="tableData"
                    ...
                    <el-table-column prop="strategy_num"
                        ...
                        <template slot-scope="scope">
                            <a href="#"
                               @click="gotoScene(scope.row.receiver, scope.row.id)">{{scope.row.strategy_num}}</a>
                        </template>
                    </el-table-column>
...
<script>
    ...
    export default {
        name: 'MirrorAlertEmailInfoReceiveList',
        data() {
            return {
                detailFlag: false,
                editFlag: false,
        ...
        methods: {
            ...
            gotoScene(receiver, id) {
                this.$router.push({ path: '/mirror_alert/emailInfo_receive/receiver-config/list/', query: { receiver: receiver, recipientId: id } })
            },
            // 1 element相关方法 多选框
            ...
            deleteFilterBatch() {
                ...
                this.$confirm('删除收件邮箱配置?', '删除', {
                    ...
                    rbEmailConfigServicesFactory.delete(ids).then(() => {
            ...
            deleteFilterScene(id, name) {
                this.$confirm('删除收件邮箱 ' + name + '配置?', '删除', {
                    ...
                    rbEmailConfigServicesFactory.delete(id).then(() => {
                        this.$message.success('删除成功')
                        this.search()
                    }).catch(() => {
                        this.$message.error('删除失败')
