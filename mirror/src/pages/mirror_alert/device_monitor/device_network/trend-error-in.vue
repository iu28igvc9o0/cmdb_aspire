                ...
                lineDatas: [
                    {
                        ...
                        chartDatas: [],
                        details: { legend: { show: false }, seriesName: [{ name: '', yLabel: 'cpu', xLabel: 'name', yLabelName: '条', yLabelDatas: '', xLabelDatas: '', lineColor: '#6FDBF8' }] }
                    },
                ],
                tableTitles: [],
                tableDatas: [],
                ...
                return this.$api.queryServerData(params).then((res) => {
                    ...
                    this.tableDatas = res.series.map((item, index) => {
                        ...
                        let resObj = {
                            ...
                            data: item.data,
                            max: item.max,
                            maxHtml: `<span style="color:${textColor}">${item.max}</span>`,
                            avg: item.avg,
                            avgHtml: `<span style="color:${textColor}">${item.avg}</span>`,
                            lineColor: flagColor,
                        }
