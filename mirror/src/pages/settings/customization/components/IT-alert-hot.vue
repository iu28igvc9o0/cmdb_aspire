...
<script>
    ...
    import { alert_level_obj, expire_status_obj } from 'src/pages/mirror_alert/alert_his/config/options.js'
    ...
    export default {
        ...
        methods: {
            ...
            clickTableRow(row) {
                ...
                // 工单
                let url = `${sessionStorage.getItem(
                    'X7_SERVER_URL'
                )}/front/#/inst/${row['order_id']}?mirrorToken=${sessionStorage.getItem('mirror')}`
            ...
            getTableDatas() {
                ...
                this.tableTitles = [
                    { name: 'level', title: '级别', style: { sortable: true, isHtml: true, width: 65 } },
                    { name: 'device_ip', title: '设备IP', style: { sortable: true, width: 120 } },
                    { name: 'idc_type', title: '资源池', style: { sortable: true, width: 90, align: 'left' } },
                    { name: 'device_class', title: '告警分类', style: { sortable: true, width: 100 } },
                    { name: 'cur_moni_time', title: '告警时间', style: { sortable: true, isHtml: true, width: 150 } },
                    { name: 'moni_index', title: '告警内容', style: {} },
                ]

                if (this.chartData.activeFilter === 1) {
                    this.tableTitles = [
                        { name: 'level', title: '级别', style: { sortable: true, isHtml: true, width: 65 } },
                        { name: 'device_ip', title: '设备IP', style: { sortable: true, width: 120 } },
                        { name: 'idc_type', title: '资源池', style: { sortable: true, width: 90, align: 'left' } },
                        { name: 'device_class', title: '告警分类', style: { sortable: true, width: 100 } },
                        { name: 'cur_solve_time', title: '处理时间', style: { sortable: true, width: 150 } },
                        { name: 'person', title: '处理人', style: { width: 70 } },
                        { name: 'moni_index', title: '告警内容', style: {} },
                    ]
                }
                ...
                this.$api.queryAlertHotNew(params).then((res) => {
                    this.tableDatas = res.map((item) => {
                        ...
                        return {
                            ...
                            // 'level': alert_level_obj[item.alert_level].name,
                            'levelHtml': levelFlag,
                            'biz_sys': item['biz_sys'],
                            'device_class': item['device_class'],
                            'person': item['exec_user'],
                            'personTooltip': `姓名：${item['exec_user']}`,
                            'moni_index': item['moni_index'],
                            'cur_solve_time': formatDate(item['exec_time']),
                            'cur_moni_time': formatDate(item['cur_moni_time']),
                            'cur_moni_timeHtml': formatDate(item['cur_moni_time']),
                            'alert_id': item['alert_id'],
                            'order_id': item['order_id'],
                            'tableRowClassName': expire_status_obj[item['expire_status']] ? expire_status_obj[item['expire_status']]['row-color'] : ''
                        }
                    })
                })
