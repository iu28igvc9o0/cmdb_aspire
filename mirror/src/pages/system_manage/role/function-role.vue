...
<script>
    ...
    export default {
        ...
        watch: {
            actionList: {
                ...
                handler: function (val) {
                    if (val && val.length > 0) {
                        const needSelected = []
                        const actionList = this.selectedActionList
        ...
        methods: {
            deleteAction(row) {
                this.selectedActionList = this.selectedActionList.del(row)
            },
            e_reset() {
                this.roleForm = {
                    id: '',
            ...
            handleContorlSelected(selection) {
                if (selection.length > 0 && !_.map(this.selectedActionList, 'id').includes(selection[0].id)) {
                    this.selectedActionList.push(selection[0])
                }
            },
            handleContorlSelectedAll(selection) {
                selection.forEach(item => {
                    if (!_.map(this.selectedActionList, 'id').includes(item.id)) {
                        this.selectedActionList.push(item)
                    }
                })
            },
            processVisitData() {
