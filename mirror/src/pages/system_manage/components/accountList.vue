<template>
    <div class="yw-el-table">
        <el-table :data="accountList"
                  ...
                  >
            <el-table-column type="selection" width="55" v-if="isNetwork">
            </el-table-column>
        ...
        methods: {
                    ...
                    if (res) {
                        res.results.map((item) => {
                            ...
                            this.accountList.push({
                                ...
                                status: item.userInfo ? item.userInfo.ldap_status : '',
                                role: roleName ? roleName.join(',') : '',
            ...
            handleUpdate(row) {
                this.$confirm('此操作将切换用户状态, 是否继续?', '提示', {
                    ...
                    systemAccountService.toggleType(row.account, !status).then(() => {
                        if (row.status === 1) {
                            row.status = 0
                        } else {
                            row.status = 1
