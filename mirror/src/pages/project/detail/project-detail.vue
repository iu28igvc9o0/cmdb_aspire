    ...
    <div>
        <div class="rb-page-heading2">
            ...
            <div class="rb-heading-button-group2">
                <el-button size="small"
                           @click="queryResourceDetail">查看资源详情</el-button>
                <el-button size="small"
                           @click="projectModify">更新</el-button>
                <el-button v-show="projectDelete"
                           size="small"
                           type="info"
        ...
        <el-card style="margin-bottom: 15px">
            ...
            <el-row :gutter="20"
                ...
                <el-col :span="2"
                        ...
                        align="right">
                    <div style="color: #A1A1A1;">创建者</div>
                </el-col>
                <el-col :span="2">
                    <div style="color: #666;margin-top: -10px;">{{project_detail.created_by}}</div>
                </el-col>
            </el-row>
            <el-row :gutter="20">
                <el-col :span="2"
                        ...
                        align="right">
                    <div style="color: #A1A1A1">创建时间</div>
                </el-col>
                <el-col :span="10">
                    <div style="color: #666">{{project_detail.created_at | formatUtcStr}}</div>
                </el-col>
                <el-col :span="2"
                        align="right">
                    <div style="color: #A1A1A1">更新时间</div>
                </el-col>
                <el-col :span="6">
                    <div style="color: #666">{{project_detail.updated_at | formatUtcStr}}</div>
                </el-col>
            </el-row>
        </el-card>
        ...
        <el-tabs type="border-card"
            ...
            <el-tab-pane label="角色"
                         name="first">
                <el-input v-model="roleSearch"
                          placeholder="关键字"
                          style="width: 300px;"
                          size="small">
                    <el-button slot="append"
                               icon="el-icon-search"
                               @click="getProjectRoles"></el-button>
                </el-input>
                <el-button v-show="roleCreate"
                           size="mini"
                           type="primary"
                           style="float: right"
                           @click="createRoleFun">创建角色</el-button>
                <el-table v-show="roleView"
                          :data="roleData"
                          stripe
                          style="width: 100%">
                    <el-table-column label="名称">
                        <template slot-scope="scope">
                            <span @click="toRoleDetailFun(scope.row.name)"
                                  class="spanTo">{{ scope.row.name }}</span>
                            <el-popover placement="top-start"
                                        trigger="click"
                                        @show="getRoleUsers(scope.row)">
                                <template v-for="user in roleUsers">
                                    <el-tag v-if="roleUsersIsExist"
                                            :key="user.user"
                                ...
                                <el-button v-if="!roleUsersIsExist"
                                           type="text"
                                           style="color: black;cursor: text;margin: 0px;padding: 0px;">该角色下暂无用户</el-button>
                                <i slot="reference"
                                   class="fa fa-group"
                                   style="cursor: pointer;color: #428bca;"></i>
                            </el-popover>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column label="操作"
                                     width="100">
                        <template slot-scope="scope">
                            <el-button v-show="roleDelete"
                                       size="mini"
                                       type="primary"
                                       icon="el-icon-delete"
                                       style="padding: 3px 5px"
                                       @click="handleDeleteRole(scope.$index, scope.row)"></el-button>
                        </template>
                    </el-table-column>
                </el-table>
            </el-tab-pane>
            ...
            <el-tab-pane label="成员"
                ...
                <el-input v-model="userSearch"
                    ...
                    <el-button slot="append"
                               icon="el-icon-search"
                               @click="getProjectAccounts"></el-button>
                </el-input>
                <el-button v-show="userCreate"
                           size="mini"
                           type="primary"
                           @click="openAssignRoleToUsers"
                           style="float: right">添加用户</el-button>
                <el-table v-show="userView"
                          :data="userData"
                          stripe
                          style="width: 100%">
                    <el-table-column label="用户名"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-popover v-show="userView"
                                        placement="top-start"
                                        trigger="click"
                                        @show="getUserRoles(scope.row)">
                                <template v-for="role in userRoles">
                                    <el-tag v-if="userRolesIsExist"
                                            :key="role.role_name"
                                            :disable-transitions="true"
                                            :closable="roleAssign"
                                            size="small"
                                        ...
                                        <a @click="toRoleDetailFun(role.role_name)"
                                           style="cursor: pointer;text-decoration: none;">{{role.role_name}}</a>
                                    </el-tag>
                                </template>
                                <el-button v-if="!userRolesIsExist"
                                           type="text"
                                           style="color: black;cursor: text;margin: 0px;padding: 0px;">该用户暂未分配任何角色</el-button>
                                <i slot="reference"
                                   class="fa fa-group"
                                   style="cursor: pointer;color: #428bca;"></i>
                            </el-popover>
                    ...
                    <el-table-column label="操作"
                                     width="100">
                        <template slot-scope="scope">
                            <el-button v-show="roleAssign"
                                       size="mini"
                                       type="primary"
                                       icon="el-icon-plus"
                                       style="padding: 3px 5px"
                                       @click="openAssignRoles(scope.row)"></el-button>
                        </template>
                    </el-table-column>
                </el-table>
            </el-tab-pane>
            ...
            <el-tab-pane label="事件"
                         name="third">
                <event-list v-show="eventView"
                            type="'organization'"
                            :noMassage="true"></event-list>
            </el-tab-pane>
        </el-tabs>
        ...
        <el-dialog title="添加角色"
            ...
            <table width="100%">
                ...
                <tr>
                    ...
                    <td style="padding-left: 20px">
                        <div>
                            <el-select v-model="selectedRoleOptions"
                                       multiple
            ...
            <span slot="footer"
                ...
                <el-button type="primary"
                           v-if="selectedRoleOptions.length"
                           @click="assignRolesConfirm">确 定</el-button>
                <el-button type="primary"
                           v-else
        ...
        <el-dialog title="添加用户"
            ...
            <table width="100%">
                <tr>
                    ...
                    <td style="padding-left: 20px">
                        <div>
                            <el-select v-model="selectedRoleOneOption"
                                       placeholder="选择要添加的角色"
                                       style="width: 90%">
                                <el-option v-for="item in roleListOptions"
                                           :key="item.name"
                                           :label="item.name"
                    ...
                    <td style="padding-left: 20px">
                        <div>
                            <el-select filterable
                                       v-model="selectedUserListOptions"
                                       multiple
            ...
            <span slot="footer"
                ...
                <el-button type="primary"
                           v-if="selectedRoleOneOption && selectedUserListOptions.length"
                           @click="assignRoleToUsersConfirm">确 定</el-button>
                <el-button type="primary"
                           v-else
                           disabled>确 定</el-button>
            </span>
        ...
        methods: {
            ...
            deleteProject() {
                this.$confirm(`确定要删除项目${this.project_name}?`, '删除项目', {
                    ...
                    rbProjectDataService.deleteProject({ project_name: this.project_name }).then(() => {
                        this.$message({
                            type: 'success',
                            message: '删除成功'
                        })
            ...
            handleDeleteRole(index, row) {
                this.$confirm(`确定要删除角色${row.name}?`, '删除角色', {
                    ...
                    rbProjectDataService.deleteRole({ role_name: row.name, project_name: this.project_name }).then(() => {
                        this.$message({
                            type: 'success',
                            message: '删除成功'
                        })
            ...
            revokeRoleUsers(user) {
                ...
                rbProjectDataService.revokeRoleUsers(data, { role_name: user.role_name, project_name: this.project_name }).then(() => {
            ...
            revokeUserRoles(role) {
                ...
                rbProjectDataService.revokeRoleUsers(data, { role_name: role.role_name, project_name: this.project_name }).then(() => {
            ...
            assignRolesConfirm() {
                ...
                rbProjectDataService.assignUserRoles(data, { username: this.username, project_name: this.project_name }).then(() => {
                })
            },
            queryResourceDetail() {
            ...
            assignRoleToUsersConfirm() {
                ...
                rbProjectDataService.assignRoleUsers(data, { role_name: this.selectedRoleOneOption, project_name: this.project_name }).then(() => {
                    this.getProjectAccounts()
                })
            },
            createRoleFun() {
