<template>
    <div>
        <template v-for="(obj, index) in resourceJson">
            <div v-if="obj.allData.length"
                 :key="index">
                <div style="width: 100%;background-color: #ffffff;height: 50px;margin-bottom: 2px;line-height: 50px;">
                    ...
                    <el-input v-model="obj.search"
                              placeholder="按名称搜索"
                              style="width: 250px;float: right;right: 5px;"
                              size="small"></el-input>
                </div>
                <el-table :data="obj.data"
                          stripe
                          style="width: 100%">
                    ...
                    <el-table-column label="状态">
                        <template slot-scope="scope">
                            <el-button v-if="scope.row.status==='success'"
                                       ...
                                       type="success"
                                       round
                                       style="cursor: default">成功</el-button>
                            <el-button v-else-if="scope.row.status==='warning'"
                                       size="mini"
                                       type="warning"
                                       round>告警</el-button>
                            <el-button v-else
...
<script>
    ...
    export default {
        ...
        watch: {
            'resourceJson.knamespace.search': function (val) {
                this.resourceJson.knamespace.data = this.resourceJson.knamespace.allData.filter((data) => {
                    return data.name.includes(val)
                })
            },
            'resourceJson.priv_regis_proj.search': function (val) {
