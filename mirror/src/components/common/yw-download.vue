...
<script>
    ...
    export default {
        props: ['showImport', 'importType', 'dataStart', 'SelectProps', 'params', 'instanceId', 'maintenanceId', 'screenParams'],
        data() {
            return {
        ...
        methods: {
            downLoadTemp() {
                ...
                if (this.importType === 'reportTable') {// 资源上报
                    let params = {
                        resourceOwner: 'resource',
                        resourceClass: '均峰值利用率'
                    }
                    rbCmdbServiceFactory.downloadReportTable(params).then((data) => {
