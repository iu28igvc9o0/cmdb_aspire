...
navConfig.categories.forEach((categorie) => {
  ...
  if (categorie.routers && categorie.routers.length > 0) {
    const firstBase = categorie.base
    // 巡检管理
    crumbtwo = categorie.name
    let mainPage = ''
    mainPage = `${firstBase}${categorie.routers[0].path}`
    categorie.routers.forEach((item) => {
      crumbthree = item.name
      let breadcrumb = [crumbone, crumbtwo, crumbthree]
      breadcrumb = breadcrumb.filter(data => {
        return data
      ...
      homeRouter.push({
        ...
        path: `${firstBase}${item.path}`,
        component: () => import( /* webpackChunkName: "thirdchunk" */ `src/pages${firstBase}${item.path}/${item.component}`)
        // component: resolve => require([`src/pages${firstBase}${item.path}/${item.component}`], resolve)
      })
    })
  }
