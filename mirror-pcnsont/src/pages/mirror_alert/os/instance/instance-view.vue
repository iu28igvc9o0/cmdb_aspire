            ...
            <div class="form">
                ...
                <div class="yw-el-table-wrap table">
                    <div style="border:1px solid #DEE9FC">
                        ...
                        >
                            <el-table-column type="selection" :width="customWidth('check')"></el-table-column>
                            <!-- sortable="custom" -->
                            <el-table-column prop="ip" label="管理IP" :min-width="'120'"  fixed sortable show-overflow-tooltip>
                                <template slot-scope="scope">
                                    <el-button style="text-decoration:none" type="text" align="left" @click="toDetail(scope.row.id)">{{scope.row.ip}}</el-button>
                                </template>
                            </el-table-column>
                            <el-table-column prop="device_name" label="设备名称" :min-width="'120'" fixed sortable show-overflow-tooltip>
                                <template slot-scope="scope">
                                    <el-button style="text-decoration:none" type="text" align="left" @click="toDetail(scope.row.id)">{{scope.row.device_name}}</el-button>
                                </template>
                            </el-table-column>
                            <el-table-column prop="insert_time" label="创建时间" :min-width="'120'" sortable show-overflow-tooltip></el-table-column>
                            <el-table-column prop="update_time" label="最后更新时间" :min-width="'120'" sortable show-overflow-tooltip></el-table-column>
                            <el-table-column prop="os_detail_version" label="os详细版本" :min-width="'120'" sortable show-overflow-tooltip></el-table-column>
                            <el-table-column prop="pod_name" label="POD名称" :min-width="'120'" sortable show-overflow-tooltip></el-table-column>
                            <el-table-column prop="roomId" label="机房位置" :min-width="'120'" sortable show-overflow-tooltip></el-table-column>
                            <el-table-column prop="device_class" label="设备分类" :min-width="'120'" sortable show-overflow-tooltip></el-table-column>
                            <el-table-column prop="device_type" label="设备类型" :min-width="'120'" sortable show-overflow-tooltip></el-table-column>
                            <el-table-column prop="idcType" label="资源池" :min-width="'120'" sortable show-overflow-tooltip></el-table-column>
                            ...
                            <!--</el-table-column>-->
                            <!--</div>-->
                            <el-table-column label="操作" :width="customWidth('操作')" fixed="right">
                                <template slot-scope="scope">
                                    <div class="yw-table-operator">
                                        <el-button type="text" @click="toInstanceDetail(scope.row.id)">{{scope.row.approvalCount}}</el-button>
                                    </div>
                                </template>
                            </el-table-column>
                        </el-table>
            ...
            toDetail(instanceId) {
                if ((this.queryForm.moduleId || '') === '') {
                    ...
                    rbCmdbServiceFactory.getModuleByInstanceId(instanceId).then((data) => {
                        if (data) {
                            _t.$router.push({ path: '/resource/iframe/detail', query: { moduleId: data.module.id, name: data.module.name, instanceId: instanceId, state: 'detail',from: 'os' } })
                        } else {
                            _t.$message.error('获取模型数据失败')
                        }
                    })
                } else {
                    this.$router.push({ path: '/resource/iframe/detail', query: { moduleId: this.queryForm.moduleId, instanceId: instanceId, state: 'detail',from: 'os' } })
                }
            },
            toInstanceDetail(instanceId) {
                if ((this.queryForm.moduleId || '') === '') {
                    let _t = this
                    rbCmdbServiceFactory.getModuleByInstanceId(instanceId).then((data) => {
                        if (data) {
