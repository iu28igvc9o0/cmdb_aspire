<template>
  <div @click="router()" class="resourceManagement">
      <el-form class="yw-form clearfix" :inline="true" label-width="80px">
          <el-form-item label>
              <el-button
                      class="btn-time"
              ...
              <el-button
                      ...
                      style="margin-left: 10px;"
                      type="text"
                      @click="getDataByTime('1month')"
              >最近一个月</el-button>

          </el-form-item>
          <el-form-item label="自定义时间" style="margin-left: 50px;">
              <el-date-picker
                      v-model="dateRange"
                      style="width:338px"
                      @change="changeTab"
          ...
          <el-form-item label="时间粒度" style="margin-left: 50px;" >
              <el-button
                      class="btn-time"
      ...
      <div class="condition">
        <div class="content">
          <div class="center" :class="{ PODHeightZ: PODheightZ }">
            ...
            <div class="moreClass moreClassRight" @click="MoreUPZ()" v-else>
            ...
            <div class="center" :class="{PODHeightS:PODheightS}">
                <div>网络设备类型</div>
                <div class="POD">
                    <p class="PODS" v-for="item in netDeviceTypeList" :key = "item.id"  @click="equipment(item.value, item.id)" :class='item.value==queryForm.deviceType?"equipmentColor":""'><span></span>{{item.name}}</p>
                </div>
          ...
          <div class="center" :class="{ PODHeight: PODheight }">
            ...
            <div v-show="pod" class="moreClass" @click="More()" v-if="moreIcon">
              更多<i class="el-icon-arrow-down"></i>
            </div>
            <div class="moreClass moreClassRight" @click="MoreUP()" v-else>
              收起<i class="el-icon-arrow-up"></i>
            </div>
          </div>
          <div class="down" :class="{ activeDown0: inputHome }">
            <div class="heightQuery" :class="{ activeDown: inputHome }">
              高级查询
            </div>
            <div class="selectQuery" :class="{ activeDown1: inputHome }">
              ...
              <div @click="showInput" v-if="conditionShow">
                更多<i class="el-icon-arrow-down"></i>
              </div>
              <div @click="showInput" style="margin-right:0" v-else>
                收起<i class="el-icon-arrow-up"></i>
              </div>
              ...
              <div v-show="inputShow">
                ...
                <p>
                  <el-input
                    v-model="queryForm.deviceProject"
                    placeholder="项目名称"
                    :clearable="true"
                    @blur="blurQuery"
                  ></el-input>
                ...
                <p>
                  ...
                  >
                    <el-option
                      v-for="item in maintenanceFactoryList"
                      :key="item.id"
                      :label="item.name"
                      :value="item.value"
                      prefix-icon="el-icon-search"
                ...
                <p>
                  <el-input
                    v-model="queryForm.deptOperation"
                    placeholder="维护部门"
                    :clearable="true"
                    @blur="blurQuery"
                  ></el-input>
              ...
              >
                <template slot-scope="scope">
                  <el-button
                    ...
                    align="left"
                    @click="toDetail(scope.row.id)"
                    >{{ scope.row.device_name }}</el-button
                  >
                </template>
              </el-table-column>
                <el-table-column
                ...
                >
                    <template slot-scope="scope">
                        <el-button   type="text" @click="getTrends(scope.row ,'cpu','max')" >{{getData(scope.row.cpuMax)}}</el-button>
                    </template>
                </el-table-column>
                <el-table-column
                        prop="cpuAvg"
                ...
                >
                    <template slot-scope="scope">
                        <el-button   type="text" @click="getTrends(scope.row,'cpu','avg')" >{{getData(scope.row.cpuAvg)}}</el-button>
                    </template>
                </el-table-column>
                <el-table-column
                        prop="cpuMin"
                ...
                >
                    <template slot-scope="scope">
                        <el-button   type="text" @click="getTrends(scope.row,'cpu','min')" >{{getData(scope.row.cpuMin)}}</el-button>
                    </template>
                </el-table-column>
                <el-table-column
                        prop="memMax"
                ...
                >
                    <template slot-scope="scope">
                        <el-button   type="text" @click="getTrends(scope.row,'mem','max')" >{{getData(scope.row.memMax)}}</el-button>
                    </template>
                </el-table-column>
                <el-table-column
                        prop="memAvg"
                ...
                >
                    <template slot-scope="scope">
                        <el-button   type="text" @click="getTrends(scope.row,'mem','avg')" >{{getData(scope.row.memAvg)}}</el-button>
                    </template>
                </el-table-column>
                <el-table-column
                        prop="memMin"
                ...
                >
                    <template slot-scope="scope">
                        <el-button   type="text" @click="getTrends(scope.row,'mem','min')" >{{getData(scope.row.memMin)}}</el-button>
                    </template>
                </el-table-column>
            </el-table>
            <YwPagination
