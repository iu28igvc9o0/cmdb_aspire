  ...
  export default {
    ...
    data () {
      return {
        ...
        tabData: [
          { label: '基本属性信息', name: 'instanceDetail', index: 0, isShow: this.$route.query.from !== 'os'},
          { label: '变更流程管理', name: 'changeInfo', index: 1, isShow: this.$route.query.from !== 'os' },
          { label: '资源关系管理', name: 'instancePortRelation', index: 2, isShow: this.$route.query.from !== 'os' },
          { label: '告警管理信息', name: 'alertProgram', index: 3, isShow: this.$route.query.from !== 'os' },
    ...
    methods: {
      ...
      getDeviceTypeWithDisplay(){
        this.currentTabComponent = this.$route.query.from === 'os' ? 'approvalInfo' : 'instanceDetail'
        var params = {
