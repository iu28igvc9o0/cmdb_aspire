      ...
      <div class="topo content-item"
        ...
        <section class="chart-section">
          <el-tooltip effect="light"
                      class="item"
                      placement="top" @dblclick.native="view(item)">
            <div slot="content"
                 style="width: 200px">
            ...
            <div class="viewer" v-if="activeTabName == 'customize'">
              <img :src="topFtpUrl + item.id + '.png'" class="image"/>
            </div>
            <div class="viewer"
                 :is="activeName"
                 :defaultOptions="item" v-else>
            </div>
          </el-tooltip>
        </section>
      </div>
...
export default {
  ...
  // },
    methods: {
        ...
        view(item) {
            let param = {}
            param.onlineTab = false
            param.name = item.name
            param.id = item.id
            if (this.activeTabName === 'online') {
                param.onlineTab = true
            }
            this.$router.push({
                path: '/topo/view',
                query: param
