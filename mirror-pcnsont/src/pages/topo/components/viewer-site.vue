  ...
  <div id="wrapper"
    ...
    <div class="has-topo"
      ...
      <div class="top">
        ...
        <div class="tools">
            <div class="line" v-if="defaultOptions.onlineTab"></div>
            <el-tooltip class="item"
                        effect="dark"
                        content="检索路径"
                        placement="bottom-start" v-if="defaultOptions.onlineTab">
                <div class="btn-lists">
                    <div class="btn-item"
                         @click="getPath">
                        <i class="el-icon-search" style="margin-top: 15px;"></i>
                    </div>
                </div>
            </el-tooltip>
          <div class="line"></div>
      ...
      <el-dialog class="yw-dialog"
                 title="检索链路"
                 width="800px" v-if="pathFlag"
                 :visible.sync="pathFlag" :close-on-click-modal="false">
          <el-form :inline="true" :model="form" class="demo-form-inline"  :rules="rules" ref="form">
              <el-form-item label="源地址"  prop="sourceIp">
                  <el-input v-model="form.sourceIp" placeholder="源地址"></el-input>
              </el-form-item>
              <el-form-item label="目标地址"  prop="destIp">
                  <el-input v-model="form.destIp" placeholder="目标地址"></el-input>
              </el-form-item>
              <el-form-item>
                  <el-button type="primary"
                             size="small"
                             @click="submitForm()">检索</el-button>
    ...
    methods: {
      ...
      submitForm(){
        this.$refs['form'].validate((valid) => {
          if (valid) {
            let data= {
              'sourceIp': this.form.sourceIp,
              'destIp':this.form.destIp,
              'content':this.topoData.content
            }
            TopoApi.getPath(data).then(data =>{
              if(data){
                if(data.code == 200){
                  let oldPathList = JSON.parse(JSON.stringify(this.pathList));

                  if(data.data.length == 0){

                    this.$message({
                      message: "没有查询到链路，请检查输入",
                      type: 'info'
                    })
                    return
                  }
                  ...
                  this.$refs.iframe.contentWindow.postMessage({
                    cmd: 'getPath',
                    params: {pathList:data.data,oldPathList:oldPathList}
                  }, '*')
                }else if(data.code == 4){
                  this.$message({
                    message: data.msg,
      ...
      // const data = this.defaultOptions
        ...
        // 外部vue向iframe内部传数据
                var resValue
                try {
                  resValue = await this.$http.sendRequest({
                    method: 'POST',
