  ...
  <div id="home">
    ...
    <div class="home-right-pane"
      ...
      <div class="right-head clearfix">
        ...
        <!-- </header> -->
        <h1 class="header__title">pctsot网络运维工具</h1>
        <div class="pull-left">
          <!--
            新导航颜色
          ...
          <el-menu :default-active="activeIndex"
            ...
            <template v-for="(item, index) of firstMenu">
              <el-menu-item v-if="item.url || item.routers"
                            :key="index"
                            :index="`${index}`">
...
<!--                  </svg>-->
                ...
                <navbar-card :menu="item"
                             @toRoute="toRoute" v-if="item.children && item.children.length > 0"/>
              </el-submenu>
            </template>
          </el-menu>
        </div>
...
export default {
  ...
  methods: {
    ...
    handleSelect(index) {
      ...
      if (this.firstMenu[index].routers && this.firstMenu[index].routers.length > 0) {
        let router = `${this.firstMenu[index].base}${this.firstMenu[index].routers[0].path}`
