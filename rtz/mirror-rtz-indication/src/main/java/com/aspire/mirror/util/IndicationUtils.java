...
public class IndicationUtils {
    ...
    /**
     * 获取指定日期 向前的position个自然日
     * @param dateStr 指定日期
     * @param position 指定的n个自然日
     * @return
     */
    public static List<String> getPrevDays(String dateStr, int position) {
        Date date = null;
        try {
            date = DateUtils.parseDate(dateStr, new String[]{IndicationConst.DATE_PATTERN});
        ...
        for (int n = Math.abs(position); n > 0; n--) {
            String value = DateFormatUtils.format(calendar.getTime(), IndicationConst.DATE_PATTERN);
            days.add(value);
    ...
    public static String getPrevDay(String dateStr, String pattern) {
        Date date = null;
        try {
    ...
    public static String formatUnit(Object value, String unit) {
        ...
        return formatUnit(value, unit, decimal,1);
    }
    ...
    public static String getProvinceByCode(String provinceCode) {
        ...
        if (provinceCode.equals(IndicationConst.COUNTRY_PROVINCE_CODE)) {
            return IndicationConst.COUNTRY_PROVINCE_NAME;
        }
        return null;
    }
