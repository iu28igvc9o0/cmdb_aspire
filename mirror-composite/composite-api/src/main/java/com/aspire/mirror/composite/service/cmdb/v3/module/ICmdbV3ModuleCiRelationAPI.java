 ...
 * 作者姓名           修改时间           版本号              描述
 */
@RequestMapping("${version}/v3/cmdb/module/relation")
public interface ICmdbV3ModuleCiRelationAPI {
    ...
    @ApiResponses(value = {@ApiResponse(code = 200, message = "保存成功", response = List.class),
            @ApiResponse(code = 500, message = "内部错误")})
    Map<String, Object> save(@RequestBody CmdbV3ModuleCiRelation relation);
    ...
    @ApiResponses(value = {@ApiResponse(code = 200, message = "更新成功", response = List.class),
            @ApiResponse(code = 500, message = "内部错误")})
    Map<String, Object> update(@RequestBody CmdbV3ModuleCiRelation relation);
