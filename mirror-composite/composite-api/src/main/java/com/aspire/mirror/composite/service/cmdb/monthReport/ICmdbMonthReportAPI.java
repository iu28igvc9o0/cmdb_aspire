package com.aspire.mirror.composite.service.cmdb.monthReport;
 ...
 * 作者姓名           修改时间           版本号              描述
 */
@RequestMapping("${version}/cmdb/monthReport")
public interface ICmdbMonthReportAPI {
    ...
    @ApiOperation(value = "资源回收/资源清理", notes = "资源回收/资源清理", tags = {"CMDB MonthReport API"})
    @ApiResponses(value = {@ApiResponse(code = 200, message = "查询成功", response = Map.class),
            @ApiResponse(code = 500, message = "内部错误")})
    ...
    @ApiResponses(value = {@ApiResponse(code = 200, message = "查询成功", response = Map.class),
            @ApiResponse(code = 500, message = "内部错误")})
    List<Map<String, Object>> countPhyUtzTrend(@RequestParam("month") String month,
                                               @RequestParam("countType") String countType,
                                               @RequestParam("deviceType") String deviceType,
                                               @RequestParam(value = "idcType", required = false) String idcType);
