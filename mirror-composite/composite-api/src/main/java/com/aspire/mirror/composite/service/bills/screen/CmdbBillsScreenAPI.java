package com.aspire.mirror.composite.service.bills.screen;
 ...
 **/
@RequestMapping("${version}/cmdb/bill/screen")
public interface CmdbBillsScreenAPI {
    ...
    @ApiResponses(value = {@ApiResponse(code = 200, message = "查询成功", response = void.class),
            @ApiResponse(code = 500, message = "内部错误")})
    void exportBillsDetail(@RequestParam("department") String department,
    ...
    @RequestMapping(value = "/bills/detail/exportPdf",method = RequestMethod.GET)
    @ApiOperation(value = "账单具体详情明细", notes = "账单具体详情明细", tags = {"CMDB Bills Screen API"})
    @ApiResponses(value = {@ApiResponse(code = 200, message = "查询成功", response = List.class),
            @ApiResponse(code = 500, message = "内部错误")})
    Map<String, Object> exportSpecificBillsWithResourceType(@RequestParam("department") String department,
                                                              @RequestParam("chargeTime") String chargeTime,
                                                                 HttpServletResponse response);
