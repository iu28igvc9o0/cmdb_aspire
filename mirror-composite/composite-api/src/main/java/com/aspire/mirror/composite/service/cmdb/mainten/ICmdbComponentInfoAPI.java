...
@RequestMapping("/${version}/cmdb/maintenance/component")
public interface ICmdbComponentInfoAPI {
    ...
    @ApiOperation(value = "导出维保部件清单", notes = "导出维保部件清单", tags = {"Cmdb Maintenance Component API"})
    @ApiResponses(value = {@ApiResponse(code = 200, message = "导出成功", response = JSONObject.class),
            @ApiResponse(code = 500, message = "内部错误")})
    JSONObject exportProject(@RequestBody CmdbComponentInfoQueryRequest request,HttpServletResponse response);
}
