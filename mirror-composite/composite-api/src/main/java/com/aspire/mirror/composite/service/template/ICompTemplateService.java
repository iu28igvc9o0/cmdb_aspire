...
public interface ICompTemplateService {
    /**
     * 创建模板信息
     *
...
//
     ...
     * @return TemplateDetailResponse 模板详情响应对象
     */
    @GetMapping(value = "/findByName/{template_name}")
    @ApiOperation(value = "详情", notes = "根据templateId获取模板详情", tags = {"Template API"})
    @ApiResponses(value = {@ApiResponse(code = 200, message = "成功", response = CompTemplateDetailResponse.class),
            @ApiResponse(code = 500, message = "内部错误")})
    CompTemplateDetailResponse findByName(@PathVariable("template_name") String templateName);

    @PostMapping(value = "/exportItems")
    @ApiOperation(value = "导出模板指标项", notes = "导出模板指标项")
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "返回"),
            @ApiResponse(code = 500, message = "Unexpected error")})
    void exportItems(@RequestParam("template_id") String templateId, HttpServletResponse response);
}
