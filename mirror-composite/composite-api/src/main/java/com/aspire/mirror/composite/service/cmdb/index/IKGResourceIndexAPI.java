...
@Api("科管资源首页")
@RequestMapping("${version}/cmdb/kg/index")
public interface IKGResourceIndexAPI {
    ...
    @RequestMapping(value = "/deviceUseByPhy", method = RequestMethod.POST)
    ...
    @ApiResponses(value = {@ApiResponse(code = 200, message = "查询成功", response = String.class),
            @ApiResponse(code = 500, message = "内部错误")})
    String deviceUseByPhy();

    @RequestMapping(value = "/deviceUseByVm", method = RequestMethod.POST)
    @ApiOperation(value = "设备使用性(虚拟机)", notes = "设备使用性(虚拟机)",tags = {"KG CMDB Index API"})
    @ApiResponses(value = {@ApiResponse(code = 200, message = "查询成功", response = String.class),
            @ApiResponse(code = 500, message = "内部错误")})
    String deviceUseByVm();

    @RequestMapping(value = "/deviceUseBySu", method = RequestMethod.POST)
    @ApiOperation(value = "设备使用性(宿主机)", notes = "设备使用性(宿主机)",tags = {"KG CMDB Index API"})
    @ApiResponses(value = {@ApiResponse(code = 200, message = "查询成功", response = String.class),
            @ApiResponse(code = 500, message = "内部错误")})
    String deviceUseBySu();
}
