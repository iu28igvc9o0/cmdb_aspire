...
public class POICascadeUtils2 {
    ...
    private static final String EXCEL_HIDE_SHEET_NAME = "excelhidesheetname";

    private static final String EXCEL_HIDE_SHEET_NAME2 = "excelhidesheetname2";
    ...
    public void generateExcel(XSSFWorkbook wb, List<ExcelDataVo> dataList, String title) {
    ...
    public static void initDropDownList(Workbook workbook, List<ExcelDataVo> dataList) {
        ...
        for (int i = 0; i < dataList.size(); i++) {
            ExcelDataVo excelData = dataList.get(i);
            ...
            if ("listSel".equals(controlCode)) {
                // 下拉
                Row row = hideInfoSheet.createRow(rows);
                ...
                if (CollectionUtils.isNotEmpty(stringList)) {
                    ...
                    creatExcelNameList(workbook, fieldCode, namesList, rows + 1, valueList.size(), false);
                }
                rows++;
                ...
                List<String> stringList = valueList.stream().map(e -> e.toString()).collect(Collectors.toList());
                creatRow(row, stringList.toArray(new String[stringList.size()]));
                String levelType = excelData.getLevelType();
                List<ExcelDataVo> childList = excelData.getChildList();
                if ("parent".equals(levelType) || StringUtils.isEmpty(levelType)) {
                    if (CollectionUtils.isNotEmpty(valueList)) {
                        creatExcelNameList(workbook, fieldCode, namesList, rows + 1, valueList.size(), false);
                    }
                    rows++;
                    if (CollectionUtils.isNotEmpty(childList)) {
                        // stringList = childList.stream().map(e -> e.toString()).collect(Collectors.toList());
                        // creatRow(row, stringList.toArray(new String[stringList.size()]));
                        for (ExcelDataVo child : childList) {
                            String fieldValue = child.getParentFieldValue();
                            List<Object> childValueList = child.getFieldValueList();
                            List<String> subStringList = childValueList.stream().map(e -> e.toString())
                                    .collect(Collectors.toList());
                            if (CollectionUtils.isNotEmpty(subStringList)) {
                                Row subRow = hideInfoSheet.createRow(rows);
                                creatRow(subRow, subStringList.toArray(new String[subStringList.size()]));
                                fieldValue = StringUtils.replaceChineseCharacter(fieldValue);
                                creatExcelNameList(workbook, fieldValue, namesList, rows + 1, subStringList.size(), true);
                            }
                            rows++;
                        }
                    }
     ...
     * @return
     */
    public static void setDataValidation(Workbook wb, List<ExcelDataVo> dataList) {
        ...
        if (sheetIndex > 0) {
            for (int i = 0; i < sheetIndex; i++) {
                ...
                if (!EXCEL_HIDE_SHEET_NAME.equals(sheet.getSheetName())) {
                    int startRow = 2;
                    List<ExcelDataVo> child = Lists.newArrayList();
                    for (int j = 0; j < dataList.size(); j++) {
                        ExcelDataVo excelData = dataList.get(j);
                        String controlCode = excelData.getFieldType();
                        String fieldCode = excelData.getFieldCode();
                        String fieldName = excelData.getFieldName();
                        List<Object> valueList = excelData.getFieldValueList();
                        if (!Arrays.asList("listSel", "cascader").contains(controlCode)) {
                            continue;
                        }
                        // if ("listSel".equals(controlCode)) {
                        // // 下拉
                        ...
                        if ("parent".equals(excelData.getLevelType())) {
                            if (CollectionUtils.isNotEmpty(excelData.getChildList())) {
                                child.addAll(excelData.getChildList());
                            }
                        }
                        Integer index = getParentIndex(excelData, child);
                        log.debug("parentIndex=={}", index);
                        ...
                        String letter = numberToLetter(index) + startRow;
                        log.debug("letter=={}", letter);
                        DataValidation validation = getDataValidationCustomFormula(sheet, "INDIRECT($" + letter + ")", startRow,
                                excelData.getColIndex());
                        sheet.addValidationData(validation);
                        // }
                    }
    ...
    private static Integer getParentIndex(ExcelDataVo data, List<ExcelDataVo> dataList) {
        String fieldCode = data.getFieldCode();
        for (ExcelDataVo excelData : dataList) {
            if (fieldCode.equals(excelData.getFieldCode())) {
                ExcelDataVo parent = excelData.getParent();
                if (parent != null) {
                    return excelData.getParent().getColIndex();
                }
            }
        }
        return null;
        ...
        for (int i = 0; i < result.size(); i++) {
            ...
            if (i == 0 || ((i >= 8 && i <= 20) && (i != 10 && i != 12 && i != 19 && i != 13))) { // 此判断用于 无下拉框的列
                ...
                for (Map<String, Object> map : business) {
                    ...
                    String[] business2 = business2List.toArray(new String[0]);
                    Row row = hideInfoSheet.createRow(L);
                ...
                for (Map<String, Object> map : device) {
                    ...
                    String[] devices = deviceLists.toArray(new String[0]);
                    Row row = hideInfoSheet.createRow(L);
                ...
                for (Map<String, Object> map : model) {
                    ...
                    String[] models = modelLists.toArray(new String[0]);
                    String[] devices = deviceLists.toArray(new String[0]);
