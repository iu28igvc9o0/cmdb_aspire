package com.migu.tsg.microservice.atomicservice.composite.clientservice.bills.log;
 ...
 */
@FeignClient("BILLS")
public interface CmdbBillsLogClient {
    @RequestMapping(value = "/v1/cmdb/bill/log/list",method = RequestMethod.POST)
    ...
    @RequestMapping(value = "/v1/cmdb/bill/log/saveBillLog",method = RequestMethod.POST)
    @ApiOperation(value = "获取账单操作日志", notes = "获取账单操作日志",
            response = Map.class, tags = {"CMDB Bill logs API"})
    @ApiResponses(value = {@ApiResponse(code = 200, message = "查询成功", response = Map.class),
            @ApiResponse(code = 500, message = "内部错误")})
    ListResult<Map<String, Object>> saveBillLogs(@RequestBody BillsLog billsLog);
