...
//					cellvalue = cell.getDateCellValue().toLocaleString();
	 ...
	 * @throws  Exception 
	 */
	public List<CompMaintenHardwareRequest> doUploadMaintenHardwareData(MultipartFile file, List<String> resourcePoolList,List<String> systemNameList,
			List<String> deviceClassifyList,List<String> deviceTypeList,List<String> deviceModelList,List<String> optionsTrueFalseList,
	        List<String> originBuyExplainList,List<String> maintenFactoryList,List<String> realMaintenList) throws  Exception {

		List<CompMaintenHardwareRequest> list = new ArrayList<CompMaintenHardwareRequest>();
		...
		for (int j = 1; j <= sheet.getLastRowNum(); j++) {
			...
			row = sheet.getRow(j);
			int t = 0;
			CompMaintenHardwareRequest	tpm = new CompMaintenHardwareRequest();
			while (t < colNum) {
				// 每个单元格的数据内容用"-"分割开，以后需要时用String类的replace()方法还原数据
				// 也可以将每个单元格的数据设置到一个javabean的属性中，此时需要新建一个javabean
...
//	 * @throws Exception
//	 */
//	public List<CompMaintenSoftwareRequest> doUploadMaintenSoftwareData(MultipartFile file,List<String> companyList,List<String> unitList) throws Exception {
//
//
//		List<CompMaintenSoftwareRequest> list = new ArrayList<CompMaintenSoftwareRequest>();
//		boolean convertSuccess = true;// 时间格式是否正确
//		// 指定日期格式为四位年/两位月份/两位日期，注意yyyy/MM/dd区分大小写；
//		SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
//
...
//				row = sheet.getRow(j);
//				int t = 0;
//				CompMaintenSoftwareRequest	tpm = new CompMaintenSoftwareRequest();
//				while (t < colNum) {
//					// 每个单元格的数据内容用"-"分割开，以后需要时用String类的replace()方法还原数据
//					// 也可以将每个单元格的数据设置到一个javabean的属性中，此时需要新建一个javabean
//					// str = getStringCellValue(row.getCell(t)).trim();
...
//					case 1:
//						if (str==null) {
//							throw new RuntimeException("第"+(j+1)+"行项目名称为空");
//						}
//						tpm.setProject(str);
//						break;
//					case 2:
...
//					case 3:
//						if (str==null) {
//							throw new RuntimeException("第"+(j+1)+"行软件名称为空");
//						}
...
//								throw new RuntimeException("第"+(j+1)+"行单位错误");
//							}
//						}
//						tpm.setUnit(str);
//						break;
...
//						   boolean isMatch = p.matcher(str).matches();
//						   if(!isMatch){
//								throw new RuntimeException("第"+(j+1)+"行数量格式错误");
//						   }
//						 }
//						tpm.setNumber(str);
//						break;
//					case 6:
//						if (str==null) {
//							throw new RuntimeException("第"+(j+1)+"行厂商名称为空");
//						}
//						if (!companyList.contains(str) ) {
//							throw new RuntimeException("第"+(j+1)+"行厂商不在范围内");
//						}
//						tpm.setCompany(str);
//						break;
//					case 7:
...
//						   boolean isMatch = p.matcher(str).matches();
//						   if(!isMatch){
//								throw new RuntimeException("第"+(j+1)+"行手机格式错误");
//						   }
//						}
//						tpm.setTelephone(str);
//						break;
//					case 9:
//                       if (StringUtils.isNotBlank(str)) {
//
...
//	}
	 ...
	 * @throws Exception 
	 */
	public List<CompInvalidResourceResp> doReadInvalidResourceData(MultipartFile file ) throws Exception {

		List<CompInvalidResourceResp> list = new ArrayList<CompInvalidResourceResp>();
		boolean convertSuccess = true;// 时间格式是否正确
		// 指定日期格式为四位年/两位月份/两位日期，注意yyyy/MM/dd区分大小写；
		SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
		...
		for (int j = 1; j <= sheet.getLastRowNum(); j++) {
			...
			row = sheet.getRow(j);
			int t = 0;
			CompInvalidResourceResp	tpm = new CompInvalidResourceResp();
			while (t < colNum) {
				// 每个单元格的数据内容用"-"分割开，以后需要时用String类的replace()方法还原数据
				// 也可以将每个单元格的数据设置到一个javabean的属性中，此时需要新建一个javabean
				// str = getStringCellValue(row.getCell(t)).trim();
				...
				case 1:
					...
					break;
				case 2:
					tpm.setDepartment1 (str);
					break;
				case 3:
					tpm.setDepartment2(str);
					break;
				case 4:

					tpm.setBizSystem (str);
					break;
				case 5:
					tpm.setPodName (str);
					break;
				case 6:
					...
					tpm.setPhysicalNumber (str);
					break;
				case 7:
					tpm.setVirtualNumber (str);
					break;
				case 8:
