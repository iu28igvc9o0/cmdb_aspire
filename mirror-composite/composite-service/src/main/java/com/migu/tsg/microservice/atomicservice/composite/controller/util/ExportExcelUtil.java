...
private static final long serialVersionUID = 2165773254718823136L;  
    ...
    Sheet sheet = workbook.createSheet();
    workbook.setSheetName(sheetNum,sheetTitle);
	// 对评估得分信息导入模板的特殊处理，冻结前两列
	if("评估得分信息导入模板".equals(sheetTitle)) {
...
//导出cmdb-device-assets   excel表格    模板
						...
						if(cellIndex == 9) {
							...
							if(floatValue == 0) {
								...
								if(floatValue.toString().length()>6) {
									...
									va ="↓ " + floatValue.toString().substring(1) + "%";
								}
								cell.setCellValue(va);
	...
	// 导出 维保周期 特定格式的导出
	public void exportExcelForPeriod(Workbook workbook, int sheetNum, String sheetTitle, String[] headers, List<Map<String, Object>> result, String[] listKey ) throws Exception{
		...
		int[] columnSpan = new int[]{0,1,2,3,4,10,11,12};
		int[] columnNoSpan = new int[]{7,8,9};
		// 遍历集合数据，产生数据行
		if (result.size() != 0) {
			int index = 1;
			for(int i=0;i<result.size();i++) {
				...
				for(int q=0;q<columnNoSpan.length;q++){
					String zeroKey = listKey[columnNoSpan[q]];
					if(map.get(zeroKey) == null || "".equals(map.get(zeroKey).toString())) {
						map.put(zeroKey,"0");
					}
				}
				// 创建行单元
				...
				if(i % 2 == 0) {
					// 处理差值的列
					map.put("period","上期周期");
					dealSpecialData(result,i);
					for(int j=0;j<listKey.length;j++) {
						// 处理需要合并列单元
						boolean flag = false;
						for(int k=0;k<columnSpan.length;k++) {
							if(j == columnSpan[k]) {
								flag = true;
								break;
							}
						...
						if(flag) {
							CellRangeAddress region = new CellRangeAddress(index, index+1, j, j);
						...
						if( null != map && null != map.get(key)) {
							// 创建单元格
							Cell cell = row.createCell(cellIndex);
							String va = map.get(key) == null ?  "" : map.get(key).toString();
							if(va.getBytes().length >= testva[cellIndex].getBytes().length)
							{
								testva[cellIndex]=va;
							}
					...
					cellPeriod.setCellValue("本期周期");
					// 依据表格，服务数量列特殊处理
					Cell cellServiceNum = row.createCell(6);
					cellServiceNum.setCellValue(map.get(listKey[6]) == null ? "" : map.get(listKey[6]).toString());
					for(int p=0;p<columnNoSpan.length;p++) {
						Cell cell = row.createCell(columnNoSpan[p]);
						String tmpRs = map.get(listKey[columnNoSpan[p]]) == null ? "0" : map.get(listKey[columnNoSpan[p]]).toString();
						cell.setCellValue(tmpRs);
					}
				}
				index++;
			}
	...
	private void dealSpecialData(List<Map<String, Object>> result,int index) {
    	Map map = result.get(index);
		Map nextMp = result.get(index + 1);
		map.put("deviceNumDiff",calStringNum(map.get("deviceNum"),nextMp.get("deviceNum")));
		map.put("moneyDiff",calStringNum(map.get("money"),nextMp.get("money")));
		map.put("rateDiff",calStringNum(map.get("discountRate"),nextMp.get("discountRate")));
		map.put("projectName",nextMp.get("projectName"));
		map.put("contractProduce",nextMp.get("contractProduce"));
		map.put("serviceProduce",nextMp.get("serviceProduce"));
		map.put("deviceType",nextMp.get("deviceType"));
		map.put("deviceArea",nextMp.get("deviceArea"));
	}

	private String calStringNum(Object a,Object b) {
		...
		String rs = String.valueOf(Float.parseFloat(b.toString()) - Float.parseFloat(a.toString()));
		int len = rs.length();
		boolean zero = true;
		if(rs.contains(".")) {
			for(int i=rs.indexOf(".")+1;i<len;i++) {
				if(rs.charAt(i) != '0') {
					zero = false;
					break;
				}
		...
		if(zero) {
			return rs.substring(0,rs.indexOf("."));
		} else {
			return rs;
		}
	}
	...
	public void exportExcel(Workbook workbook, int sheetNum, String sheetTitle, List<List<String>> headers, List<LinkedHashMap<String, Object>> result, String[] listKey ) throws Exception{
		...
		for (List<String> header : headers) {
			...
			for (String title : header) {
				...
				cell.setCellValue(text.toString());
				if (j > 0 && title.equals(header.get(j-1))) {
					sheet.addMergedRegion(new CellRangeAddress(i, i, j-1, j));
				}
				if (i > 0 && headers.get(i).get(j).equals(headers.get(i-1).get(j))) {
		...
		toHandleDataCell(sheet, style, headers, result);
...
//		}
	...
	private void toHandleDataCell(Sheet sheet, CellStyle style, List<List<String>> headers, List<LinkedHashMap<String, Object>> result) {

			for (int i = 0; i < result.size(); i++) {
				Row row = sheet.createRow(headers.size() + i);
				LinkedHashMap<String, Object> dataMap = result.get(i);
				List<String> firstHeader = headers.get(0);
				List<String> lastHeader = headers.get(headers.size()-1);
				for (int j=0; j< lastHeader.size() ; j++) {
					Cell cell = row.createCell(j);
//					cell.setCellStyle(style);
					HSSFRichTextString text = new HSSFRichTextString();
					if (dataMap.get(lastHeader.get(j)) == null) {
						Map<String, String> subData = (Map<String, String>)dataMap.get(firstHeader.get(j));
						text = new HSSFRichTextString(subData.get(lastHeader.get(j)));
					} else {
						text = new HSSFRichTextString(dataMap.get(lastHeader.get(j)).toString());
					}
					cell.setCellValue(text.toString());
				}
			}
