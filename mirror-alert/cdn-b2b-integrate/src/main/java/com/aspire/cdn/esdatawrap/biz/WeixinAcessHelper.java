...
final class WeixinAcessHelper {
	...
	public String getAgentIdByProvinceName(String provinceName) {
		...
		if (agentSecret == null) {
			log.warn("There is no weixin agent and secret info for the province: {}.", provinceName);
			return null;
		}
	...
	public String getWeixinCorpAccessTokenByProvince(String provinceName) {
		Map<String, String> agentSecret = proAgentSecretCache.get(provinceName);
		if (agentSecret == null) {
			throw new RuntimeException("There is no weixin agent and secret info for the province: " + provinceName);
		}
