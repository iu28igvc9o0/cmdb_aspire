...
import com.aspire.mirror.alert.server.annotation.ResFilterKeysValid;
...
public class ControllerAuthAspect {
    ...
    @Value("${auth.data.flag:true}")
    private Boolean authFlag;

    /**
     * 切入点： 定位到 controller中的方法 com.aspire.mirror.alert.server.controller
     */
    @Pointcut("execution(public * com.aspire.mirror.alert.server.controller..*Controller.*(..)) || execution(public * com.aspire.mirror.alert.server.v2.*.*.*Controller.*(..))")
    ...
    */
    ...
    private void checkResFilterConfigKeys(final JoinPoint joinPoint, final RequestAuthContext authCtx) {
    	...
    	// 管理员不验证
    	if (reqUser.isAdmin() || reqUser.isSuperUser() || !authFlag) {
    		return;
    	}
