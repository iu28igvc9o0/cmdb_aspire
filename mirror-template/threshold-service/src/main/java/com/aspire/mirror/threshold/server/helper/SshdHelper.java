package com.aspire.mirror.threshold.server.helper;

import com.aspire.mirror.threshold.server.config.CommonSftpServerConfig;
...
public class SshdHelper {
    private static final String LINUX_CMD_LS_FILE = "if [ -d \"%s\" ] ;   then     ls \"%s\" | grep -v ^d|awk '{print i$0}' i=\"%s\"'/';   else     ls \"%s\" ; fi";
...
//            localDownDir = Files.createDirectory(Paths.get(localDownDir.toFile().getAbsolutePath() + File.separator + filePath));
            ...
            return Pair.of(localDownDir.toFile().getAbsolutePath(), filePathList);
        } catch (IOException e) {
            log.error("download file for sftp is error!", e);
        }
        return Pair.of("", filePathList);
    }
...
//        String mimeType = "application/octet-stream";
    ...
    public Pair<String, List<File>> downloadFile(String removePath, String localPath) throws IOException {
        removePath = sftpConfig.getRootDirectory() + removePath;
//        String beforecmd = String.format(LINUX_CMD_LS_FILE, removePath, removePath, removePath, removePath);
//        SshUtil.SshResultWrap beforeResult = SshUtil.executeShellCmd(this.getSshdServer(), 0, beforecmd);
//        String[] filePathList = {removePath};
//        if (beforeResult.isSuccess()) {
//            filePathList = beforeResult.getBizLog().split("\n");
//        }
        Path localDownDir = Files.createTempDirectory(localPath);
        Pair<SshUtil.SshResultWrap, List<Path>> downResult
                = SshUtil.sftpDownloadDir(this.getSshdServer(), removePath, localDownDir.toFile().getCanonicalPath());
        SshUtil.SshResultWrap wrap = downResult.getLeft();
        List<File> files = Lists.newArrayList();
        if (!wrap.isSuccess()) {
            throw new RuntimeException(wrap.getBizLog());
        } else {
            if (!CollectionUtils.isEmpty(downResult.getRight())) {
                downResult.getRight().forEach(item -> files.add(item.toFile()));
            }
        }
        return Pair.of(localDownDir.toFile().getAbsolutePath(), files);
    }
}
