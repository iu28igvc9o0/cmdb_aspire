...
import static org.apache.sshd.client.subsystem.sftp.SftpClient.*;
...
//			client.setNioWorkers(16);
	...
	public static Pair<SshResultWrap, List<Path>> sftpDownloadDir(SshdServer sftpServer, String remoteDirPath, String localDirectory) {
		SshClient client = getGlobalSshClient();
		try (ClientSession session = client.connect(sftpServer.getLoginUser(),
				new InetSocketAddress(sftpServer.getIpAddress(), sftpServer.getPort())).verify(
				DEFAULT_CONNECT_TIMEOUT, TimeUnit.SECONDS).getSession()) {
			...
			try (SftpFileSystem fs = SftpClientFactory.instance().createSftpFileSystem(session);) {
				List<Path> resultPathList = Lists.newArrayList();
				if (Files.exists(fs.getDefaultDir().resolve(remoteDirPath))) {
					Path remoteDirFile = fs.getDefaultDir().resolve(remoteDirPath);
					stream =  Files.list(remoteDirFile);
					List<Path> listSubPath = stream.collect(Collectors.toList());
					for (Path itemPath : listSubPath) {
						String fileName = itemPath.getFileName().toString();
						Path localeDown = Paths.get(localDirectory, fileName);
						Files.deleteIfExists(localeDown);
						Files.copy(itemPath, localeDown);
						resultPathList.add(localeDown);
					}
				}
				return Pair.of(SshResultWrap.ofSuccess(ResultFlagEnum.FLAG_9), resultPathList);
			} catch (Exception e) {
				log.error("Exception when invoke sftpDownloadDir1().", e);
			} finally {
				if (stream != null) {
					stream.close();
				}
			...
			log.error("Exception when invoke sftpDownloadDir2().", e);
			return Pair.of(SshResultWrap.ofException(e, ResultFlagEnum.FLAG_101), null);
		}
		return null;
	}
	...
	public enum ResultFlagEnum {
		...
		private ResultFlagEnum(int flag, String label) {
			this.flag = flag;
			this.label = label;
		}
