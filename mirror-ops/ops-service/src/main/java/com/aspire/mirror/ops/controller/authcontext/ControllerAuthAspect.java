package com.aspire.mirror.ops.controller.authcontext;
...
import com.aspire.mirror.ops.controller.authcontext.RequestAuthContext.RequestHeadUser;
...
public class ControllerAuthAspect {
     ...
     */
    @Pointcut("execution(public * com.aspire.mirror.ops.controller..*Controller.*(..))")
    ...
    */
    ...
    private void checkResFilterConfigKeys(final JoinPoint joinPoint, final RequestAuthContext authCtx) {
    	RequestHeadUser reqUser = authCtx.getUser();
    	// 管理员不验证
        ...
        if (MapUtils.isEmpty(resFilterCfgMap)) {
        	throw new OpsServiceException(tip);
        }
...
//        final String authAccounts = httpReq.getHeader("head_authAccounts");
        ...
        if (StringUtils.isNotBlank(resFilterConfigBase64)) {
			try {
				...
				throw new OpsServiceException(e);
			}
        }

//        List<String> userList = null;
//        if (!StringUtils.isBlank(authAccounts)) {
//            userList = Lists.newArrayList(Arrays.asList(authAccounts.split(",")));
//        }
        //根据报文头信息生成用户信息，不需要转义
        RequestHeadUser user = new RequestHeadUser(namespace, username, isAdminBool, isSuperUserBool, resFilterConfig);
