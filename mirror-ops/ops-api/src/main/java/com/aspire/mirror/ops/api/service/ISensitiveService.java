 ...
 * 类名称:    ISensitiveService.java
 * 类描述:    敏感指令暴露接口
 * 创建人:    JinSu
 * 创建时间:  2020/1/20 11:28
 * 版本:      v1.0
 */
@Api(value = "敏感指令服务")
...
public interface ISensitiveService {
    ...
    @PostMapping(value = "/saveSensitiveReview", produces = MediaType.APPLICATION_JSON_VALUE)
    @ApiOperation(value = "保存敏感指令审核数据", notes = "保存敏感指令审核数据", response = GeneralResponse.class, tags = {"Sensitive service API"})
    @ApiResponses(value = {@ApiResponse(code = 200, message = "返回", response = GeneralResponse.class),
            @ApiResponse(code = 500, message = "Unexpected error")})
    GeneralResponse saveSensitiveReview(@RequestBody SensitiveReview sensitiveReview);

    @GetMapping(value = "/getSensitiveReview", produces = MediaType.APPLICATION_JSON_VALUE)
    @ApiOperation(value = "根据id查询敏感指令审核数据", notes = "根据id查询敏感指令审核数据", response = GeneralResponse.class, tags = {"Sensitive service API"})
    @ApiResponses(value = {@ApiResponse(code = 200, message = "返回", response = SensitiveConfig.class),
            @ApiResponse(code = 500, message = "Unexpected error")})
    SensitiveReview querySensitiveReviewById(@RequestParam("review_id") Long reviewId);

    @GetMapping(value = "/getSensitiveReviewByPipelineInstanceId", produces = MediaType.APPLICATION_JSON_VALUE)
    @ApiOperation(value = "根据作业实例ID查询敏感指令审核数据", notes = "根据作业实例ID查询敏感指令审核数据", response = SensitiveReview.class, tags = {"Sensitive service API"})
    @ApiResponses(value = {@ApiResponse(code = 200, message = "返回", response = SensitiveConfig.class),
            @ApiResponse(code = 500, message = "Unexpected error")})
    ...
    @ApiOperation(value = "根据ID修改敏感指令审核数据", notes = "根据ID修改敏感指令审核数据", response = GeneralResponse.class, tags = {"Sensitive service API"})
    @ApiResponses(value = {@ApiResponse(code = 200, message = "返回", response = GeneralResponse.class),
            @ApiResponse(code = 500, message = "Unexpected error")})
    GeneralResponse reviewInstance(@RequestParam("review_ids") String reviewIds, @RequestParam("review_status") Integer reviewStatus);

    @DeleteMapping(value = "/removeSensitiveReview/{review_id}", produces = MediaType.APPLICATION_JSON_VALUE)
    @ApiOperation(value = "删除敏感指令审核数据", notes = "删除敏感指令审核数据", response = GeneralResponse.class, tags = {"Sensitive service API"})
    @ApiResponses(value = {@ApiResponse(code = 200, message = "返回", response = GeneralResponse.class),
            @ApiResponse(code = 500, message = "Unexpected error")})
    GeneralResponse removeSensitiveReview(@PathVariable("review_id") Long reviewId);

    @DeleteMapping(value = "/removeSensitiveReviewByInstanceId/{pipeline_instance_id}", produces = MediaType.APPLICATION_JSON_VALUE)
    @ApiOperation(value = "删除敏感指令审核数据", notes = "删除敏感指令审核数据", response = GeneralResponse.class, tags = {"Sensitive service API"})
    @ApiResponses(value = {@ApiResponse(code = 200, message = "返回", response = GeneralResponse.class),
            @ApiResponse(code = 500, message = "Unexpected error")})
    GeneralResponse removeSensitiveReviewByInstanceId(@PathVariable("pipeline_instance_id") Long pipelineInstanceId);
